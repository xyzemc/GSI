...
PDY=`echo $corr_obs_adate | cut -c1-8`
cyc=`echo $corr_obs_adate | cut -c9-10`
GDATE=`$ndate -06 $corr_obs_adate`
...
datobs=$corr_obs_datobs/gdas.$PDY/$cyc
datanl=$corr_obs_datobs/gdas.$PDY/$cyc
datges=$corr_obs_datges/gdas.$gPDY/$gcyc
datens=$corr_obs_datges/enkfgdas.$gPDY/$gcyc
...
if [ $ICO2 -gt 0 ] ; then
        ...
        yyyy=$(echo ${CDATE:-$corr_obs_adate}|cut -c1-4)
...
if [ "$debug" = ".false." ]; then
   . $scripts/regression_namelists.sh global_corr_obs
else
   . $scripts/regression_namelists_db.sh global_corr_obs
fi
...
anavinfo=$fixgsi/global_anavinfo_corrobs.txt
