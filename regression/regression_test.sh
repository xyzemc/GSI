...
# First, wall time (both maximum allowable time and max/min allowable deviation)
   ...
   # This part is for the maximum allowable time (operationally)
     ...
     if [[ "$timelogic" = 1 ]]; then
       ...
       echo 'resulting in MaxTime Failure of the regression test.'
       echo  
       failed_test=1
     else
...

   {
     ...
     if [[ "$timelogic" = 1 ]]; then
       ...
       echo 'resulting in Time Threshhold Failure of the regression test.'
       echo
       failed_test=1
     else
...

   {
     ...
     if [[ "$timelogic" = 1 ]]; then
       ...
       echo 'resulting in Time Thresh2 Failure of the regression test.'
       echo
       failed_test=1
     else
...

   {
     ...
     if [[ $(awk '{ print $8 }' memory.$exp1.txt) -gt $maxmem ]]; then
       ...
       echo 'resulting in Max Memory Failure of the regression test.'
       echo
       failed_test=1
     else
...
#      echo 'since the corresponding penalties and gradients are identical with '$(grep -c 'cost,grad,step' penalty.${exp1}-${exp2}.txt)' lines different.'
      ...
      echo 'thus the regression test has Failed (cost) for '${exp1}' and '${exp2}' analyses.'
...
then
   ...
   echo 'The results between the two runs ('${exp1}' and '${exp2}') are not reproducible'
   echo 'Thus, the case has Failed (wrf_inout06) the regression tests.'
   echo
   failed_test=1
fi
...
then
   ...
   echo 'The results between the two runs ('${exp1}' and '${exp2}') are not reproducible'
   echo 'Thus, the case has Failed (siginc) the regression tests.'
   echo
   failed_test=1
fi
