  ...
  foreach(_MPI_LINK_FLAG IN LISTS MPI_ALL_LINK_FLAGS)
    ...
    if (NOT "${_MPI_LINK_FLAG}" MATCHES "(-Wl,|-Xlinker )-z,noexecstack")
      if (MPI_LINK_FLAGS_WORK)
        set( tmpflags ${MPI_LINK_FLAGS_WORK} )
        set(MPI_LINK_FLAGS_WORK "${tmpflags} ${_MPI_LINK_FLAG}")
...
function(_IN_LIST _search _list)
  list (FIND _list _search _index)
  set(_item_found FALSE)
  message("looking for ${_search} in list ${_list}")
  if (${_index} GREATER -1)
    message("found in list")
    set(_item_found TRUE )
  endif()
endfunction()
...
macro(_MPI_assemble_include_dirs LANG)
  if("${MPI_${LANG}_COMPILER}" STREQUAL "${CMAKE_${LANG}_COMPILER}")
    ...
    if("${LANG}" MATCHES "(C|CXX)")
      ...
      _IN_LIST( ${MPI_${LANG}_MODULE_DIR} ${MPI_${LANG}_INCLUDE_DIRS} )
#     if(MPI_${LANG}_MODULE_DIR AND NOT "${MPI_${LANG}_MODULE_DIR}" IN_LIST MPI_${LANG}_INCLUDE_DIRS)
      if(MPI_${LANG}_MODULE_DIR AND NOT ${_item_found} )
...
foreach(LANG IN ITEMS C CXX Fortran)
  if(CMAKE_${LANG}_COMPILER_LOADED)
    message("HEY, ${MPI_FIND_COMPONENTS} !!!!")
    if(MPI_FIND_COMPONENTS)
      _IN_LIST(${LANG} ${MPI_FIND_COMPONENTS} )
      set(found_lang ${_item_found})
      _IN_LIST(${MPICXX} ${MPI_FIND_COMPONENTS} )
      set(found_mpicxx ${_item_found})
    else()
      set(${found_lang} FALSE)
      set(${found_mpicxx} FALSE)
    endif()
...
#   elseif( ${LANG} STREQUAL CXX AND NOT MPI_CXX_SKIP_MPICXX AND MPICXX IN_LIST MPI_FIND_COMPONENTS )
    elseif( ${LANG} STREQUAL CXX AND NOT MPI_CXX_SKIP_MPICXX AND ${found_mpicxx})
      set(_MPI_FIND_${LANG} TRUE)
    else()
...
#   if( ${LANG} STREQUAL CXX AND NOT MPICXX IN_LIST MPI_FIND_COMPONENTS )
    ...
    if( ${LANG} STREQUAL CXX AND NOT ${found_mpicxx} )
    ...
    message("HOOO ${MPI_${LANG}} and ${MPI_${LANG}_REQUIRED_VARS} and ${MPI_${LANG}_VERSION}")
...
foreach(LANG IN ITEMS C CXX Fortran)
  ...
  if((NOT MPI_FIND_COMPONENTS AND CMAKE_${LANG}_COMPILER_LOADED) OR (found_lang) )
    list(APPEND _MPI_REQ_VARS "MPI_${LANG}_FOUND")
    message("mpi_req_vars is ${_MPI_REQ_VARS}")
  endif()
endforeach()
if(MPI_FIND_COMPONENTS)
  _IN_LIST( ${MPICXX} ${MPI_FIND_COMPONENTS} )
...
if(${found_lang})
  message("HEY in this if statement")
  list(APPEND _MPI_REQ_VARS "MPI_MPICXX_FOUND")
endif()
#set(_MPI_MIN_VERSION MPI_C_VERSION)
message("HEY 2 ${_MPI_REQ_VARS} ${_MPI_MIN_VERSION}")
