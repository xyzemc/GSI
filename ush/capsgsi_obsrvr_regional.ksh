...
#####################################################
#SBATCH -e gsi4hxb.%j.%N.log
#SBATCH -o gsi4hxb.%j.%N.log
#SBATCH -J gsi4hxb
#SBATCH -p skx-dev
#SBATCH -N 4
#SBATCH -n 140
##SBATCH --tasks-per-node=35
#SBATCH -t 00:30:00
...
#####################################################
#
# ADATETIME= analysis time (complete form, in YYYYMMDDHHNN)
# ANAL_TIME= analysis time to the hour digit (YYYYMMDDHH)
...
# ENS_ROOT = path where ensemble background files exist
  ADATETIME=201904301900
  ANAL_TIME=`echo $ADATETIME | cut -c1-10`
  HH=`echo $ANAL_TIME | cut -c9-10`
  MN=`echo $ADATETIME | cut -c11-12`
  N_CYC=1
  ...
  RUN_NAME=HYB40%bk_verif
  OBS_ROOT=/scratch/01540/ctong/GSIobs/BOKverif
  BK_ROOT=${JOB_DIR}/cycle${N_CYC}/GENBE
  GSI_ROOT=/home1/01540/ctong/ProdGSI
  CRTM_ROOT=/work/01540/ctong/CRTM_v2.3.0
  ENS_ROOT=the_directory_where_ensemble_backgrounds_are_located
      #ENS_ROOT is not required if not running hybrid EnVAR 
  GSI_EXE=/home1/01540/ctong/build/bin/gsi.x  #assume you have a copy of gsi.x here
  WORK_ROOT=${JOB_DIR}/cycle${N_CYC}/${RUN_NAME}
  FIX_ROOT=$GSI_ROOT/fix
  GSI_NAMELIST=${GSI_ROOT}/ush/comgsi_caps_namelist.sh
  PREPBUFR=${OBS_ROOT}/rap.t${HH}z.prepbufr.tm00.nr
  L2RWBUFR_GT=l2rwbufr_gatetilt.${ADATETIME}
  L2RWBUFR_CL=l2rwbufr_colmtilt.${ADATETIME}
  DBZBUFR=dbzbufr_mrmsg2.${ADATETIME}
  BK_FILE=${BK_ROOT}/dynvars.mean
...
# Radar DA section namelist (note: variable name with suffix "_op" comparing to its name used in code)
  use_rw=Yes
  use_dbz=Yes
  if [ "${use_rw}" = Yes ] || [ "${use_rw}" = YES ] || [ "${use_rw}" = yes ] ; then
      l_use_rw_caps_op='.true.'
  else
      l_use_rw_caps_op='.false.'
  fi
  if [ "${use_dbz}" = Yes ] || [ "${use_dbz}" = YES ] || [ "${use_dbz}" = yes ] ; then
      l_use_dbz_caps_op='.true.'
  else
      l_use_dbz_caps_op='.false.'
  fi
  mphyopt_op=2
  oe_rw_op=3.0
  oe_dbz_op=5.0
  refl_lowbnd_rw_op=5.0
  refl_lowbnd_dbz_op=-60.0
  l_set_be_rw_op='.false.'
  l_set_be_dbz_op='.true.'
  be_sf_op=0.0444
  hscl_sf_op=20000.0
  vscl_sf_op=1.5
  be_vp_op=0.0444
...
# be_qr/qs/qg dpeneds on log or non-log (set after l_use_log_qx)
  hscl_qx_op=6000.0
  vscl_qx_op=1.5
  l_decouple_sf_vp_op='.true.'
  l_decouple_sf_tps_op='.true.'
  rw_obs4wrd_bmwth_op=2
  l_plt_diag_rw_op='.true.'
  l_chk_bmwth_op='.true.'
  l_correct_azmu_op='.true.'
  l_correct_tilt_op='.true.'
  i_correct_tilt_op=2
  l_azm_east1st_op='.true.'
  l_use_log_qx_op='.true.'
  l_be_T_dep_op='.true.'
  l_gpht2gmht_op='.false.'
  l_plt_be_stats_op='.true.'
  lvldbg_op=2
  i_melt_snow_op=-1              # <0: dry snow;    100: wet snow;     other(>=0 and /=100): melting on temperature
  i_melt_graupel_op=100           # <0: dry graupel; 100: wet graupel;  other(>=0 and /=100): melting on temperature
# set be_rq/qs/qg for log and non-log respectively
  if [ "$l_use_log_qx_op" = .true. ] || [ "$l_use_log_qx_op" = .TRUE. ] || [ "$l_use_log_qx_op" = .True. ] || [ "$l_use_log_qx_op" = .T. ] ; then
      be_qr_op=0.4
      be_qs_op=0.4
      be_qg_op=0.4
  else
...
# BK_FILE_mem   path and base for ensemble members
  no_member=40
  BK_FILE_mem1=${BK_ROOT}/dynvars.e
  BK_FILE_mem2=${BK_ROOT}/tracer.e
#
#
#####################################################
...
case $ARCH in
   ...
   'LINUX_SLURM_STAMPEDE2')
      if [ $GSIPROC = 1 ]; then
         #### Linux cluster SLURM on Stampede2
         RUN_COMMAND=""
      else
         ###### Linux workstation -  mpi run
        RUN_COMMAND="ibrun"
...
# link to the radar radial wind data
if [ "${use_rw}" = Yes ] || [ "${use_rw}" = YES ] || [ "${use_rw}" = yes ] ; then
  rm -f                             l2rwbufr_cltl
  ln -s ${OBS_ROOT}/${L2RWBUFR_CL}  l2rwbufr_cltl
fi

# link to the radar reflectivity data
if [ "${use_dbz}" = Yes ] || [ "${use_dbz}" = YES ] || [ "${use_dbz}" = yes ] ; then
  rm -f                             dbzbufr
  ln -s ${OBS_ROOT}/${DBZBUFR}      dbzbufr
fi
...
if [ ${bkcv_option} = GLOBAL ] ; then
  ...
  if [ ${bk_core} = FV3 ] ; then
    if [ "${use_dbz}" = Yes ] || [ "${use_dbz}" = YES ] || [ "${use_dbz}" = yes ] ; then
      ANAVINFO=${FIX_ROOT}/anavinfo_fv3_netcdf_dbz
    else
      ANAVINFO=${FIX_ROOT}/anavinfo_fv3_netcdf_dbz
    fi
  fi
...
if [ "${use_prepbufr}" = Yes ] || [ "${use_prepbufr}" = YES ] || [ "${use_prepbufr}" = yes ] ; then
  CONVINFO=${FIX_ROOT}/global_convinfo_dbzrw.txt
else
  CONVINFO=${FIX_ROOT}/global_convinfo_dbzrw_noconv.txt
fi
...
if [ ${bk_core} = FV3 ] ; then
   ...
   bk_if_netcdf='.true.'
   (( precyc = $N_CYC - 1 ))
   ln -s $JOB_DIR/001/fv3_C768_201904301800/RESTART/coupler.res     $WORK_ROOT/coupler.res
   ln -s $JOB_DIR/001/fv3_C768_201904301800/grid_spec.nc            $WORK_ROOT/fv3_grid_spec
   ln -s $JOB_DIR/001/fv3_C768_201904301800/RESTART/fv_core.res.nc  $WORK_ROOT/fv3_akbk
   cp $BK_ROOT/dynvars.mean                                         $WORK_ROOT/fv3_dynvars
   cp $BK_ROOT/tracer.mean                                          $WORK_ROOT/fv3_tracer
   ln -s $JOB_DIR/001/fv3_C768_201904301800/RESTART/sfc_data.nc     $WORK_ROOT/fv3_sfcdata
   cp $BK_ROOT/dynvars.mean                                         $WORK_ROOT/ana_dynvars
   cp $BK_ROOT/tracer.mean                                          $WORK_ROOT/ana_tracer
fi
...
ln -s stdout      stdout.anl.${ADATETIME}
ln -s ana_dynvars dynvanl.${ADATETIME}
ln -s ana_tracer  tracanl.${ADATETIME}
ln -s fort.201    fit_p1.${ADATETIME}
ln -s fort.202    fit_w1.${ADATETIME}
ln -s fort.203    fit_t1.${ADATETIME}
ln -s fort.204    fit_q1.${ADATETIME}
ln -s fort.207    fit_rad1.${ADATETIME}
ln -s fort.209    fit_rw.${ADATETIME}
ln -s fort.238    fit_dbz.${ADATETIME}
...
# listall=`ls pe* | cut -f2 -d"." | awk '{print substr($0, 0, length($0)-3)}' | sort | uniq `
 listall="conv "  
   ...
   for type in $listall; do
      ...
      if [[ $count -gt 0 ]]; then
         cat pe*${type}_${loop}* > diag_${type}_${string}.${ADATETIME}
      fi
   done
done
...
#
#
exit # Code test EXIT, only process ensmean then terminate
#################################################
...
if [ ${if_observer} = Yes ] ; then
  ...
  for type in $listall; do
    count=0
    if [[ -f diag_${type}_${string}.${ADATETIME} ]]; then
       mv diag_${type}_${string}.${ADATETIME} diag_${type}_${string}.ensmean
    fi
  done
  mv ana_dynvars dynvars_ensmean
  mv ana_tracer  tracer_ensmean
...
# get new background for each member
     if [[ -f fv3_dynvars ]]; then
       rm fv3_dynvars
     fi
     if [[ -f fv3_tracer ]]; then
       rm fv3_tracer
     fi

     BK_FILE1=${BK_FILE_mem1}${ensmemid}
     BK_FILE2=${BK_FILE_mem2}${ensmemid} 
     echo $BK_FILE1 $BK_FILE2
     ln -s $BK_FILE1 fv3_dynvars
     ln -s $BK_FILE2 fv3_tracer
