...
$optchoice = $response ;
if ($response <=4 ) { config_omp('  OMP = -mp')} #PGI
if ($response ==5 ) { config_omp('  OMP = -fopenmp')}    #GNU
if ($response >=6 ) { config_omp('  OMP = -openmp -qopenmp')}  #INTEL
...
{ 
   if ( $ifintel ) 
   {
      if ( !$LAPACK_PATH )
      {
        ...
        printf "\$MKLROOT on NOAA and NCAR  supercomputers. \n";
...
sub config_omp {
  my ($text) = @_;
  my $files = './configure.omp';
  if (-f $files) { unlink $files}
  my $OMPFILE;
  open ($OMPFILE, '>>./configure.omp');
  print $OMPFILE $text;
  print $OMPFILE "\n";
  close $OMPFILE;
}
