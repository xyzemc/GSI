...
if [[ "$io_format" = "binary" ]]; then
   ...
   echo "***ERROR*** INVALID io_format = $io_format"
   exit
fi
...
tmpdir=$tmpdir/tmpreg_arw_binary/${exp}
savdir=$savdir/outreg/arw_binary/${exp}
...
if [ $ICO2 -gt 0 ] ; then
	...
	yyyy=$(echo ${CDATE:-$arw_binary_adate}|cut -c1-4)
...
anavinfo=$fixgsi/anavinfo_arw_binary
if [[ "$io_format" = "binary" ]]; then
...
$ncp $arw_binary_obs/${prefixo}.prepbufr.tm06   ./prepbufr
$ncp $arw_binary_obs/${prefixo}.satwnd.$suffix  ./satwndbufr
$ncp $arw_binary_obs/${prefixo}.1bhrs3.$suffix  ./hirs3bufr
$ncp $arw_binary_obs/${prefixo}.1bhrs4.$suffix  ./hirs4bufr
$ncp $arw_binary_obs/${prefixo}.1bamua.$suffix  ./amsuabufr
$ncp $arw_binary_obs/${prefixo}.1bamub.$suffix  ./amsubbufr
$ncp $arw_binary_obs/${prefixo}.1bmhs.$suffix   ./mhsbufr
$ncp $arw_binary_obs/${prefixo}.goesfv.$suffix  ./gsnd1bufr
$ncp $arw_binary_obs/${prefixo}.airsev.$suffix  ./airsbufr
$ncp $arw_binary_obs/${prefixo}.radwnd.$suffix  ./radarbufr
if [[ "$io_format" = "netcdf" ]]; then
   $ncp $arw_binary_obs/${prefixo}.nexrad.$suffix  ./l2rwbufr
fi
...
#
$ncp $arw_binary_obs/${prefixa}.satbias.tm06      ./satbias_in
$ncp $arw_binary_obs/${prefixa}.satang.tm06        ./satbias_angle
if [[ "$io_format" = "binary" ]]; then
   $ncp $arw_binary_ges/wrfinput_d01_2010-07-24_12:00:00       ./wrf_inout
elif [[ "$io_format" = "netcdf" ]]; then
   $ncp $arw_binary_ges/wrfinput_d01_arw_netcdf       ./wrf_inout
fi
...
cat stdout fort.2* > $savdir/stdout.anl.${arw_binary_adate}
$ncp wrf_inout       $savdir/wrfanl.${arw_binary_adate}
$ncp satbias_out     $savdir/biascr.${arw_binary_adate}

# If desired, copy guess file to unique filename in $savdir
$ncp wrf_ges         $savdir/wrfges.${arw_binary_adate}
...
# Collect diagnostic files for obs types (groups) below
   ...
   for type in $listall; do
      ...
      if [[ $count -gt 0 ]]; then
         cat dir.*/${type}_${loop}* > diag_${type}_${string}.${arw_binary_adate}
         compress diag_${type}_${string}.${arw_binary_adate}
         $ncp diag_${type}_${string}.${arw_binary_adate}.Z $savdir/
      fi
   done
done
