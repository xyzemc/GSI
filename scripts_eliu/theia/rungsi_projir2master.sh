...
#BSUB -W 02:00
#BSUB -cwd /gpfs/hps3/emc/da/noscrub/Emily.Liu/GSI_test/ProdGSI-ProjIR/scripts_eliu/logs
...
#PBS -l nodes=120:ppn=6
##PBS -l nodes=80:ppn=6
...
dir_root=/scratch4/NCEPDEV/da/save/Emily.Liu/GSI_test/ProdGSI-ProjIR
...
#   . $MODULESHOME/init/sh
    ...
    SAVE=/scratch4/NCEPDEV/da/save
    NOSCRUB=/scratch4/NCEPDEV/da/noscrub
    GLOBALDUMP=/scratch4/NCEPDEV/global/noscrub/dump
...
if [ $machine = "WCOSS_C" ]; then
   ...
   gsiexec=/gpfs/hps3/emc/da/noscrub/Emily.Liu/GSI_test/ProdGSI-ProjIR/exec/global_gsi.x
   DATA=$PTMP/$LOGNAME/GSI_test/tmp${JCAP}_sigmap/${exp}
   SAVDIR=$PTMP/$LOGNAME/GSI_test/out${JCAP}_sigmap/${exp}
   fixgsi=/gpfs/hps3/emc/da/noscrub/Emily.Liu/GSI_test/ProdGSI-master/fix
  #fixcrtm=$CRTM_FIX   # CRTM_FIX defined by crtm module
   fixcrtm=/gpfs/hps3/emc/da/noscrub/Emily.Liu/RTM/CRTM/REL-2.3.0/fix_crtm   # CRTM_FIX defined by crtm module

   if [[ "$use_emc_para" = "YES" ]]; then
      DMPROOT=`echo $NWROOTp1 | cut -d"/" -f1-3`
      datobs=$DMPROOT/emc/globaldump/$adate/${dumpobs}
      COMROOT=/gpfs/hps3/emc/da/noscrub/Emily.Liu/Data_Input
   ...
   gsiexec=${SAVE}/$LOGNAME/GSI_test/ProdGSI-ProjIR/exec/global_gsi.x
   fixgsi=${SAVE}/$LOGNAME/GSI_test/ProdGSI-ProjIR/fix
  #fixcrtm=$CRTM_FIX   # CRTM_FIX defined by crtm module
   fixcrtm=${SAVE}/$LOGNAME/RTM/CRTM/releases/REL-2.3.1.beta/fix_crtm        # CRTM_FIX defined by crtm module
   fixcrtm=${SAVE}/$LOGNAME/RTM/CRTM/releases/REL-2.3.0/fix_crtm             # CRTM_FIX defined by crtm module

   datobs=${GLOBALDUMP}/$adate/$dumpobs
   COMROOT=${NOSCRUB}/$USER/Data_Input/fv3test
   datges=$COMROOT/$dumpges.$PDYg/$cycg
   datanl=$COMROOT/$dumpanl.$PDYa/$cyca   
   datens=$COMROOT/enkf.gdas.$PDYg/$cycg  
fi
...
cat <<EOF > gsiparm.anl
 &SETUP
   ...
   qoption=2,
   gencode=0,factqmin=5.0,factqmax=0.005,deltim=$DELTIM,
   iguess=-1,
...
#anavinfo=$fixgsi/global_anavinfo_allhydro.l${LEVS}.txt
...
if [[ "$DOHYBVAR" = "YES" ]]; then
  ...
  for fh in $flist; do
    sigens=${prefix_ens}.atmf0${fh}s${enkf_suffix}.nemsio
    imem=1
