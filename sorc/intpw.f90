subroutine intpw(rq,sq,drq,dsq)
...
!    Forward model
     do k=1,nsig
        val=val+(w1* sq(i1(k))+w2* sq(i2(k))            &
               ...
               +(w1*dsq(i1(k))+w2*dsq(i2(k))            &
               + w3*dsq(i3(k))+w4*dsq(i4(k)))*time_pw)* &
                 tpwcon*pwptr%dp(k)
     end do
...
!    Adjoint
     do k=1,nsig
        ...
        drq(i1(k))=drq(i1(k))+w1*pwcon1*time_pw
        drq(i2(k))=drq(i2(k))+w2*pwcon1*time_pw
        drq(i3(k))=drq(i3(k))+w3*pwcon1*time_pw
        drq(i4(k))=drq(i4(k))+w4*pwcon1*time_pw
     end do
