subroutine read_obs(ndata,mype,channelinfo)
...
!   Declare local variables
    ...
    integer(i_kind) nprof_gps1,npem1,krsize
    ...
    real(r_kind) gstime,val_dat,rmesh,twind,rtasks_all,rseed
    ...
    do i=1,ndat
       ...
       if (obstype == 't'  .or. obstype == 'uv' .or. &
           ...
           ditype(i) = 'conv'
            ...
            ditype(i) = 'rad'
       else if (obstype == 'sbuv2' .or. obstype == 'omi' .or. obstype == 'gome') then
            ditype(i) = 'ozone'
...
!      only allow number of reader tasks > 1 for select obstype.
       ...
       if ( ditype(i)=='rad') then
          ...
          if (obstype=='iasi') ntasks_read(i)=ntasks(i)
