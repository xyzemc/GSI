...
MODULE CRTM_Atmosphere_Define
  ...
  END INTERFACE CRTM_Sum_Atmosphere
  ...
  '$Id: CRTM_Atmosphere_Define.f90 1847 2008-02-25 20:49:30Z paul.vandelst@noaa.gov $'
...
!--------------------------------------------------------------------------------
  ...
  FUNCTION Assign_Scalar( Atmosphere_in , &  ! Input
    ...
    ! ----------------------
    IF ( CRTM_Associated_Atmosphere( Atmosphere_Out, Skip_Cloud=SET, Skip_Aerosol=SET ) .AND. &
         Atmosphere_Out%Max_Layers   >= Atmosphere_in%n_Layers+na  .AND. &
         Atmosphere_Out%Max_Clouds   >= Atmosphere_in%n_Clouds     .AND. &
         Atmosphere_Out%Max_Aerosols >= Atmosphere_in%n_Aerosols         ) THEN
         
      ! If structure already allocated to sufficient size,
      ! then zero and set dimensions
      CALL CRTM_Zero_Atmosphere( Atmosphere_Out )
      Atmosphere_Out%n_Layers   = Atmosphere_in%n_Layers+na
    ...
    ELSE
      ...
      Error_Status = CRTM_Allocate_Atmosphere( Atmosphere_in%n_Layers+na, &
                                               Atmosphere_in%n_Absorbers, &
                                               Atmosphere_in%n_Clouds, &
                                               Atmosphere_in%n_Aerosols, &
                                               Atmosphere_out, &
                                               Message_Log=Message_Log )
      IF ( Error_Status /= SUCCESS ) THEN
        CALL Display_Message( ROUTINE_NAME, &
    ...
    ! Assign data
    ! -----------
    Atmosphere_out%n_Added_Layers = Atmosphere_in%n_Added_Layers+na
    ...
    no = Atmosphere_In%n_Layers
    nt = Atmosphere_Out%n_Layers
    Atmosphere_out%Level_Pressure(na:nt) = Atmosphere_in%Level_Pressure(0:no)
    Atmosphere_out%Pressure(na+1:nt)     = Atmosphere_in%Pressure(1:no)
    Atmosphere_out%Temperature(na+1:nt)  = Atmosphere_in%Temperature(1:no)
    Atmosphere_out%Absorber(na+1:nt,:)   = Atmosphere_in%Absorber(1:no,:)
...
!--------------------------------------------------------------------------------
  ...
  FUNCTION Sum_Scalar( A           , &  ! Input/Output
    ...
    INTEGER :: Error_Status
    ! Local parameters
    CHARACTER(*), PARAMETER :: ROUTINE_NAME = 'CRTM_Sum_Atmosphere(Scalar)'
    ! Local variables
    INTEGER :: j, k
    REAL(fp) :: m, c
    ...
    ! Sum the Cloud structure
    ! -----------------------
    IF ( A%n_Clouds > 0 ) THEN
      Error_Status = CRTM_Sum_Cloud( A%Cloud, &
                                     B%Cloud, &
    ...
    IF ( A%n_Aerosols > 0 ) THEN
      Error_Status = CRTM_Sum_Aerosol( A%Aerosol, &
                                       B%Aerosol, &
                                       Scale_Factor=Scale_Factor, &
                                       Offset      =Offset      , &
                                       Message_Log =Message_Log   )
  ...
  FUNCTION Sum_Rank1( A           , &  ! Input/Output
    ...
    INTEGER :: Error_Status
    ! Local parameters
    CHARACTER(*), PARAMETER :: ROUTINE_NAME = 'CRTM_Sum_Atmosphere(Rank-1)'
    ! Local variables
    CHARACTER(ML) :: Message
    INTEGER :: Scalar_Status
    INTEGER :: i, n
  ...
  FUNCTION Sum_Rank2( A           , &  ! Input/Output
    ...
    INTEGER :: Error_Status
    ! Local parameters
    CHARACTER(*), PARAMETER :: ROUTINE_NAME = 'CRTM_Sum_Atmosphere(Rank-2)'
    ! Local variables
    CHARACTER(ML) :: Message
    INTEGER :: Scalar_Status
    INTEGER :: l, nChannels
...
!--------------------------------------------------------------------------------
  ...
  END SUBROUTINE CRTM_RCS_ID_Atmosphere
