...
contains
  ...
  subroutine write_station_lvldata(lunstn,stid,rlat,rlon,rtim,n_chan,nelem_lv,nuchan,data_lv)
    implicit none

    character(8):: stid
    integer:: lunstn,n_chan,nelem_lv
    real(4),dimension(n_chan):: nuchan
    real(4):: rlat,rlon,rtim
    real(4),dimension(nelem_lv,n_chan):: data_lv
    integer:: nflag,i,k
    ...
    nflag=1
    write(lunstn) stid,rlat,rlon,rtim,n_chan+1,nflag
    write(lunstn) (nuchan(k),(data_lv(i,k),i=1,nelem_lv),k=1,n_chan)

    return
  end subroutine write_station_lvldata
  ...
  subroutine write_station_lvlctl(lunctl,comment,filename,datetime,tint,nt,n_chan,nelem_lvl,data_name)
    use grads_hdr
    use read_diag
    implicit none
    ...
    integer :: datetime(5)
    integer :: tint, nt,i
    integer :: n_chan,nelem_lvl
    character(len=10):: data_name(nelem_lvl)
    character(len=GRADS_MAXLEN_FILENAME+4):: dsetname
    character(len=3),dimension(12):: cmonth
    character(len=15):: string
    integer :: imissing
    ...
    write(string,'(i2.2,":",i2.2,"Z",i2.2,a3,i4.4)') datetime(4),datetime(5),datetime(3),cmonth(datetime(2)),datetime(1)
    write(lunctl,'("tdef ",i3," linear ",a,1x,i2,"hr")') nt,string,tint
    write(lunctl,'("vars ",i3)') nelem_lvl
    do i=1,nelem_lvl
       write(lunctl,'(a,1x,i6,(" 99 "),a)') trim(data_name(i)),n_chan,trim(data_name(i))
    end do

    write(lunctl,'("endvars")')

  end subroutine write_station_lvlctl
