...
export LOGDIR=/ptmpp1/Xiujuan.Su/logs/plot_gsiqc3
export group=dev

pdate=${sdate}-${edate}

mkdir -p /stmpp1/Xiujuan.Su/plotjobs_gsiqc
mkdir -p $LOGDIR
...
for stype in $dstype; do
cmdfile=/stmpp1/Xiujuan.Su/plotjobs_gsiqc/cmdfile_phorz_$stype
rm -f $cmdfile
>$cmdfile
...
cmdfile=/stmpp1/Xiujuan.Su/plotjobs_gsiqc/cmdfile_phorzwind_$stype
rm -f $cmdfile
>$cmdfile
echo "/bin/sh $scripts/plot_horz.sh $stype $itype $tmpdir $sdate $edate $WSHOME $WSUSER $WS $scripts $gscripts $dtime" >> $cmdfile
chmod 755 $cmdfile
...
for stype in $dstype; do
cmdfile=/stmpp1/Xiujuan.Su/plotjobs_gsiqc/cmdfile_ptimesfc_$stype
rm -f $cmdfile
>$cmdfile
...
for stype in $dstype; do
cmdfile=/stmpp1/Xiujuan.Su/plotjobs_gsiqc/cmdfile_ptime_$stype
rm -f $cmdfile
>$cmdfile
echo "/bin/sh $scripts/plot_time.sh $stype $itype $tmpdir $sdate $edate $WSHOME $WSUSER $WS $scripts $gscripts " >> $cmdfile
chmod 755 $cmdfile
...
cmdfile=/stmpp1/Xiujuan.Su/plotjobs_gsiqc/cmdfile_ptimewind_$stype
rm -f $cmdfile
>$cmdfile
...
itype=1
for stype in $dstype; do
cmdfile=/stmpp1/Xiujuan.Su/plotjobs_gsiqc/cmdfile_ptime_obs_$stype
rm -f $cmdfile
>$cmdfile
...
itype=1
for stype in $uvsondtype; do
cmdfile=/stmpp1/Xiujuan.Su/plotjobs_gsiqc/cmdfile_ptime_obs_dir_$stype
rm -f $cmdfile
>$cmdfile
