!  the program to read prepbufr rawinsonde data
    ...
    subroutine read_prepbufr_sfc_ps(sub,dtype,rtype,gross)
...
!  data vqcstr / 'PVWTG QVWTG TVWTG WVWTG PVWTA QVWTA TVWTA WVWTA '/
   data allstr /'PQM PAN PFC POE PVWTG PVWTA CAT'/
   data obstr /'POB PPC PRC'/
...
!  call convinfo_read(dtype,gross,iuse)
   ...
   print *,'read_prepbufr_sfc_ps: dtype=',dtype
  nread=0
  ndata=0
  ps180_no=0.0
  ps180=0.0
  ...
  do while(ireadmg(lunin,subset,idate).eq.0)
    ...
    do while(ireadsb(lunin).eq.0)
       ...
       call ufbevn(lunin,obs,3,1,15,iret,obstr)  !  read different observation  
       icat=int(all(7))
       if(hdr(2) <0.0) hdr(2)=360.00+hdr(2)
       if(hdr(2) >360.00) then
         print *,'read_prepbufr_sfc_ps:problem with longitudedtype=',dtype,hdr(1),hdr(2),hdr(3)
         cycle
        endif
       if(abs(hdr(3)) >90.) then
         print *,'read_prepbufr_sfc_ps:problem with latitude, dtype=',dtype,hdr(1),hdr(2),hdr(3)
         cycle
        endif
       if(hdr(7) /= rtype .or. icat /=0 .or. all(3) >20000.0 ) cycle
       if(hdr(7) == rtype .and. icat ==0 .and. all(3) <20000.0) then
          nread=nread+1
          if(nread==1) then
             ndata=ndata+1
             rstationid=hdr(1)
          ...
          else if(obs(1,1) <2000.0 .and. all(3) <2000.0 .and. all(4) <1000.0) then
                j=0
                do i=1,4
                   if(obs(1,i) <20000.00 .and. obs(2,i) /=17.0) then
                      j=i
                      exit
                   endif
                enddo
