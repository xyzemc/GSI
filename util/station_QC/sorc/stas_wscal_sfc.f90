!   to calculate statistics for each wind scale
    ...
    subroutine stas_wscal_sfc(dtype,stdid,cid,nob,nsta,nt,tdata)
    ...
    character(len=8),dimension(nob) ::  stdid
     ...
     real(4),dimension(nob,14,nt) :: tdata 
     real(8),dimension(12) :: wscale
   ...
   print *,'tas_wscal_sfc:',nob,nsta,nt
    ...
    print *,'begin tas_spdir_wscal '
     ...
     do it=1,nt
       jj=0
       do j=1,12
         if(tdata(i,14,it) >-300.0 .and. tdata(i,14,it) <= wscale(j)) then
            jj=j
            exit
         endif
       enddo
        if( jj >0) then
           rno(i,j)=rno(i,j)+1.0
           rmean(i,j,1)=rmean(i,j,1)+tdata(i,9,it)
           rmean(i,j,2)=rmean(i,j,2)+tdata(i,11,it)
           std(i,j,1)=std(i,j,1)+tdata(i,9,it)*tdata(i,9,it)
           std(i,j,2)=std(i,j,2)+tdata(i,11,it)*tdata(i,11,it)
        endif
     enddo
     enddo
        ...
        fileo1=trim(dtype)//'_spscal_hist_grads'
         ...
         open (30,file=fileo1,form='unformatted')
          ...
          open(40,file=fileo1,form='unformatted')
            ...
            do i=1,12
              tmp2(1:nsta)=rmean(1:nsta,i,1)
               write(40) tmp2
            enddo
            do i=1,12
              tmp2(1:nsta)=rmean(1:nsta,i,2)
               write(40) tmp2
            enddo
            do i=1,12
              tmp2(1:nsta)=std(1:nsta,i,1)
               write(40) tmp2
            enddo
            do i=1,12
              tmp2(1:nsta)=std(1:nsta,i,2)
               write(40) tmp2
            enddo
           do i=1,12
