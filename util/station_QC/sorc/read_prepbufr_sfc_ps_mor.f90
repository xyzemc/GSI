!  the program to read prepbufr rawinsonde data
    ...
    subroutine read_prepbufr_sfc_ps_mor(sub,dtype,rtype,gross)
   ...
   do while(ireadmg(lunin,subset,idate).eq.0)
      ...
      do while(ireadsb(lunin).eq.0)
       ...
       if(hdr(2) >360.00) then
         print *,'read_prepbufr_sfc_ps_mor:problem with longitudedtype=',dtype,hdr(1),hdr(2),hdr(3)
         cycle
        endif
       if(abs(hdr(3)) >90.0) then
         print *,'read_prepbufr_sfc_ps_mor:problem with latitude, dtype=',dtype,hdr(1),hdr(2),hdr(3)
         cycle
        endif
         ...
         if(hdr(7) == rtype .and. icat ==0 .and. all(3) <20000.0) then
            ...
            if( all(4) <1000.0) then
               ...
               endif             ! end of quality mark check
