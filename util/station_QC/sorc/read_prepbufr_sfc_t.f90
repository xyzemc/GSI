!  the program to read prepbufr rawinsonde data
    ...
    subroutine read_prepbufr_sfc_t(sub,dtype,rtype,gross)
...
!      nread=nread+1
     ...
     call ufbevn(lunin,obs,3,1,15,iret,obstr)  !  read different deck observation

     if(hdr(2) <0.0) hdr(2)=360.00+hdr(2)
       if(hdr(2) >360.00) then
         print *,'read_prepbufr_sfc_t:problem with longitudedtype=',dtype,hdr(1),hdr(2),hdr(3)
         cycle
        endif
       if(abs(hdr(3)) >90.0) then
         print *,'read_prepbufr_sfc_t:problem with latitude, dtype=',dtype,hdr(1),hdr(2),hdr(3)
         cycle
        endif
         ...
         if(nread==1) then
          ...
          if (nread <10) then
              ...
              print *,obs(2,1),obs(2,2),obs(2,3),obs(3,1),obs(3,2),obs(3,3),all(4),all(5),all(6)
               ...
               nn=n_ps180(ndata)
                if( all(1) <0.0  .or. all(4) >1000.0 .or. all(1) ==10.0) then  ! gm=10 rejected by GSI gross check
