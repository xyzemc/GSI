...
! same stations from both different observation types
  ...
  subroutine compare_four_sfc(type,fileout,stdidall,ncount_all,fileexist,itype,ndata)


   character(5) atype,atype1,atype2,atype3,atype4
   ...
   write(21,100)
          ...
          enddo 
          nnobs=nobs
          do i=1,ncount_all(ll)
          do j=1,nobs
             if(trim(stdidall(ll,i)) == trim(stdid(j))) then
                atype=atype_fnl(j)
                atype(ll+1:ll+1)=atype4(ll+1:ll+1)
                atype_fnl(j)=atype
                exit
             else if( j ==nobs) then
                    nnobs=nnobs+1
                    stdid(nnobs)=stdidall(ll,i)
                    atype_fnl(nnobs)=atype4
             endif
          enddo 
          enddo 
