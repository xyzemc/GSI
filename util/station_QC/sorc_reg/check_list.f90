...
!   compare their values  
      ...
      subroutine check_list(dtype,filetype,n,nd,nd2,nk,ncount1,ncount2,index1,index2,&
       ...
       do k=1,nk
          if(rmean1(index2,i,k,1) >-900.0) then
            write(6,110) k, rmean1(index2,i,k,1),rmean1(index2,i,k,2),&
                        std1(index2,i,k,1),std1(index2,i,k,2),iqcflg1(index2,i,k)
    ...
    do j=1,ncount2
       if( trim(stdid1(index1,i)) == trim(stdid1(index2,j))) then
          if(trim(filetype) == 'bias') then
             do k=1,nk
                if(abs(rmean1(index1,i,k,1)) <900.0  .and. &
                   abs(rmean1(index2,j,k,1)) < 900.0 ) then
                   if(abs(rmean1(index1,i,k,1)) <abs(rmean1(index2,j,k,1))) then
...
!                else if(abs(rmean1(index1,i,k,1)) >900.0 ) then
!                   rmean1(index2,j,k,1:nd) = -999.0 
!                   std1(index2,j,k,1:nd) = -999.0 
!                   xmed1(index2,j,k,1:nd2) = -999.0 
!                   break1(index2,j,k,1:nd2) = -999.0 
!                   std_no1(index2,j,k) = 0.0 
!                   tdata_no1(index2,j,k,1:2)=0.0
                endif
             enddo
             exit
             ...
             do k=1,nk
                if(abs(rmean1(index1,i,k,1)) <900.0  .and. &
                   abs(rmean1(index2,j,k,1)) < 900.0 ) then
                   if(std1(index1,i,k,1) <std1(index2,j,k,1)) then
                      rmean1(index2,j,k,1:nd) = rmean1(index1,i,k,1:nd)
                      std1(index2,j,k,1:nd) = std1(index1,i,k,1:nd)
                      xmed1(index2,j,k,1:nd2) = xmed1(index1,i,k,1:nd2)
                      break1(index2,j,k,1:nd2) = break1(index1,i,k,1:nd2)
...
!                   tdata_no1(index2,j,k,1:2)=0.0
    ...
    enddo
      ...
      do i=1, nrecord
       ...
       do k=1,nk
          if(rmean1(index2,i,k,1) >-900.0) then
            write(6,110) k, rmean1(index2,i,k,1),rmean1(index2,i,k,2),std1(index2,i,k,1),std1(index2,i,k,2),iqcflg1(index2,i,k)
