! the program is sorting out list for different time delay analysis
 ...
 if(mfile >0) then
   if(itype == 0 .and. ddtype == 'u') then
    call compare_list_sfc_sp(ntm,dtype,tm,fileexist,filetype,4,4)
  else if(itype == 1 .and. ddtype == 'u') then
    nk=43
    call compare_list_sp(ntm,dtype,tm,fileexist,filetype,4,4,nk)
  endif
else
    print *,'main_compare_list_sp: there is no ',filetype,' file in all analysis time'
  endif
