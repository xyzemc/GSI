...
!   compare their values  
      ...
      subroutine check_std_sfc_sp(dtype,tm,filetype,nd,nd2,stdid_input,rmean_input,std_input,&
  ...
  if(iqcflg ==2) then
     ...
     call get_crit_sp_sfc(dtype,filetype,ct)
  endif
!     print *,'check_std_sfc_sp:tm,dtype,filetype=', tm,dtype,filetype
     ...
     if(.not. lexist) then
       print *,'check_std_sfc_sp:the file doesnot exist ',file1
       return
     endif
...
!     print *,'check_std_sfc: ndata,nread,n_miss_crit',ndata,nread,n_miss_crit
    ...
    read(21) std_no_s(1:ndata,:)
       ...
       close(21)
        ...
        ct=ct-0.2
!        print *,'check_std_sfc_sp:ct=',ct
      
      do i=1,ndata
         if(trim(stdid_s(i)) == trim(stdid_input) .and. std_no_s (i,1) >5.0) then
            std_exist=.true.
            print *, '1,check_std_sfc_sp:,',rmean_input(3),rmean_s(i,3),std_input(3),std_s(i,3)     
            if(trim(filetype) == 'bias') then
               if(abs(rmean_input(3)) > abs(rmean_s(i,3)) .and. &
                   abs(rmean_s(i,3)) >= ct) then 
                  print *,'2,check_std_sfc_sp:',(break_s(i,j),j=1,4)
                  list_true=.true. 
                  ...
                  iqcout=iqcinput
                  print *,'2,check_std_sfc_sp:',(break_out(j),j=1,4)
               else if( abs(rmean_input(3)) <abs(rmean_s(i,3))) then
                  list_true=.true. 
!                  print *,'3,check_std_sfc_sp:',(break_s(i,j),j=1,4)
                  ...
                  iqcout=iqcinput
                  print *,'3,check_std_sfc_sp:',(break_out(j),j=1,4),tno_out(1),tno_out(2)
               endif
            else if(trim(filetype) == 'rej') then
               if(std_input(3) > std_s(i,3) .and. std_s(i,3) >= ct ) then 
                  list_true=.true. 
                  rmean_out(1:nd) = rmean_s(i,1:nd)
                  std_out(1:nd) = std_s(i,1:nd)
                  xmean_out(1:nd2) = xstas_s(i,1:nd2)
                  ...
                  tno_out(1:2)=tno2_s(i,1:2)
                  iqcout=iqcinput
                  print *,'4,check_std_sfc_sp:',(break_out(j),j=1,4),std_input(3),std_s(i,3)
               else if (std_input(3)  < std_s(i,3) ) then
                  list_true=.true.
                  rmean_out(1:nd) = rmean_input(1:nd)
                  std_out(1:nd) = std_input(1:nd)
                  xmean_out(1:nd2) = xmed_input(1:nd2)
                  ...
                  tno_out(1:2)=tno_input(1:2)
                  iqcout=iqcinput
                  print *,'5,check_std_sfc_sp:',(break_out(j),j=1,4),std_input(3),std_s(i,3)
               endif
            endif   
            exit
         endif
...
!      print *, 'check_std_sfc_sp:',stdid_input,std_exist,list_true,rmean_out(1),std_out(1)
