...
!   compare their values  
      ...
      subroutine check_list_sp(dtype,filetype,n,nd,nd2,nk,ncount1,ncount2,index1,index2,&
       ...
       print *,'check_list_sp:,ncount1,ncount2=',ncount1,ncount2

   do i=1,ncount2 
       write(6,100) stdid1(index2,i),cid1(index2,i,1),cid1(index2,i,2)
       do k=1,nk
          if(rmean1(index2,i,k,3) >-900.0) then
    ...
    do j=1,ncount2
       if( trim(stdid1(index1,i)) == trim(stdid1(index2,j))) then
          if(trim(filetype) == 'bias') then
             do k=1,nk
                if(abs(rmean1(index1,i,k,3)) <900.0  .and. &
                   abs(rmean1(index2,j,k,3)) < 900.0 ) then
                   if(abs(rmean1(index1,i,k,3)) <abs(rmean1(index2,j,k,3))) then
...
!                else if(abs(rmean1(index1,i,k,3)) >900.0 ) then
...
!                   tdata_no1(index2,j,k,1:2)=0.0
             ...
             do k=1,nk
                if(abs(rmean1(index1,i,k,3)) <900.0  .and. &
                   abs(rmean1(index2,j,k,3)) < 900.0 ) then
                   if(std1(index1,i,k,3) <std1(index2,j,k,3)) then
                      rmean1(index2,j,k,1:nd) = rmean1(index1,i,k,1:nd)
                      std1(index2,j,k,1:nd) = std1(index1,i,k,1:nd)
                      xmed1(index2,j,k,1:nd2) = xmed1(index1,i,k,1:nd2)
                      break1(index2,j,k,1:nd2) = break1(index1,i,k,1:nd2)
