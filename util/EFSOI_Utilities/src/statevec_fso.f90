...
use gridio, only: readgriddata, writegriddata, get_weight, destroy_weight, &
                  divide_weight
use mpisetup
...
use params, only: nlevs,nvars,ndim,nbackgrounds,&
                  ...
                  nhr_anal,fso_flag,evalft,andataname
...
if(nproc == 0) then 
   ...
   call readgriddata(0,evalft+nhr_anal,0,grdin) 
   ...
   grdin2(:,ndim) = grdin2(:,ndim) / grdin(:,ndim) 
   ...
   do np=1,numproc 
      do nn=1,ndim 
         do i=1,numptsperproc(np) 
            ...
            sendbuf2(n) = grdin2(indxproc(np,i),nn) 
         end do 
      end do 
   end do 
