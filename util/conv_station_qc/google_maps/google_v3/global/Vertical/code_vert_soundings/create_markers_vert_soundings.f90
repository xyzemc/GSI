...
program create_markers_vert_soundings
 use strings
 implicit none
 ...
 character(len=40) :: png_obs, png_omg
 ...
 if(argc<1) then
   print *, "Usage: create_markers_vert_soundings <input_ctl_file>"
   call exit(1)
 end if
...
! 97   Format('  +', "'", '<a href="../pngs/', a25, '" target="_blank" >', "OMG</a> <br>'")
 ...
 do
   ...
   if(num_tokens .gt. 2) then
      ...
      png_obs = trim(type)//'/'//trim(type)//'_obs_'//trim(station_id)//'.png'
      png_omg = trim(type)//'/'//trim(type)//'_'//trim(station_id)//'.png'

      write(51, fmt=95) lat, lon, qflag, station_id
      write(51, *) '  +',"'", '<a href="../', trim(png_obs),'" target="_blank" >', "OBS</a> <br>'"
      write(51, *) '  +',"'", '<a href="../', trim(png_omg),'" target="_blank" >', "OMG</a> <br>'"
      write(51, *) "],"
      write(51, *)
...
end program create_markers_vert_soundings
