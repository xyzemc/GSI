...
!   compare their values  
      ...
      subroutine check_data_sfc_sp(nfile,dtype,filetype,tm,filetm,n,nd,nd2,stdid,&
      ...
      real(4),dimension(nd) :: std_out3,rmean_out3,std_out4,rmean_out4
      real(4),dimension(2) :: tno_out,tno_out2,tno_out3
      real(4),dimension(2) :: tno_out4,tno_input
      real(4),dimension(nd) :: std_input,rmean_input
      real(4),dimension(nd2) :: xmed_out,xmed_out2,xmed_out3,xmed_input 
      real(4),dimension(nd2) :: xmed_out4,break_out,break_input 
      real(4),dimension(nd2) :: break_out2,break_out3,break_out4 
...
!  compare all tm list, get smallest rmean and std values for all tm
  ...
  do i=1,ndata                          !  tm 00
     if(iqcflg00(i) == 2) then
        ...
        iqcinput=iqcflg00(i)
        call check_std_sfc_sp(dtype,tm1,filetype,nd,nd2,stdid_input,rmean_input,std_input, &  
...
!           print *,'1,check_data_sfc:',tm1,rmean_input(1),std_input(1),std_no_input
           call check_std_sfc_sp(dtype,tm2,filetype,nd,nd2,stdid_input,rmean_input,std_input, &  
                    std_no_input,xmed_input,break_input,tno_input,iqcinput,& 
           ...
           if(std2_exist .and. list2_true ) then
              rmean_input(1:nd)=rmean_out2(1:nd)
              std_input(1:nd)=std_out2(1:nd)
              std_no_input=std_no_out2
              xmed_input(1:nd2)=xmed_out2(1:nd2)
              break_input(1:nd2)=break_out2(1:nd2)
              tno_input(1:2)=tno_out2(1:2)
              iqcinput=iqcout
!              print *,'2,check_data_sfc:',tm2,rmean_input(1),std_input(1),std_no_input
              call check_std_sfc_sp(dtype,tm3,filetype,nd,nd2,stdid_input,rmean_input,std_input,&   
                        std_no_input,xmed_input,break_input,tno_input,iqcinput,& 
              ...
              if(std3_exist .and. list3_true ) then
                 ...
                 stdid(ncount) = stdid_00(i)
                 cid(ncount,1:2) = cid_00(i,1:2)
                 rmean(ncount,1:nd) = rmean_out3(1:nd)
                 std(ncount,1:nd) = std_out3(1:nd)
                 xmed(ncount,1:nd2) = xmed_out3(1:nd2)
                 break(ncount,1:nd2) = break_out3(1:nd2)
                 std_no(ncount) = std_no_out3
                 tno(ncount,1:2)=tno_out4(1:2)
...
!!                   print *,'4,check_data_sfc:',tm3,ncount,break(ncount,1),break(ncount,2),&
!                              break(ncount,3),break(ncount,4)
!                   print *,'4,check_data_sfc:',xmed(ncount,1),xmed(ncount,2), &
...
!                              xmed(ncount,3),xmed(ncount,4)
                 ...
                 iqcflg(ncount)=iqcout
!                   print *,'25,check_data_sfc:',tm3,ncount,break(ncount,1),break(ncount,2),&
!                              break(ncount,3),break(ncount,4)
!                   print *,'25,check_data_sfc:',xmed(ncount,1),xmed(ncount,2), &
!                              xmed(ncount,3),xmed(ncount,4)
              endif
           endif
...
!                              xmed(ncount,3),xmed(ncount,4)
              ...
              if(std3_exist .and. list3_true ) then
                 ...
                 tno(ncount,1:2)=tno_out3(1:2)
                 iqcflg(ncount)=iqcout
!                  print *,'49,check_data_sfc:',tm3,ncount,break(ncount,1),break(ncount,2),&
!                              break(ncount,3),break(ncount,4)
!                   print *,'49,check_data_sfc:',xmed(ncount,1),xmed(ncount,2), &
!                              xmed(ncount,3),xmed(ncount,4)
!              print *,'49,check_data_sfc:',tm3,rmean(ncount,1),std(ncount,1),std_no(ncount)
              else if(.not. std3_exist) then
                 ncount=ncount+1
                 stdid(ncount) = stdid_00(i)
                 cid(ncount,1:2) = cid_00(i,1:2)
...
!                std(i,1),std(i,2),std_no(i),tno(i,1),tno(i,2)
