...
!  of the surface pressure stations
        ...
        subroutine q_selet_sfc_reg(dtype,iqcflg)

!!!    get criteria from crite_constnt module
  
     use crite_constnt_reg,only:q_c_rm,q_c_std,q_c_smad
...
600 format(10f8.2)
  ...
  q_c_std2=0.8*q_c_std                    ! For the bias standard deviation criteria  
!  if(q_c_std2 >1.0) q_c_std2=1.0
  rm_c=0.8*q_c_rm           !! the standard to reject the station which has data 70%
  std_c=0.8*q_c_std        !!! rejected by quality control 

  print *, 't_selet_sfc:q_c_std2,rm_c,std_c=',q_c_std2,rm_c,std_c
  allocate(tmp2(12))
    do i=1,ndata
       ag=abs(rmean(i,1)) 
       if(cid(i,6) <-50.0 ) then
          ...
          if(abs(cid(i,2)) <65.0 ) then            ! don't reject the station with latitude greater than 65
             if(tdata_no2(i,3) >=0.80 .and. (abs(rmean(i,1)) >= rm_c .or. &
                ...
                if(ag >=q_c_rm .or.  std(i,1) >=q_c_std ) then
                   stid=stdid(i)
                   rlat=cid(i,2)
                   rlon=cid(i,1)
                   ...
                   if(std(i,1) >=q_c_std .and. smad(i,1) >= q_c_mad .and. break(i,4) >=q_c_mad ) then 
                      n_rej=n_rej+1
                      write(73,100) stdid(i),i,cid(i,1),cid(i,2),std_no(i,1),rmean(i,1),&
                           rmean(i,2),std(i,1),std(i,2),tdata_no2(i,1),tdata_no2(i,2),iqcflg(3)
                      write(73,110) xmed(i,1),xmed(i,2),smad(i,1),smad(i,2)
                      write(73,120) n_tdata(i),break(i,1), break(i,2),break(i,3),break(i,4)
                   else if(ag >=q_c_rm .and. std(i,1) <=q_c_std2 .and. break(i,4) >0) then
                      n_bias=n_bias+1
                   ...
                   else if(ag >=q_c_rm .and. ag/std(i,1) >=3.0 .and. &
                      ...
                      write(72,110) xmed(i,1),xmed(i,2),smad(i,1),smad(i,2)
                      write(72,120) n_tdata(i),break(i,1), break(i,2),break(i,3), break(i,4) 
                   else if(ag >=5.0*q_c_std .and. break(i,4) >0.0 ) then
                      n_rej=n_rej+1
                      write(73,100) stdid(i),i,cid(i,1),cid(i,2),std_no(i,1),rmean(i,1),&
                          rmean(i,2),std(i,1),std(i,2),tdata_no2(i,1),tdata_no2(i,2),iqcflg(3)
                      write(73,110) xmed(i,1),xmed(i,2),smad(i,1),smad(i,2)
                      write(73,120) n_tdata(i),break(i,1), break(i,2),break(i,3), break(i,4) 
                   else if (ag >q_c_rm .and. std(i,1) >=q_c_std2 ) then 
