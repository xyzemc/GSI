...
!  of the surface pressure stations
        ...
        subroutine t_selet_reg(dtype,nk,iqcflg)

    use crite_constnt_reg,only: t2_c_rm,t2_c_std1
...
!     real(4),dimension(2000) :: index2_rej,index2_bias,index2_other,index2_qly,index2,index2_norm
     ...
     real(4),dimension(nk) :: rm_c,std_c,t2_c_std2 
...
!! creat normal station frequecy criteria
  ...
  std_c=0.8*t2_c_std1 
   ...
   print *, 't_select',rm_c(1),rm_c(2) 

   do k=1,nk
     t2_c_std2(k)=0.8*t2_c_std1(k)
!     if(t2_c_std2(k) >1.0) t2_c_std2(k)=1.0
   enddo
   print *, 't_select',t2_c_std2(1),t2_c_std2(2) 
    ...
    do k=1,nk
      ...
      if(rmean(i,k,1) >-900.0 .and. std(i,k,1) >-900.0) then       
        ...
        print *,'k,plevel(k),kkk=',k,plevel(k),kkk
         if( kkk /=0) then
            if(ag <t2_c_rm(k) .and. std(i,k,1) <t2_c_std1(k) .and. &
      ...
      if(tdata_no2(i,k,3) >=0.80 .and. (ag >= rm_c(k) .or. std(i,k,1) >= std_c(k))) then
         ...
         if(ag >t2_c_rm(k) .or.  std(i,k,1) >t2_c_std1(k) ) then
            n_sus(k)=n_sus(k)+1
            index(i,k)=1
            ...
            if(std(i,k,1) >t2_c_std1(k) .and. break(i,k,4) >0.0 ) then
            ...
            else if(ag >3.0*t2_c_std1(k) .and. break(i,k,4) >0.0) then
               if(abs(rlat) >11.0 ) then
                  n_rej(k)=n_rej(k)+1
                  index_rej(i,k)=1
                  index2_rej(i)= index2_rej(i)+1.0
            ...
            else if(ag >t2_c_rm(k) .and. std(i,k,1) <t2_c_std2(k) .and.  break(i,k,4) >0.0) then
               if(abs(rlat) >11.0 ) then  
                  n_bias(k)=n_bias(k)+1
                  index_bias(i,k)=1
            ...
            else if (ag >t2_c_rm(k) .and. std(i,k,1) >t2_c_std2(k)) then 
!                  if(abs(rlat) >11.0 ) then
...
120 format(8x,4f8.2)
       ...
       Print *,'t_selet:',dtype
       print *,t2_c_std2(1),t2_c_std2(2),t2_c_std2(3),t2_c_std2(4)
...
200 format(2x,a8,i8,3f8.2)
   ...
   Print *,'t_selet_reg:,dtype,ndata,n_qly,n_sup,n_bias,n_rej,n_other'
       Print *,'t_selet_reg:',dtype,ndata,n_qly,n_sup,n_bias,n_rej,n_other
