...
!  of the surface pressure stations
        ...
        subroutine t_selet_sfc_reg(dtype,iqcflg)

!!!    get criteria from crite_constnt module
  
       use crite_constnt_reg,only: t4_c_rm,t4_c_std,t4_c_smad,t3_c_rm,t3_c_std,t3_c_smad
...
600 format(10f8.2)
  ...
  t_c_std2=0.8*t_c_std                    ! For the bias standard deviation criteria  
!  if(t_c_std2 >1.0) t_c_std2=1.0
  rm_c=0.8*t_c_rm           !! the standard to reject the station which has data 70%
  std_c=0.8*t_c_std        !!! rejected by quality control 

  print *, 't_selet_sfc:t_c_std2,rm_c,std_c=',t_c_std2,rm_c,std_c
  allocate(tmp2(12))
    do i=1,ndata
       ag=abs(rmean(i,1)) 
       if(cid(i,6) <-50.0 ) then
          ...
          if(abs(cid(i,2)) <65.0 ) then            ! don't reject the station with latitude greater than 65
             if(tdata_no2(i,3) >=0.80 .and. (abs(rmean(i,1)) >= rm_c .or. &
                std(i,1) >= std_c))then 
                n_qly=n_qly+1
                ...
                if(ag >=t_c_rm .or.  std(i,1) >=t_c_std ) then
                   stid=stdid(i)
                   rlat=cid(i,2)
                   rlon=cid(i,1)
                   ...
                   if(std(i,1) >=t_c_std .and. smad(i,1) >= t_c_smad .and. break(i,4) >=t_c_smad ) then 
                      n_rej=n_rej+1
                      write(73,100) stdid(i),i,cid(i,1),cid(i,2),std_no(i,1),rmean(i,1),&
                           rmean(i,2),std(i,1),std(i,2),tdata_no2(i,1),tdata_no2(i,2),iqcflg(3)
                      write(73,110) xmed(i,1),xmed(i,2),smad(i,1),smad(i,2)
                      write(73,120) n_tdata(i),break(i,1), break(i,2),break(i,3),break(i,4)
                   else if(ag >=t_c_rm .and. std(i,1) <=t_c_std2 .and. break(i,4) >0) then
                      n_bias=n_bias+1
                   ...
                   else if(ag >=t_c_rm .and. ag/std(i,1) >=3.0 .and. &
                      ...
                      write(72,110) xmed(i,1),xmed(i,2),smad(i,1),smad(i,2)
                      write(72,120) n_tdata(i),break(i,1), break(i,2),break(i,3), break(i,4) 
                   else if(ag >=4.0*t_c_std .and. break(i,4) >0.0 ) then
                      n_rej=n_rej+1
                      write(73,100) stdid(i),i,cid(i,1),cid(i,2),std_no(i,1),rmean(i,1),&
                          rmean(i,2),std(i,1),std(i,2),tdata_no2(i,1),tdata_no2(i,2),iqcflg(3)
                      write(73,110) xmed(i,1),xmed(i,2),smad(i,1),smad(i,2)
                      write(73,120) n_tdata(i),break(i,1), break(i,2),break(i,3), break(i,4) 
                   else if (ag >t_c_rm .and. std(i,1) >=t_c_std2 ) then 
...
120 format(16x,i8,4f8.2)
       ...
       Print *,'t_selet_sfc:,dtype,ndata,n_qly,n_sup,n_bias,n_rej,n_other,n_norm'
       Print *,'t_selet_sfc:',dtype,ndata,n_qly,n_sup,n_bias,n_rej,n_other,n_norm
       print *,t_c_rm,t_c_std,t_c_std2
