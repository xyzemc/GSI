!  the program to read prepbufr rawinsonde data
   ...
   subroutine read_prepbufr_sfc_w(sub,dtype,rtype,gross)
   ...
   do while(ireadsb(lunin).eq.0)
     ...
     if(hdr(7) /= rtype ) cycle
       ...
       if(hdr(2) >360.00) then
         print *,'read_prepbufr_sfc_w:problem with longitudedtype=',dtype,hdr(1),hdr(2),hdr(3)
         cycle
        endif
       if(abs(hdr(3)) >90.0) then
         print *,'read_prepbufr_sfc_w:problem with latitude, dtype=',dtype,hdr(1),hdr(2),hdr(3)
         cycle
        endif
...
250 format(a8)
         if( all(1) <0.0 .or. all(1) ==10.0) then
            ...
            if(all(1) >7.0 .and. all(1) /=10.0) then
