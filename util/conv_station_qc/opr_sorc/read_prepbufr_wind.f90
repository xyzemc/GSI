!  the program to read prepbufr rawinsonde data
   ...
   subroutine read_prepbufr_wind(sub,dtype,rtype,gross)
...
!   data obstr /'POB UOB VOB WPC WRC '/
  ...
  equivalence(rstationid,stationid)
     ...
     print *,'sub,dtype, gross,rtype', sub,dtype, gross,rtype
     ...
     if( hdr(7) == rtype) then 
      ...
      if(hdr(2) <0.0) hdr(2)=360.00+hdr(2)
       if(hdr(2) >360.00) then
         print *,'read_prepbufr_wind:problem with longitudedtype=',dtype,hdr(1),hdr(2),hdr(3)
         cycle
        endif
       if(abs(hdr(3)) >90.0) then
         print *,'read_prepbufr_wind:problem with latitude, dtype=',dtype,hdr(1),hdr(2),hdr(3)
         cycle
        endif
...
!              print *, nkk
              ...
              print *,'wind ',ndata,hdr(7),nkk,obs(1,kk),obs(2,kk),ans(1,kk),bac(1,kk)
               if(qms(kk)  <0.0 .or. qms(kk) ==10.0) then 
                  ...
                  if(qms(kk) >7.0 .and. qms(kk) /=10.0 ) then
                     diff=(obs(2,kk)-bac(1,kk))**2+(obs(3,kk)-bac(2,kk))**2
                     diff=sqrt(diff)
                     ratio=ratio/obserr(kk)
                     if(ratio >gross) then
                        w220_no(ndata,nkk,2)=w220_no(ndata,nkk,2)+1.0
                     else
...
!  for wind speed and direction
                     ...
                     w220(ndata,nkk,13)=w220(ndata,nkk,13)+obu      ! obs direction
                     w220(ndata,nkk,14)=w220(ndata,nkk,14)+obv      ! obs speed
                     print *,ndata,nkk,w220_no(ndata,nkk,1),w220(ndata,nkk,1),w220(ndata,nkk,2),w220(ndata,nkk,5),w220(ndata,nkk,9),w220(ndata,nkk,10)
                  endif
               endif
         enddo       !! end do nlev
