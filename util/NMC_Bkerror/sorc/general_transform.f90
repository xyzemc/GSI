...
subroutine general_s2g0(sp,spectral_in,grid_out)
...
!   2010-02-18  parrish, copy s2g0 to general_s2g0--replace 'use specmod' with
!   input structure var sp
!                           also replace 'use gridmod' with input structure
!                           variable grd
!
!   input argument list:
...
!$$$
  ...
  use general_specmod, only: spec_vars
  use kinds, only: r_kind,i_kind
  use variables, only: zero,nlon,nlat
  implicit none
  ...
  real(r_kind) work(nlon,nlat-2),spec_work(sp%nc)
...
!  (replace earlier algorithm that assumed zero gradient next to pole)
  call general_spectra_pole_scalar (sp,grid_out,spec_work)
...
subroutine general_zds2pcg(sp,zsp,dsp,pgrd,cgrd)
...
! program history log:
!   2006-07-15  kleist
!   2010-02-18  parrish, copy zds2pcg to general_zds2pcg--replace 'use specmod'
!   with structure var sp
!                           also replace 'use gridmod' with input structure
!                           variable grd
!
!   input argument list:
...
! Inverse laplacian
  ...
  call general_s2g0(sp,spc1,pgrd)
  call general_s2g0(sp,spc2,cgrd)

  return
end subroutine general_zds2pcg

subroutine general_spectra_pole_scalar (sp,field,coefs)
...
! program history log:
!   2007-05-15  errico
!   2009-11-24  parrish -- change values of starting assoc. legendre polynomial
!   alp0(0),alp0(1)
!                           to be consistent with what is computed in sptranf0.
!                          alp0(0) = sqrt(half), instead of alp0(0)=one
!                    and   alp0(1) = sqrt(three)*alp0(0), instead of
!                    alp0(1)=sqrt(three)
...
!$$$
  use general_specmod, only: spec_vars
  use kinds, only: r_kind,i_kind
  use variables, only: zero,half,three,nlon,nlat
