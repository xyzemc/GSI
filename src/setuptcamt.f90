...
contains
  ...
  !$$$
    ...
    use obsmod, only: tcamthead,tcamttail,rmiss_single,i_tcamt_ob_type,obsdiags,&
  ...
  !    Link obs to diagnostics structure
       if(luse_obsdiag)then
          if (.not.lobsdiag_allocated) then
             if (.not.associated(obsdiags(i_tcamt_ob_type,ibin)%head)) then
                allocate(obsdiags(i_tcamt_ob_type,ibin)%head,stat=istat)
                if (istat/=0) then
                   write(6,*)'setuptcamt: failure to allocate obsdiags',istat
                   call stop2(295)
                end if
                obsdiags(i_tcamt_ob_type,ibin)%tail => obsdiags(i_tcamt_ob_type,ibin)%head
             else
                allocate(obsdiags(i_tcamt_ob_type,ibin)%tail%next,stat=istat)
                if (istat/=0) then
                   write(6,*)'setuptcamt: failure to allocate obsdiags',istat
                   call stop2(295)
                end if
                obsdiags(i_tcamt_ob_type,ibin)%tail => obsdiags(i_tcamt_ob_type,ibin)%tail%next
             end if
             allocate(obsdiags(i_tcamt_ob_type,ibin)%tail%muse(miter+1))
             allocate(obsdiags(i_tcamt_ob_type,ibin)%tail%nldepart(miter+1))
             allocate(obsdiags(i_tcamt_ob_type,ibin)%tail%tldepart(miter))
             allocate(obsdiags(i_tcamt_ob_type,ibin)%tail%obssen(miter))
             obsdiags(i_tcamt_ob_type,ibin)%tail%indxglb=i
             obsdiags(i_tcamt_ob_type,ibin)%tail%nchnperobs=-99999
             obsdiags(i_tcamt_ob_type,ibin)%tail%luse=.false.
             obsdiags(i_tcamt_ob_type,ibin)%tail%muse(:)=.false.
             obsdiags(i_tcamt_ob_type,ibin)%tail%nldepart(:)=-huge(zero)
             obsdiags(i_tcamt_ob_type,ibin)%tail%tldepart(:)=zero
             obsdiags(i_tcamt_ob_type,ibin)%tail%wgtjo=-huge(zero)
             obsdiags(i_tcamt_ob_type,ibin)%tail%obssen(:)=zero
  
             n_alloc(ibin) = n_alloc(ibin) +1
             my_diag => obsdiags(i_tcamt_ob_type,ibin)%tail
             my_diag%idv = is
             my_diag%iob = i
             my_diag%ich = 1
          else
             if (.not.associated(obsdiags(i_tcamt_ob_type,ibin)%tail)) then
                obsdiags(i_tcamt_ob_type,ibin)%tail => obsdiags(i_tcamt_ob_type,ibin)%head
             else
                obsdiags(i_tcamt_ob_type,ibin)%tail => obsdiags(i_tcamt_ob_type,ibin)%tail%next
             end if
             if (obsdiags(i_tcamt_ob_type,ibin)%tail%indxglb/=i) then
                write(6,*)'setuptcamt: index error'
                call stop2(297)
             end if
          end if
       end if
  ...
  !    Interpolate to get tcamt at obs location/time
       call tintrp2a11(this%ges_tcamt,tcamtges,dlat,dlon,dtime,hrdifsig,&
              mype,nfldsig)
  ...
  !       Accumulate statistics for obs belonging to this task
       ...
       if(luse_obsdiag)then
          obsdiags(i_tcamt_ob_type,ibin)%tail%luse=luse(i)
          obsdiags(i_tcamt_ob_type,ibin)%tail%muse(jiter)=muse(i)
          obsdiags(i_tcamt_ob_type,ibin)%tail%nldepart(jiter)=ddiff
          obsdiags(i_tcamt_ob_type,ibin)%tail%wgtjo= (error*ratio_errors)**2
       end if
  ...
  !    in inner loop minimization (int* and stp* routines)
       if (.not. last .and. muse(i)) then
          ...
          if(.not. associated(tcamthead(ibin)%head))then
             allocate(tcamthead(ibin)%head,stat=istat)
             if(istat /= 0)write(6,*)' failure to write tcamthead '
             tcamttail(ibin)%head => tcamthead(ibin)%head
          else
             allocate(tcamttail(ibin)%head%llpoint,stat=istat)
             if(istat /= 0)write(6,*)' failure to write tcamttail%llpoint '
             tcamttail(ibin)%head => tcamttail(ibin)%head%llpoint
          end if
  
  	m_alloc(ibin) = m_alloc(ibin) + 1
  	my_head => tcamttail(ibin)%head
  	my_head%idv = is
  	my_head%iob = i
  
  !       Set (i,j) indices of guess gridpoint that bound obs location
          call get_ij(mm1,dlat,dlon,tcamttail(ibin)%head%ij(1),tcamttail(ibin)%head%wij(1))
  
          tcamttail(ibin)%head%res     = ddiff
          tcamttail(ibin)%head%err2    = error**2
          tcamttail(ibin)%head%raterr2 = ratio_errors**2    
          tcamttail(ibin)%head%time    = dtime
          tcamttail(ibin)%head%b       = cvar_b(ikx)
          tcamttail(ibin)%head%pg      = cvar_pg(ikx)
          tcamttail(ibin)%head%luse    = luse(i)
          if(luse_obsdiag)then
             tcamttail(ibin)%head%diags => obsdiags(i_tcamt_ob_type,ibin)%tail
   
             my_head => tcamttail(ibin)%head
             my_diag => tcamttail(ibin)%head%diags
  ...
  !    Save stuff for diagnostic output
       if(conv_diagsave .and. luse(i))then
          ...
          if (lobsdiagsave) then
             ...
             do jj=1,miter 
                ioff=ioff+1 
                if (obsdiags(i_tcamt_ob_type,ibin)%tail%muse(jj)) then
             ...
             do jj=1,miter+1
                ioff=ioff+1
                rdiagbuf(ioff,ii) = obsdiags(i_tcamt_ob_type,ibin)%tail%nldepart(jj)
             enddo
             do jj=1,miter
                ioff=ioff+1
                rdiagbuf(ioff,ii) = obsdiags(i_tcamt_ob_type,ibin)%tail%tldepart(jj)
             enddo
             do jj=1,miter
                ioff=ioff+1
                rdiagbuf(ioff,ii) = obsdiags(i_tcamt_ob_type,ibin)%tail%obssen(jj)
             enddo
          endif
  ...
  ! Write information to diagnostic file
    if(conv_diagsave)then
       call dtime_show(this%myname,'diagsave:tcamt',i_tcamt_ob_type)
