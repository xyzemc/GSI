...
!                       . removed (%dlat,%dlon) debris.
  ...
  use obsmod, only: rmiss_single,i_tcamt_ob_type,obsdiags,&
       ...
       if(luse_obsdiag)then
          if (.not.lobsdiag_allocated) then
             if (.not.associated(obsdiags(i_tcamt_ob_type,ibin)%head)) then
              obsdiags(i_tcamt_ob_type,ibin)%n_alloc = 0
           ...
           obsdiags(i_tcamt_ob_type,ibin)%n_alloc = obsdiags(i_tcamt_ob_type,ibin)%n_alloc +1
           ...
           obsdiags(i_tcamt_ob_type,ibin)%tail%indxglb=ioid(i)
           ...
           if (.not.associated(obsdiags(i_tcamt_ob_type,ibin)%tail)) then
              call die(this%myname,'.not.associated(obsdiags(i_tcamt_ob_type,ibin)%tail)')
           end if
           if (obsdiags(i_tcamt_ob_type,ibin)%tail%indxglb/=ioid(i)) then
  ...
  !    in inner loop minimization (int* and stp* routines)
       if (.not. last .and. muse(i)) then
        ...
        my_node => my_head
        call obsLList_appendNode(tcamthead(ibin),my_node)
        my_node => null()
  ...
  !       Set (i,j) indices of guess gridpoint that bound obs location
        ...
        my_head%luse    = luse(i)
          ...
          if(luse_obsdiag)then
           my_head%diags => obsdiags(i_tcamt_ob_type,ibin)%tail
