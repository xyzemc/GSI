...
use abstract_setup_mod
  ...
  interface setupbend_class
     module procedure setup_ctor
  end interface
contains
  type(setupbend_class) function setup_ctor(obsname,varname1,varname2,varname3)
  ...
  !    Link obs to diagnostics structure
       if(luse_obsdiag)then
         if (.not.lobsdiag_allocated) then
            ...
            if (.not.associated(obsdiags(i_gps_ob_type,ibin)%tail)) then
               call die(this%myname,'.not.associated(obsdiags(i_gps_ob_type,ibin)%tail)')
            end if
