...
contains
  ...
  !   2005-11-29  derber - remove psfcg and use this%ges_lnps instead
  !   2006-01-31  todling/treadon - store wgt/wgtlim in rdiagbuf(6,ii)
  !   2006-02-02  treadon - rename lnprsl as this%ges_lnprsl
  !   2006-02-08  treadon - correct vertical dimension (nsig) in call tintrp2a(this%ges_tv...)
  ...
  !    Link obs to diagnostics structure
     if (luse_obsdiag) then
          if (.not.lobsdiag_allocated) then
           ...
           if (.not.associated(obsdiags(i_spd_ob_type,ibin)%tail)) then
              call die(this%myname,'.not.associated(obsdiags(i_spd_ob_type,ibin)%tail)')
           end if
...
!   call this%final_vars_
  ...
  ! Write information to diagnostic file
    if(conv_diagsave .and. ii>0)then
       call dtime_show(this%myname,'diagsave:spd',i_spd_ob_type)
