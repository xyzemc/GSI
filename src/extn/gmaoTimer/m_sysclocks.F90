...
!#include "regime.H"
    ...
    module m_sysclocks
      ...
      public:: sysclocks_kind

      integer(kind=kind(C_DOUBLE)),parameter:: sysclocks_kind=C_DOUBLE

      interface sysclocks_get
        subroutine sysclocks_get(times) bind(C,name='get_zeits_')
	  ! void get_zeits_(double *zts){}
          import sysclocks_kind
	  implicit none
	  real(kind=sysclocks_kind),dimension(*),intent(out):: times
