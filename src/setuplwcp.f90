...
!
! abstract:  For solid-water condensate path (lwcp), this routine
...
! Declare local variables
  ...
  integer(i_kind) ier,ilon,ilat,ilwcp,id,itime,ikx,ilwcpmax,iqc
  ...
  real(r_kind),dimension(nsig):: qlges, qrges
...
!    Link obs to diagnostics structure
     if (luse_obsdiag) then
        if (.not.lobsdiag_allocated) then
           if (.not.associated(obsdiags(i_lwcp_ob_type,ibin)%head)) then
              obsdiags(i_lwcp_ob_type,ibin)%n_alloc = 0
              allocate(obsdiags(i_lwcp_ob_type,ibin)%head,stat=istat)
              if (istat/=0) then
                 write(6,*)'setuplwcp: failure to allocate obsdiags',istat
                 call stop2(269)
              end if
              obsdiags(i_lwcp_ob_type,ibin)%tail => obsdiags(i_lwcp_ob_type,ibin)%head
           else
              allocate(obsdiags(i_lwcp_ob_type,ibin)%tail%next,stat=istat)
              if (istat/=0) then
                 write(6,*)'setuplwcp: failure to allocate obsdiags',istat
                 call stop2(270)
              end if
              obsdiags(i_lwcp_ob_type,ibin)%tail => obsdiags(i_lwcp_ob_type,ibin)%tail%next
           end if
           obsdiags(i_lwcp_ob_type,ibin)%n_alloc = obsdiags(i_lwcp_ob_type,ibin)%n_alloc +1
    
           allocate(obsdiags(i_lwcp_ob_type,ibin)%tail%muse(miter+1))
           allocate(obsdiags(i_lwcp_ob_type,ibin)%tail%nldepart(miter+1))
           allocate(obsdiags(i_lwcp_ob_type,ibin)%tail%tldepart(miter))
           allocate(obsdiags(i_lwcp_ob_type,ibin)%tail%obssen(miter))
           obsdiags(i_lwcp_ob_type,ibin)%tail%indxglb=ioid(i)
           obsdiags(i_lwcp_ob_type,ibin)%tail%nchnperobs=-99999
           obsdiags(i_lwcp_ob_type,ibin)%tail%luse=luse(i)
           obsdiags(i_lwcp_ob_type,ibin)%tail%muse(:)=.false.
           obsdiags(i_lwcp_ob_type,ibin)%tail%nldepart(:)=-huge(zero)
           obsdiags(i_lwcp_ob_type,ibin)%tail%tldepart(:)=zero
           ...
           if (.not.associated(obsdiags(i_lwcp_ob_type,ibin)%tail)) then
              obsdiags(i_lwcp_ob_type,ibin)%tail => obsdiags(i_lwcp_ob_type,ibin)%head
           else
              obsdiags(i_lwcp_ob_type,ibin)%tail => obsdiags(i_lwcp_ob_type,ibin)%tail%next
           end if
           if (.not.associated(obsdiags(i_lwcp_ob_type,ibin)%tail)) then
              call die(myname,'.not.associated(obsdiags(i_lwcp_ob_type,ibin)%tail)')
           end if
           if (obsdiags(i_lwcp_ob_type,ibin)%tail%indxglb/=ioid(i)) then
              write(6,*)'setuplwcp: index error'
              call stop2(271)
           end if
        endif
     endif
...
! Interpolate ges_* to obs location
     ...
     ! Interpolate model lwcp to obs location
     call tintrp2a11(ges_lwcp,lwcpges,dlat,dlon,dtime, &
...
!    Gross checks using innovation
     ...
     residual = abs(ddiff)
     if (residual>grsmlt*data(ilwcpmax,i)) then
        error = zero
        ratio_errors=zero
...
!    Compute penalty terms (linear & nonlinear qc).
        ...
        if (cvar_pg(ikx) > tiny_r_kind .and. error > tiny_r_kind) then
           ...
           cg_lwcp=cvar_b(ikx)
           wgross = cg_term*cvar_pg(ikx)/(cg_lwcp*wnotgross)
...
!       Accumulate statistics for obs belonging to this task
     ...
     if (luse_obsdiag) then
        obsdiags(i_lwcp_ob_type,ibin)%tail%muse(jiter)=muse(i)
        obsdiags(i_lwcp_ob_type,ibin)%tail%nldepart(jiter)=ddiff
        obsdiags(i_lwcp_ob_type,ibin)%tail%wgtjo= (error*ratio_errors)**2
     endif
...
!    in inner loop minimization (int* and stp* routines)
     if ( .not. last .and. muse(i)) then
        ...
        allocate(my_head%ij(4, nsig), &
                 ...
                 my_head%jac_q(nsig  ), &
                 my_head%jac_ql(nsig  ), &
                 my_head%jac_qr(nsig  ), stat=istat)
        if (istat/=0) write(6,*)'MAKECOBS:  allocate error for lwcphead, istat=',istat


!       Set (i,j) indices of guess gridpoint that bound obs location
        call get_ij(mm1,dlat,dlon,lwcp_ij,my_head%wij)
...
! Calculate Jacobians for lwcp
        ...
        do k=1,nsig
          ...
          my_head%ij(1,k)=lwcp_ij(1)+(k-1)*latlon11
          my_head%ij(2,k)=lwcp_ij(2)+(k-1)*latlon11
          my_head%ij(3,k)=lwcp_ij(3)+(k-1)*latlon11
          my_head%ij(4,k)=lwcp_ij(4)+(k-1)*latlon11
...
!=============================================================================================================
        ...
        if (luse_obsdiag) then
           my_head%diags => obsdiags(i_lwcp_ob_type,ibin)%tail
...
!    Save select output for diagnostic file
     if(conv_diagsave .and. luse(i))then
        ...
        rdiagbuf(17,ii) = dlwcp              ! solid-water content path obs (kg/m**2)
        rdiagbuf(18,ii) = ddiff              ! obs-ges used in analysis (kg/m**2)
        rdiagbuf(19,ii) = dlwcp-lwcpges          ! obs-ges w/o bias correction (kg/m**2) (future slot)

        ioff=ioff0
        if (lobsdiagsave) then
           do jj=1,miter 
              ioff=ioff+1 
              if (obsdiags(i_lwcp_ob_type,ibin)%tail%muse(jj)) then
           ...
           do jj=1,miter+1
              ioff=ioff+1
              rdiagbuf(ioff,ii) = obsdiags(i_lwcp_ob_type,ibin)%tail%nldepart(jj)
           enddo
           do jj=1,miter
              ioff=ioff+1
              rdiagbuf(ioff,ii) = obsdiags(i_lwcp_ob_type,ibin)%tail%tldepart(jj)
           enddo
           do jj=1,miter
              ioff=ioff+1
              rdiagbuf(ioff,ii) = obsdiags(i_lwcp_ob_type,ibin)%tail%obssen(jj)
           enddo
        endif
