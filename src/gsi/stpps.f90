...
!           .      .    .                                       .
! module:   stppsmod    module for stpps_search and stpps
!  prgmmr:
!
! abstract: module for stpps_search and stpps
!
! program history log:
...
!   2019-08-15  kbathmann - split into stpps and stpps_search
!
! subroutines included:
!   sub stpps_search,stpps
!
! attributes:
!   language: f90
!   machine:
...
PRIVATE
PUBLIC stpps_search,stpps

contains

subroutine stpps_search(pshead,rval,out,sges,nstep)
!$$$  subprogram documentation block
!                .      .    .                                       .
! subprogram:    stpps_search calculate search direction, penalty and contribution to
...
! Simply return if any pointer not found
  ...
  do while (associated(psptr))
     if(psptr%luse)then
        ...
        w3 = psptr%wij(3)
        w4 = psptr%wij(4)
        psptr%val =w1* rp(j1)+w2* rp(j2)+w3* rp(j3)+w4* rp(j4)
        do kk=1,nstep
           ps=psptr%val2+sges(kk)*psptr%val
...
!   for Dr. Jim purser' non liear quality control
  ...
  end do !while associated(psptr)
  
  return
end subroutine stpps_search
subroutine stpps(pshead,out,sges,nstep)
!$$$  subprogram documentation block
!                .      .    .                                       .
