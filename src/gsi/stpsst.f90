...
!           .      .    .                                       .
! module:   stpsstmod    module for stpsst_search and stpsst
!  prgmmr:
!
! abstract: module for stpsst_search and stpsst
!
! program history log:
...
!   2019-08-23  kbathmann - split into stpsst and stpsst_search
!
! subroutines included:
!   sub stpsst_search,stpsst
!
! attributes:
!   language: f90
!   machine:
...
PRIVATE
PUBLIC stpsst_search,stpsst

contains

subroutine stpsst_search(ssthead,rval,out,sges,nstep)
!$$$  subprogram documentation block
!                .      .    .                                       .
! subprogram: stpsst_search  calculate search direction, penalty and contribution to stepsize
  ...
  do while (associated(sstptr))
     if(sstptr%luse)then
        ...
        if ( nst_gsi > 2 .and. (sstptr%tz_tr > zero .and. sstptr%tz_tr <= one) ) then
           ...
           sstptr%val  = sstptr%tz_tr*rdir
        else
           sstptr%val =w1*rsst(j1)+w2*rsst(j2)+w3*rsst(j3)+w4*rsst(j4)
        endif
        do kk=1,nstep
           sst=sstptr%val2+sges(kk)*sstptr%val
...
!  Modify penalty term if nonlinear QC
  ...
  return
end subroutine stpsst_search
subroutine stpsst(ssthead,out,sges,nstep)
!$$$  subprogram documentation block
!                .      .    .                                       .
...
!
!   output argument list:
!     out(1:nstep)   - contribution to penalty for conventional sst -
!     sges(1:nstep)
!
! attributes:
!   language: f90
  ...
  do while (associated(sstptr))
     if(sstptr%luse)then
        if(nstep > 0)then
           ...
           pen(1)=sstptr%res*sstptr%res*sstptr%err2 !KAB is this right?
        end if
