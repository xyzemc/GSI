...
!           .      .    .                                       .
! module:   stpgustmod    module for stpgust_search and stpgust
!  prgmmr:
!
! abstract: module for stpgust_search and stpgust
!
! program history log:
!   2009-02-24  zhu
!   2016-05-18  guo     - replaced ob_type with polymorphic obsNode through type casting
!   2019-08-26  kbathmann - split into stpgust and stpgust_search
!
! subroutines included:
!   sub stpgust_search, stpgust
!
! attributes:
!   language: f90
!   machine:
...
PRIVATE
PUBLIC stpgust_search,stpgust

contains

subroutine stpgust_search(gusthead,rval,out,sges,nstep)
!$$$  subprogram documentation block
!                .      .    .                                       .
! subprogram: stpgust_search  calculate search direction, penalty and contribution to stepsize
...
!
! program history log:
!   2019-08-26  zhu  - split computation of val into its own subroutine
!
!   input argument list:
!     gusthead
...
! Simply return if any pointer not found
  ...
  do while (associated(gustptr))
     if(gustptr%luse)then
        ...
        gustptr%val =w1*rgust(j1)+w2*rgust(j2)+w3*rgust(j3)+w4*rgust(j4)

        do kk=1,nstep
           gust=gustptr%val2+sges(kk)*gustptr%val
...
!  Modify penalty term if nonlinear QC
  ...
  end do !while associated(gustptr)
  
  return
end subroutine stpgust_search

subroutine stpgust(gusthead,out,sges,nstep)
!$$$  subprogram documentation block
!                .      .    .                                       .
...
!
!   output argument list:
!     out(1:nstep)   - contribution to penalty for conventional gust -
!     sges(1:nstep)
!
! attributes:
!   language: f90
