...
!  SUBSET=NC0062XX -- RADIAL WIND FROM CAPS Level 2 radar obs on grid-tilt
...
!   2016-09-xx  G. Zhao -  add capability reading CAPS column-tilt radar radial wind data
...
! --- CAPS ---
  ...
  use caps_radaruse_mod, only: oe_rw, lvldbg, refl_lowbnd_rw
  use caps_radaruse_mod, only: l_correct_azmu, l_correct_tilt, i_correct_tilt, &
                               l_azm_east1st, l_plt_diag_rw
  use caps_radaruse_mod, only: l_use_rw_caps
! --- CAPS ---
  implicit none 
...
! ====================================================================!
!       variables used for CAPS L2RWBUFR data
!
! --- BUFR Key word variables for l2rwbufr_colmtilt by CAPS f88d2arps
  character(80):: hdrstr2(3), obstr
  real(r_kind) :: thiserr_rw
  real(r_double),dimension(5,maxlevs):: radar_obs_caps                ! to read in radar obs in CAPS l2rwbufr

! CAPS L2 radar data number statisitcs
...
! filename for CAPS level 2 radar obs data on column-tilt (produced by 88d2arps)
  character(255) :: fname_l2rwbufr_caps
! ====================================================================!

  real(r_kind) thisazm, azm_east
  integer(i_kind) ncapsl2_in,ncapsl2_kept
! --- CAPS --- 
...
! --- CAPS ---
! used for BUFR CAPS Radial Wind on column-tilt
...
! --- CAPS----
  if ( l_use_rw_caps) then
! add level 2 bufr format RW data on column-tilt by CAPS f88d2arps (g.zhao)
     fname_l2rwbufr_caps='l2rwbufr_cltl'
     inquire(file=trim(fname_l2rwbufr_caps),exist=lexist3)
     if ( .not. lexist3 )  then
         write(6,'(1x,A70,A20,A10,I6)') &
             ' CAPS level-2 column-tilt radar wind file is not found and to skip : ', &
             trim(fname_l2rwbufr_caps), ' on pe:',mype
     else
         write(6,'(1x,A70,A20,A10,I6)') &
             ' CAPS level-2 column-tilt radar wind file is found and to read : ', &
             trim(fname_l2rwbufr_caps), ' on pe:',mype
     end if
  end if
! --- CAPS ---
...
!  Bigger loop over first level 2.5 data, and then level3 data
  ...
  ! do loop=1,2
     ...
     write(6,*)'READ_RADAR: CAPS L2RW radar radial wind obs error (user-defined in namelist)=',thiserr_rw,'(m/s).'
...
!    dist2min=huge(dist2min)
     ...
     if(loop==-1)     outmessage='level 2 radar obs (CAPS):'

!    Open, then read bufr data
     open(lnbufr,file=trim(fname_l2rwbufr_caps),form='unformatted')
!     rewind(lnbufr)

     call openbf(lnbufr,'IN',lnbufr)
!     call dxdump(lnbufr,lnbufr_table)
     call datelen(10)

!     call readmg(lnbufr,subset,idate,iret)
!     if(iret/=0) then
!       write(6,*)'(mype=',mype,') CAPS L2 radar file is not in BUFR format! '
!       call closbf(lnbufr)
!       go to 1001
!     end if
     ...
     msg_report: do while (ireadmg(lnbufr,subset,idate) == 0)
         ...
         if(subset(1:6)/='NC0062') then
             write(6,*)'mype=',mype,' subset check does not pass for CAPS L2 file. subset=',subset
             cycle msg_report
         end if
         if (lvldbg > 1) &
             write(6,'(1x,A6,I4,A30,A10,1x,A12,I10,1x,A10,I8)')'(mype:',mype,') READ_RADAR: subset:',subset,' data_date:',idate,' nmsg:',nmsg

!        Big loop over bufr file

! 51     call readsb(lnbufr,iret)
! 61     continue
...
!            if(hdr(3)==r360) hdr(3)=hdr(3)-r360
!            if(hdr(3)<zero ) hdr(3)=hdr(3)+r360
...
!            check up radar_table
             ...
             if ( icnt_radar_read == 0 ) then                             ! first radar in table
                 ...
                 loop_rdrtbl: do irdr=1,icnt_radar_read
                     if ( this_staid .eq. radartable_id(irdr) ) then      ! found same radar in table
                         irdr_ptr=irdr
                         exit loop_rdrtbl
                     end if
                 end do loop_rdrtbl
                 if (irdr_ptr .le. 0) then                            !  found new radar and add it into table
...
!            check up whether radar station is inside the domain
             ...
             if(regional)then
                 ...
                 if (outside) then
                     if ( radartable_inside(irdr_ptr) > 0 ) then
                         write(6,*)'*** WARNING --> READ_RADAR: (CAPS) radar:',this_staid,' is found both inside and outside. Wrong! <-- WARNING ***'
                         call stop2(999)
                     end if
             ...
             staheight=hdr(4)             ! tation elevation  HSMSL 
!            staheight=staheight+hdr(5)   ! is adding height of Antenna  HSALG

!---------------------------------------------------------------------------------!
!            Skipping finding vad wind match for CAPS column-tilt radar wind
!            data
!---------------------------------------------------------------------------------!

!            Get model terrain at radar station location: zsges
             call deter_zsfc_model(dlat,dlon,zsges)
!            If want to use radar which is outside of domain but its data might
!            be inside
!            if (outside) then
!                use station height as model terrain if outside of domain
!                zsges = staheight
!            else
!                call deter_zsfc_model(dlat,dlon,zsges)
!            end if
...
!          Go through the data levels
             call ufbint(lnbufr,radar_obs_caps,5,maxlevs,levs,obstr)
             if(levs>maxlevs) then
                 write(6,*)'READ_RADAR:  ***ERROR*** increase read_radar bufr size since ',&
                     'number of levs=',levs,' > maxlevs=',maxlevs
                 call stop2(84)
             ...
             loop_lvl : do k=1,levs
                 ncapsl2_in=ncapsl2_in+1
                 nread=nread+1
                 ntb=ntb+1
!                 t4dvo=real(minobs+radar_obs_caps(1,k)-iwinbgn,r_kind)*r60inv
...
!              Exclude data if it does not fall within time window
!              Skiipping the following part since no individual time for each obs
!                 if (l4dvar.or.l4densvar) then
!                     if (t4dvo<zero .OR. t4dvo>winlen) cycle loop_lvl
!                     timeo=t4dv
!                 else
!                     timeo=(real(minobs-mincy,r_kind)+real(radar_obs_caps(1,k),r_kind))*r60inv
!                     timeo=(real(minobs-mincy,r_kind)+real(0.0,r_kind))*r60inv
!                     if(abs(timeo)>twind .or. abs(timeo) > ctwind(ikx)) then
!                         write(6,*)'READ_RADAR:  time outside window ',timeo,&
!                            ' skip obs ',nread,' at lev=',k
!                         cycle loop_lvl
...
!              Extract radial wind data
                 thistilt = radar_obs_caps(1,k)              ! radar scan/tilt angle (degree)
                 height   = radar_obs_caps(2,k)              ! obs beam height (meter)
                 range1   = radar_obs_caps(3,k)              ! obs beam range (meter)
                 rwnd     = radar_obs_caps(4,k)              ! radial wind (m/s)
                 rdbz     = radar_obs_caps(5,k)              ! reflectivity (dBZ)
...
!                        write(9992,'(1x,A4,A4,A9,F7.3,A10,F12.1,A10,F12.1,A17,F7.3,A9,F7.3,A14,F7.1,A8,F12.1)') &
!                            'ID:',this_staid,' tilt(o):',thistilt,' range(o):',range1,  &
!                            ' range(c):',range0,' tilt(computed):',elvang0, &
!                            ' tilt(crc):',corrected_tilt,' obs_hgt(o):',height, &
!                            ' sfcrng:',sfcrng0
                     end if
...
!              obs box (???? not used for CAPS L2RWBUFR data)
!                 iaaa=azm/(r360/(r8*irrr))
!                 iaaa=mod(iaaa,8*irrr)
!                 if(iaaa<0) iaaa=iaaa+8*irrr
!                 iaaa=iaaa+1
...
!              obs error inflation with erradar_inflate (not really used for CAPS L2RWBUFR data)
!                error = erradar_inflate*radar_obs(7,k)
                 error = erradar_inflate*oe_rw

!              Increase error for lev2.5 and lev3
!               (for specific domain: but commented off and not used for CAPS L2RWBUFR data)
...
!                write(9999,*) '------------------------------------------'
                 if(abs(azm)>r400) then
                     ibadazm=ibadazm+1; good0=.false.
!                    write(9999,'(1x,A65,3F20.5)') 'bad azmimuth -- azm_corr_rotat (rwnd rdbz):', azm,rwnd,rdbz
                 end if
                 if(abs(rwnd)>r200) then
                     ibadwnd=ibadwnd+1; good0=.false.
!                    write(9999,'(1x,A50,3F20.5)') 'bad rw wind -- rw rdbz:', rwnd, rdbz
                     ...
                     else if (rdbz>=-20.0 .AND. rdbz < -0.5 ) then
                         igoodwnd_dbz(2) = igoodwnd_dbz(2)+1
                     else if (rdbz>=-0.5 .AND. rdbz <  0.5 ) then
                         igoodwnd_dbz(3) = igoodwnd_dbz(3)+1
                     else if (rdbz>= 0.5 .AND. rdbz <  5.0 ) then
                         igoodwnd_dbz(4) = igoodwnd_dbz(4)+1
                     else if (rdbz>= 5.0 .AND. rdbz <  100.0 ) then
                 ...
                 if(dist>r400) then
                     ibaddist=ibaddist+1; good0=.false.
!                    write(9999,'(1x,A50,3F20.5)') 'bad distance -- dist (rwnd rdbz):', dist,rwnd,rdbz
                 end if
                 if(staheight<-r1000 .or. staheight>r50000) then
                     ibadstaheight=ibadstaheight+1; good0=.false.
!                    write(9999,'(1x,A50,3F20.5)') 'bad station height -- staheight:', staheight
                 end if
                 if(height<-r1000 .or. height>r50000) then
                     ibadheight=ibadheight+1; good0=.false.
!                    write(9999,'(1x,A50,3F20.5)') 'bad obs hgt -- height (rwnd rdbz):', height,rwnd,rdbz
                 end if
                 if(height<staheight) then
                     iheightbelowsta=iheightbelowsta+1 ; good0=.false.
!                    write(9999,'(1x,A50,4F20.5)') 'obs below stn -- height staheight (rwnd rdbz):', height,staheight,rwnd,rdbz
                 end if

!                if(radar_obs(7,k)>r6 .or. radar_obs(7,k)<=zero) then
!                if(oe_rw>r6*100.0_r_kind .or. oe_rw<=zero) then    ! used to pre-set very large value for oe_rw in namelist in some special tests
...
!                    write(9999,'(1x,A50,3F20.5)') 'bad dbz -- rdbz:', rdbz,rwnd
!                else if (rdbz >= -20.0_r_kind .AND. rdbz < 5.0_r_kind) then
                 else if (rdbz >= -20.0_r_kind .AND. rdbz < refl_lowbnd_rw) then
                     idbz_clr=idbz_clr+1; good0=.false.
!                    write(9999,'(1x,A50,3F20.5)') 'clearsky dbz -- rdbz:', rdbz,rwnd
...
!                     write(6,*)'READ_RADAR:  ***ERROR*** wrong dbz :',rdbz
!                    write(9999,'(1x,A50,3F20.5)') 'wrong dbz :', rdbz,rwnd
                 end if
!                write(9999,*) '=========================================='

!              good and notgood are used for further QC check with VAD match.
!              note:   No further QC check with VAD match for CAPS L2RWBUFR data
!                      So good is actually same as good0
                 ...
                 if(.not.good0) then
                     ...
                     cycle loop_lvl
!                else
!                     Note: No further QC Check against vad wind quality mark for CAPS L2RWBUFR data
                 end if

!              If data is good, load into output array
                 if(good) then

!                  no vad and obs box counting for CAPS L2RWBUFR
!                  (they are used for NCEP L2/2.5/3 radar obs, see code for those data)
                     ...
                     ncapsl2_kept=ncapsl2_kept+1
                     if ( ndata+1 .gt. maxobs ) then
                         do irpt=1,5
                             write(6,'(1x,A128)')' *** *** WARNING --> READ_RADAR: ndata is more than maxobs(=2,000,000) --> increase maxobs!!! <-- WARNING *** ***'
                         end do
                         write(6,'(1x,A128)')' *** *** WARNING --> INCREASE maxobs in READ_RADAR beyond 2,000,000, re-compile GSI, re-run !!! <-- WARNING *** ***'
                     end if
                     ndata  = min(ndata+1,maxobs)
                     nodata = min(nodata+1,maxobs)  !number of obs not used (no meaning here)
                     usage  = zero
...
!                     end if
                     ...
                     cdata(20)=zsges              ! model elevation at radar site
!                    cdata(21)=radar_obs(7,k)     ! original error from bufr file
!                    cdata(21)=oe_rw              ! original error (user-defined in namelist)
...
!    Close unit to bufr file
     ...
     write(6,*)'READ_RADAR:  ',trim(outmessage),' reached eof on level 2 CAPS radar file (grid-tilt).'

     if(loop==-1)    write(6,*)'READ_RADAR: ncapsl2_in,ncapsl2_kept=',ncapsl2_in,ncapsl2_kept
     write(6,*)'READ_RADAR: # no vad match   =',novadmatch
     write(6,*)'READ_RADAR: # out of vadrange=',ioutofvadrange
     write(6,*)'READ_RADAR: # bad azimuths=',ibadazm
     write(6,*)'READ_RADAR: # bad winds   =',ibadwnd
...
!        if ( radartable_inside(icnt) ) then
...
! end do       !   end bigger loop

! end of read CAPS radar level 2 data
!====================================================================================!
  end if
! --- CAPS ---
