...
! Declare local parameters
  ...
  integer(i_kind),parameter:: maxdat=24_i_kind, ione = 1_i_kind         ! Used in generating cdata array
  character(len=4), parameter :: radid = 'XXXX'
  
! === Grid vr data declaration

  real(r_kind), allocatable, dimension(:)       :: vrQC, data_r_1d, vr_err, height,&
                                                   lon, lat, utime
  real(r_kind), allocatable, dimension(:,:)     :: data_r_2d, sta_info
  ...
  real(r_kind)                                  :: dir1, dir2, dir3 


!--Counters for diagnostics
 integer(i_kind) :: num_missing=0,num_nopcp=0, &      !counts 
                    numbadtime=0, numoutdomain=0, &  
                    num_m2nopcp=0, &
  ...
  integer(i_kind) :: ierror,i,j,k,v,na,nb,nelv,nvol, &
                     ikx,mins_an,mins_ob
  integer(i_kind) :: maxobs,nchanl,ilat,ilon,idomsfc,scount
  ...
  do i=ione,nconvtype
     if(trim(obstype) == trim(ioctype(i)) .and. abs(icuse(i))== ione) then
        ...
        radartwindow=ctwind(ikx)*r60         !Time window units converted to minutes 
                                             !  (default setting for vr within convinfo is 0.05 hours)
        exit                                 !Exit loop when finished with initial convinfo fields     
     else if ( i==nconvtype ) then
        write(6,*) 'READ_RADAR_WIND: ERROR - OBSERVATION TYPE IS NOT PRESENT IN CONVINFO OR USE FLAG IS ZERO'
        write(6,*) 'READ_RADAR_WIND: ABORTTING read_radar_wind_netcdf.f90 - NO radial velocity OBS READ!'
        return
     endif
  end do     

  if (minobrange >= maxobrange) then
  write(6,*) 'MININMUM OB RANGE >= MAXIMUM OB RANGE FOR READING dBZ - PROGRAM STOPPING FROM READ_RADAR_WIND_NETCDF.F90'
  call stop2(400)
  end if
...
!!end modified for thinning
  ...
  var_list(1:7) = (/ "sta      ", "error_var", "height   ", "lon      ", &
                     "lat      ", "value    ", "utime    "/)
  var_num       = 7

  !print *, "read_radar_wind_netcdf.f90: open ",trim(infile)       
 ...
 fileopen: if (if_input_exist) then         
  ...
  DO ivar = 1, var_num
    ...
    allocate( vrQC(dims(6,1)), vr_err(dims(2,1)), height(dims(3,1)), &
               lon(dims(4,1)),    lat(dims(5,1)),  utime(dims(7,1)), &
              sta_info(dims(1,1), dims(1,2)) )
    ...
    do ivar = 1, var_num
      if( ivar .eq. 1 )then
       ...
       !sta:units = "Platform metadata array:  degrees, degrees,
       !             undef/sfc/modlevel/Pa/meters, vert_coord, radians, 
       !             radians, radians, m/s, number" ;
       !sta:description = "Platform metadata array:  lon, lat, height,
       !                   vertical_coord, dir1, dir2, dir3, Nyquist, key" 

       do i = 1, dims(ivar,1)
         do j = 1, dims(ivar,2)
           sta_info(i,j)    = data_r_2d(i,j)
         end do
       end do
       ...
       if( ivar == 6 )then
         ...
         vr_err = data_r_1d   !input value actually variance for now
       else if( ivar == 3 )then
         height = data_r_1d
       else if( ivar == 4 )then
         lon    = data_r_1d
  ...
  do i = 1, dims(ivar,1)
       ...
       if (outside) then 
          numoutdomain = numoutdomain+ione
          cycle
       endif
       ...
       if ( (dir1 /= dir1) .or. (dir2 /= dir2) .or. (dir3 /= dir3) ) then
         print '(A25, 4F8.2)', 'WARNING: NaN angles, skip ', vrQC(i), dir1, dir2, dir3
         cycle
       endif
       ...
       !    cycle azms.         
       radar_lon=deg2rad*sta_info(1,i)
       radar_lat=deg2rad*sta_info(2,i)
...
!reachded ok
           ...
           !!end modified for thinning
            ...
            cdata_all(1,iout) = thiserr                       ! radial velocity obs error (ms-1) - inflated/adjusted
            cdata_all(2,iout) = dlon                          ! grid relative longitude
            cdata_all(3,iout) = dlat                          ! grid relative latitude
            cdata_all(4,iout) = thishgt                       ! obs absolute height (m)
            cdata_all(5,iout) = vrQC(i)                       ! radar radial velocity
            cdata_all(6,iout) = dir1                          ! sin(azimuth)*cos(elevation)
            cdata_all(7,iout) = timeb*r60inv                  ! obs time (analyis relative hour)
            cdata_all(8,iout) = ikx                           ! type		   
            cdata_all(9,iout) = dir2                          ! cos(azimuth)*cos(elevation)
            cdata_all(10,iout)= sta_info(3,i)                 ! station elevation (m)
            ...
            cdata_all(23,iout) = sta_info(8,i)                ! nyq vel (m/s)
            cdata_all(24,iout) =dir3                          ! sin(elevation)

            !print*, cdata_all(1,iout), cdata_all(21,iout)
...
!---all looping done now print diagnostic output
  ...
  write(6,*)'READ_RADAR_WIND: # read in obs. number               =',nread
  write(6,*)'READ_RADAR_WIND: # readout obs. numver               =',ndata
  write(6,*)'READ_RADAR_WIND: # obs outside time window           =',numbadtime
  write(6,*)'READ_RADAR_WIND: # obs outside model domain          =',numoutdomain
