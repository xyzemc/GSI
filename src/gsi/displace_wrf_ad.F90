...
module displace_wrf_ad_m
  use fp_types_m, only: fp, fp_kind
...
SUBROUTINE DISPLACE_WRF_FIELDS_AD(th_compute, istep, qv_ind, moist, nmoist, moist_ad&
     ...
     hgt, mub, mu, mu_ad, psfc, psfc_ad, znu, fcadisp, fcadisp_ad,&
  ...
  REAL(fp), DIMENSION(ims:ime, jms:jme, kms:kme), INTENT(INOUT) :: ph_ad, ph, phb
  REAL(fp), DIMENSION(ims:ime, jms:jme, kms:kme) :: p_ad, t_ad, u_ad, v_ad, w_ad
  REAL(fp), DIMENSION(ims:ime, jms:jme, kms:kme, 1:nmoist), INTENT(INOUT) :: moist
  REAL(fp), DIMENSION(ims:ime, jms:jme, kms:kme, 1:nmoist) :: moist_ad
  REAL(fp), DIMENSION(ims:ime, jms:jme), INTENT(IN) :: hgt, mub
  REAL(fp), DIMENSION(ims:ime, jms:jme), INTENT(INOUT) :: mu, psfc
  REAL(fp), DIMENSION(ims:ime, jms:jme) :: mu_ad, psfc_ad
  ...
  REAL(fp), DIMENSION(ims:ime, jms:jme, kms:kme) :: ph_hyd_ad, ph_nl_ad
  ...
  integer :: ISIZE2OFu, ISIZE1OFu, ISIZE3OFu, ISIZE2OFv, ISIZE1OFv, ISIZE3OFv, &
  ...
  ph_nl = ph + phb - ph_hyd
  CALL PUSHREAL4ARRAY(u(ims,jms,kms), fp_kind*ISIZE3OFu*ISIZE1OFu*ISIZE2OFu/4)
  ...
  CALL APPLY_VERT(u, 0, work2d, fcadisp,&
       needed_ij_in, needed_ij_out, index_in, needed_wgts_out, num_glob_needed, interp_order)
  if (istep .gt. 1) then
     CALL PUSHREAL4ARRAY(v(ims,jms,kms), fp_kind*ISIZE3OFv*ISIZE1OFv*ISIZE2OFv/4)
     CALL APPLY_VERT(v, 0, work2d, fcadisp,&
          needed_ij_in, needed_ij_out, index_in, needed_wgts_out, num_glob_needed, interp_order)
     if (istep .gt. 2) then
        CALL PUSHREAL4ARRAY(w(ims,jms,kms), fp_kind*ISIZE3OFw*ISIZE1OFw*ISIZE2OFw/4)
    ! NOT: Only apply displacements to first kte levels, even for staggered variables w, ph_nl
	CALL APPLY_VERT(w, 1, work2d, fcadisp,&
             needed_ij_in, needed_ij_out, index_in, needed_wgts_out, num_glob_needed, interp_order)
        if (istep .gt. 3) then
	   CALL PUSHREAL4ARRAY(ph_nl(ims,jms,kms), fp_kind*SIZE(ph, 1)*SIZE(ph, 2)*SIZE(ph, &
                3)/4)
           ...
           if (istep .gt. 4) then
              ...
              DO i=1,result1
                 IF (i .NE. qv_ind) THEN
		    CALL PUSHREAL4ARRAY(moist(ims, jms, kms, i), fp_kind*ISIZE1OFmoist*&
	         ...
	         CALL ADJ_WRF_DERIVED_AD(moist(:, :, :, qv_ind), moist_ad(:, :, :, &
                      ...
                      psfc_ad, znu, th_compute, fcadisp, fcadisp_ad, istep,&
                      needed_ij_in, needed_ij_out, index_in, needed_wgts_out, num_glob_needed, interp_order)
	      endif ! .gt. 5
	     
              DO i=result1,1,-1
                 IF (i .NE. qv_ind) THEN
		    CALL POPREAL4ARRAY(moist(ims, jms, kms, i), fp_kind*ISIZE1OFmoist*&
                         ISIZE2OFmoist*ISIZE3OFmoist/4)
	   ...
	   CALL POPREAL4ARRAY(ph_nl(ims,jms,kms), fp_kind*SIZE(ph, 1)*SIZE(ph, 2)*SIZE(ph, 3&
                )/4)
	   CALL APPLY_VERT_AD(ph_nl, ph_nl_ad, 1, work2d, work2d_ad, fcadisp, fcadisp_ad, interp_order)
	   
        endif ! .gt. 3
        CALL POPREAL4ARRAY(w(ims,jms,kms), fp_kind*ISIZE3OFw*ISIZE1OFw*ISIZE2OFw/4)
	CALL APPLY_VERT_AD(w, w_ad, 1, work2d, work2d_ad, fcadisp, fcadisp_ad, interp_order)
	
     endif ! .gt. 2
     CALL POPREAL4ARRAY(v(ims,jms,kms), fp_kind*ISIZE3OFv*ISIZE1OFv*ISIZE2OFv/4)
     CALL APPLY_VERT_AD(v, v_ad, 0, work2d, work2d_ad, fcadisp, fcadisp_ad, interp_order)
  endif ! .gt. 1
  CALL POPREAL4ARRAY(u(ims,jms,kms), fp_kind*ISIZE3OFu*ISIZE1OFu*ISIZE2OFu/4)
...
SUBROUTINE CALC_PH_HYD_WRF_AD(qvapor, qvapor_ad, p, p_ad, pb, t, t_ad&
     , hgt, mu, mu_ad, mub, znu, ph_hyd, ph_hyd_ad)
  IMPLICIT NONE
  REAL(fp), DIMENSION(ims:ime, jms:jme, kms:kme), INTENT(IN) :: qvapor, p, pb, t
  REAL(fp), DIMENSION(ims:ime, jms:jme, kms:kme) :: qvapor_ad, p_ad, t_ad
  REAL(fp), DIMENSION(ims:ime, jms:jme), INTENT(IN) :: hgt, mu, mub
  REAL(fp), DIMENSION(ims:ime, jms:jme) :: mu_ad
  REAL(fp), DIMENSION(kms:kme), INTENT(IN) :: znu
  ! nonhydrostatic component for geopotential
  REAL(fp), DIMENSION(ims:ime, jms:jme, kms:kme) :: ph_hyd
  REAL(fp), DIMENSION(ims:ime, jms:jme, kms:kme) :: ph_hyd_ad
  INTEGER :: i
  REAL(fp), DIMENSION(ims:ime, jms:jme, kms:kme) :: rhod
  REAL(fp), DIMENSION(ims:ime, jms:jme, kms:kme) :: rhod_ad
  ! model Eta vertical levels
  REAL(fp) :: deta(SIZE(znu)), znuw(size(znu)+1)
  REAL(fp) :: temp3
  REAL(fp) :: temp2(ims:ime, jms:jme, kms:kme)
  REAL(fp) :: temp1(ims:ime, jms:jme, kms:kme)
  REAL(fp) :: temp0(ims:ime, jms:jme, kms:kme)
  REAL(fp) :: temp1_ad(ims:ime, jms:jme, kms:kme)
  REAL(fp) :: temp4_ad(ims:ime, jms:jme)
  INTRINSIC SIZE
  REAL(fp) :: temp_ad(ims:ime, jms:jme, kms:kme)
  REAL(fp) :: temp(ims:ime, jms:jme, kms:kme)
  
#ifdef TRACE_USE
  if (trace_use) call da_trace_entry("calc_ph_hyd_wrf_ad")
#endif
  
  rhod(:,:,kts:kte) = (p(:,:,kts:kte)+pb(:,:,kts:kte))*(1-rv/rd*qvapor(:,:,kts:kte))/(rd*(t(:,:,kts:kte)+t0)*((p(:,:,kts:kte)+pb(:,:,kts:kte))/p0)**(rd/cp))
  znuw(kte+1) = 0
  znuw(kts) = 1
  ...
  rhod_ad = 0.0
  DO i=kte+1,kts+1,-1
     temp4_ad = deta(i-1)*ph_hyd_ad(:, :, i)/rhod(:, :, i-1)
     ph_hyd_ad(:, :, i-1) = ph_hyd_ad(:, :, i-1) + ph_hyd_ad(:, :, i)
     mu_ad = mu_ad + temp4_ad
     rhod_ad(:, :, i-1) = rhod_ad(:, :, i-1) - (mub+mu)*temp4_ad/rhod(:&
          , :, i-1)
     ph_hyd_ad(:, :, i) = 0.0
  END DO
  ...
  WHERE (temp2(:,:,kts:kte) .LE. 0.0 .AND. (temp3 .EQ. 0.0 .OR. temp3 .NE. INT(&
       temp3))) 
     p_ad(:,:,kts:kte) = p_ad(:,:,kts:kte) + temp0(:,:,kts:kte)*temp1_ad(:,:,kts:kte)
  ELSEWHERE
     p_ad(:,:,kts:kte) = p_ad(:,:,kts:kte) + temp3*temp2(:,:,kts:kte)**(temp3-1)*rd*(t(:,:,kts:kte)+t0)*temp_ad(:,:,kts:kte)/p0 + temp0(:,:,kts:kte)&
          *temp1_ad(:,:,kts:kte)
  END WHERE
...
SUBROUTINE ADJ_WRF_DERIVED_AD(qvapor, qvapor_ad, p, p_ad, pb, t, t_ad&
  ...
  REAL(fp), DIMENSION(ims:ime, jms:jme, kms:kme) :: qvapor_ad, p_ad, t_ad
  REAL(fp), DIMENSION(ims:ime, jms:jme, kms:kme) :: ph_ad
  REAL(fp), DIMENSION(ims:ime, jms:jme, kms:kme), INTENT(IN) :: ph_nl, phb
  REAL(fp), DIMENSION(ims:ime, jms:jme, kms:kme) :: ph_nl_ad
  REAL(fp), DIMENSION(ims:ime, jms:jme), INTENT(IN) :: hgt, mub
  REAL(fp), DIMENSION(ims:ime, jms:jme), INTENT(INOUT) :: mu, psfc
  REAL(fp), DIMENSION(ims:ime, jms:jme) :: mu_ad, psfc_ad
  REAL(fp), DIMENSION(kms:kme), INTENT(IN) :: znu
  ...
  real(fp), intent(in) :: needed_wgts_out(:,:)
  integer, intent(in) :: interp_order
  integer :: ISIZE2OFt, ISIZE1OFt, ISIZE3OFt, ISIZE2OFqvapor, ISIZE3OFqvapor, ISIZE1OFqvapor, &
  ...
  REAL(fp), DIMENSION(ims:ime, jms:jme, kms:kme) :: tk_ad, &
       ptot_ad, p_inc_ad, q_inc_ad
  REAL(fp), DIMENSION(ims:ime, jms:jme, kms:kme) :: ph_temp
  REAL(fp), DIMENSION(ims:ime, jms:jme, kms:kme) :: ph_temp_ad
  ! model Eta vertical levels
  REAL(fp) :: deta(kms:kme), znuw(kms:kme)
  REAL(fp), DIMENSION(ims:ime, jms:jme) :: sum1, sum2
  REAL(fp), DIMENSION(ims:ime, jms:jme) :: sum1_ad, sum2_ad
  REAL(fp), DIMENSION(ims:ime, jms:jme, 2) :: work2d
  REAL(fp), DIMENSION(ims:ime, jms:jme, 2) :: work2d_ad
  REAL(fp), DIMENSION(ims:ime, jms:jme) :: tbl, ts, tm, zl, tslv&
       , pslv, ps_inc, mu_inc
  REAL(fp), DIMENSION(ims:ime, jms:jme) :: tbl_ad, ts_ad, tm_ad&
       , zl_ad, tslv_ad, pslv_ad, ps_inc_ad, mu_inc_ad
  REAL(fp), DIMENSION(ims:ime, jms:jme) :: result1
  REAL(fp), DIMENSION(ims:ime, jms:jme) :: result1_ad
  INTEGER :: branch
  REAL(fp) :: temp3
  REAL(fp) :: temp2
  REAL(fp) :: temp0_ad
  ...
  INTRINSIC EXP
  REAL(fp) :: temp1_ad
  REAL(fp) :: temp15_ad(ims:ime, jms:jme)
  REAL(fp) :: temp18(ims:ime, jms:jme)
  REAL(fp) :: temp17(ims:ime, jms:jme)
  REAL(fp) :: temp16(ims:ime, jms:jme)
  REAL(fp) :: temp15(ims:ime, jms:jme)
  REAL(fp) :: temp14(ims:ime, jms:jme)
  REAL(fp) :: temp4_ad
  REAL(fp) :: temp13(ims:ime, jms:jme)
  REAL(fp) :: temp12(ims:ime, jms:jme)
  REAL(fp) :: temp11(ims:ime, jms:jme)
  REAL(fp) :: temp10
  INTRINSIC SIZE
  REAL(fp) :: temp19_ad(ims:ime, jms:jme)
  REAL(fp) :: temp7_ad
  INTRINSIC LOG
  REAL(fp) :: temp19_ad0(ims:ime, jms:jme)
  REAL(fp) :: temp11_ad(ims:ime, jms:jme)
  REAL(fp) :: temp9_ad(ims:ime, jms:jme)
  REAL(fp) :: temp12_ad(ims:ime, jms:jme)
  REAL(fp) :: temp
  REAL(fp) :: temp9(ims:ime, jms:jme)
  REAL(fp) :: temp8
  REAL(fp) :: temp7
  REAL(fp) :: temp6
  REAL(fp) :: temp5
  ...
  IF (th_compute .EQ. 1) THEN
     CALL PUSHREAL4ARRAY(t(ims,jms,kms), fp_kind*ISIZE3OFt*ISIZE1OFt*ISIZE2OFt/4)
     CALL DISPLACE_THETA_WRF(t, ph, phb, fcadisp,&
          needed_ij_in, needed_ij_out, index_in, needed_wgts_out, num_glob_needed, interp_order)
  ELSE
     CALL PUSHREAL4ARRAY(t(ims,jms,kms), fp_kind*ISIZE3OFt*ISIZE1OFt*ISIZE2OFt/4)
     CALL PUSHREAL4ARRAY(t, fp_kind*ISIZE3OFt*ISIZE1OFt*ISIZE2OFt/4)
  ...
  if (istep .gt. 6) then
     ...
     DO i= ims, ime
           ...
           DO k= kts,kte-1
              ...
              ELSE IF (ptot(i, j, k) .GT. psfc(i, j) - 100e2 .AND. ptot(i, j&
                 ...
                 tbl(i, j) = (tk(i, j, k+1)*LOG(ptot(i, j, k)/(psfc(i, j)-&
                      100e2))+tk(i, j, k)*LOG((psfc(i, j)-100e2)/ptot(i, j, k+1)&
        ...
        END DO
     END DO
     ts = tbl*(psfc/(psfc-100e2))**(rd*gamma/grav)
     ...
     tslv = tbl + gamma*zl
     !sea-level pressure
     pslv = psfc*EXP(grav*hgt/(rd*(ts+tslv)/2.))
     ! Now perform displacement of sea level pressure
     call apply_disp_2d(Pslv,work2d(:,:,1),fcadisp,&
          needed_ij_in, needed_ij_out, index_in, needed_wgts_out, num_glob_needed, interp_order)
     CALL PUSHREAL4ARRAY(work2d(ims, jms, 2), fp_kind*SIZE(p, 1)*SIZE(p, 2)/4&
          )
     work2d(:, :, 2) = work2d(:, :, 1)/EXP(grav*hgt/(rd*(ts+tslv)/2.))
     ! get the surface pressure increment
     ps_inc = work2d(:, :, 2) - psfc
     if (istep .gt. 7) then
        ! Store the new Psfc
        DO k= kts, kte
           CALL PUSHREAL4ARRAY(work2d(ims, jms, 1), fp_kind*SIZE(p, 1)*SIZE(p, 2)&
                /4)
	   ...
	   CALL PUSHREAL4ARRAY(q_inc(ims, jms, k), fp_kind*SIZE(p, 1)*SIZE(p, 2)/&
                4)
           q_inc(:, :, k) = result1 - qvapor(:, :, k)
        END DO
	CALL PUSHREAL4ARRAY(qvapor(ims,jms,kms), fp_kind*ISIZE1OFqvapor*ISIZE3OFqvapor*&
             ISIZE2OFqvapor/4)
        qvapor(:, :, :) = qvapor(:, :, :) + q_inc
        if (istep .gt. 8) then
           ...
           CALL PUSHREAL4ARRAY(mu(ims,jms), fp_kind*ISIZE2OFmu*ISIZE1OFmu/4)
           ...
           if (istep .gt. 9) then
              DO i= kte, kts, -1
                 ...
                 END IF
              END DO
	      CALL PUSHREAL4ARRAY(p(ims,jms,kms), fp_kind*ISIZE3OFp*ISIZE1OFp*ISIZE2OFp/4)
              p = p + p_inc
              if (istep .gt. 10) then
		 ...
		 ! no pushing or popping in CALC_PH_HYD_WRF_AD
                 CALL CALC_PH_HYD_WRF_AD(qvapor, qvapor_ad, p, p_ad, pb, t, t_ad, hgt&
                      , mu, mu_ad, mub, znu, ph_temp, ph_temp_ad)
              endif !istep .gt. 10
              p_inc_ad = 0.0
              CALL POPREAL4ARRAY(p(ims,jms,kms), fp_kind*ISIZE3OFp*ISIZE1OFp*ISIZE2OFp/4)
              p_inc_ad = p_ad
              q_inc_ad = 0.0
              mu_inc_ad = 0.0
              DO i= kts, kte, 1
                 ...
                 IF (.NOT.branch .LT. 2) p_inc_ad(:, :, i+1) = p_inc_ad(:, :, i+1) &
                      + p_inc_ad(:, :, i)
                 temp19_ad0 = deta(i)*p_inc_ad(:, :, i)
                 mu_inc_ad = mu_inc_ad + (qvapor(:, :, i)+1)*temp19_ad0
                 qvapor_ad(:, :, i) = qvapor_ad(:, :, i) + mu_inc*temp19_ad0
                 mu_ad = mu_ad + q_inc(:, :, i)*temp19_ad0
                 q_inc_ad(:, :, i) = q_inc_ad(:, :, i) + (mub+mu)*temp19_ad0
                 p_inc_ad(:, :, i) = 0.0
              END DO
           endif !istep .gt. 9
           CALL POPREAL4ARRAY(mu(ims,jms), fp_kind*ISIZE2OFmu*ISIZE1OFmu/4)
           ...
           ps_inc_ad = temp19_ad
           mu_ad = mu_ad - sum1*temp19_ad
           sum1_ad = -((mub+mu)*temp19_ad)
           sum2_ad = -((ps_inc-(mub+mu)*sum1)*temp19_ad/sum2)
        endif !istep .gt. 8
	
        DO i= kts, kte, 1
           qvapor_ad(:, :, i) = qvapor_ad(:, :, i) + deta(i)*sum2_ad
           q_inc_ad(:, :, i) = q_inc_ad(:, :, i) + deta(i)*sum1_ad
        END DO
        CALL POPREAL4ARRAY(qvapor(ims,jms,kms), fp_kind*ISIZE1OFqvapor*ISIZE3OFqvapor*&
             ISIZE2OFqvapor/4)
...
!!$    dx_n_ad = 0.0
	...
	DO k= kte, kts, -1
           result1_ad = 0.0
	   CALL POPREAL4ARRAY(q_inc(ims, jms, k), fp_kind*SIZE(p, 1)*SIZE(p, 2)/4&
                )
	   ...
	   CALL POPREAL4ARRAY(work2d(ims, jms, 1), fp_kind*SIZE(p, 1)*SIZE(p, 2)/&
                4)
     ...
     ts_ad = 0.0
     tslv_ad = 0.0
     CALL POPREAL4ARRAY(work2d(ims, jms, 2), fp_kind*SIZE(p, 1)*SIZE(p, 2)/4)
     temp18 = rd*(ts+tslv)
     temp17 = 2.*grav*hgt
     ...
     zl_ad = gamma*tslv_ad
     tm_ad = 0.0
     temp11 = (psfc-100e2)/psfc
     tm_ad = -(LOG(temp11)*rd*zl_ad/grav)
     ts_ad = temp12_ad + tm_ad/2. + temp15_ad
     temp11_ad = -(tm*rd*zl_ad/(psfc*temp11*grav))
     temp10 = rd*gamma/grav
     temp9 = psfc/(psfc-100e2)
     WHERE (temp9 .LE. 0.0 .AND. (temp10 .EQ. 0.0 .OR. temp10 .NE. INT(&
          temp10))) 
        temp9_ad = 0.0
     ELSEWHERE
        temp9_ad = temp10*temp9**(temp10-1)*tbl*ts_ad/(psfc-100e2)
     END WHERE
     psfc_ad = EXP(temp12)*pslv_ad + temp9_ad - temp9*temp9_ad + &
          temp11_ad - temp11*temp11_ad - ps_inc_ad
     tbl_ad = 0.0
     tbl_ad = tm_ad/2. + temp9**temp10*ts_ad + tslv_ad
  ...
  IF (th_compute .EQ. 1) THEN
     CALL POPREAL4ARRAY(t(ims,jms,kms), fp_kind*ISIZE3OFt*ISIZE1OFt*ISIZE2OFt/4)
...
SUBROUTINE DISPLACE_THETA_WRF_AD(t, t_ad, ph, ph_ad, phb, fcadisp, fcadisp_ad,&
  ...
  REAL(fp), DIMENSION(ims:ime, jms:jme, kms:kme) :: t_ad
  ...
  REAL(fp), DIMENSION(ims:ime, jms:jme, kms:kme) :: ph_tot_ad
  REAL(fp), DIMENSION(ims:ime, jms:jme, kms:kme) :: tknew, znew, zlvl
  REAL(fp), DIMENSION(ims:ime, jms:jme, kms:kme) :: tknew_ad, znew_ad, zlvl_ad
  INTEGER :: k
  ...
  DO k=kte,kts,-1
     ...
     ph_tot_ad(:, :, k) = ph_tot_ad(:, :, k) + zlvl_ad(:, :, k)/(2*grav)
     ph_tot_ad(:, :, k+1) = ph_tot_ad(:, :, k+1) + zlvl_ad(:, :, k)/(2*grav)
     zlvl_ad(:, :, k) = 0.0
  END DO
  ph_ad = 0.0
  ph_ad = ph_tot_ad
...
SUBROUTINE APPLY_DISP_THETA_AD(fcadisp, fcadisp_ad, tk, zlvl, zlvl_ad, znew, znew_ad, tknew_ad, &
  ...
  type (fca_gridded_disp), intent(in) :: fcadisp			! displacement field generated by FCA routine
  type (fca_gridded_disp), intent(inout) :: fcadisp_ad		! displacement field generated by FCA routine
  REAL(fp), DIMENSION(ims:ime,jms:jme,kts:kte), INTENT(IN) :: znew, tk, zlvl
  REAL(fp), DIMENSION(ims:ime,jms:jme,kts:kte), intent(out) :: znew_ad, zlvl_ad
  REAL(fp), DIMENSION(ims:ime,jms:jme,kts:kte), intent(inout) :: tknew_ad
  integer, intent(in) :: num_glob_needed, needed_ij_in(:,:), needed_ij_out(:,:), index_in(:)
  real(fp), intent(in) :: needed_wgts_out(:,:)
  integer, intent(in) :: interp_order
  INTEGER :: i, j, k, l
  ...
  REAL(fp), DIMENSION(SIZE(tk, 1), SIZE(tk, 2), 2) :: t_bot_top_ad
...
SUBROUTINE SH2RH_AD0(hum, hum_ad, t, t_ad, parr, parr_ad, sh2rh_ad)
  ...
  REAL(fp), DIMENSION(ims:ime, jms:jme) :: hum_ad, t_ad, parr_ad
  REAL(fp), DIMENSION(ims:ime, jms:jme) :: sh2rh
  REAL(fp), DIMENSION(ims:ime, jms:jme) :: sh2rh_ad
  INTEGER :: i, j
  REAL(fp) :: r, e, esat, p
  REAL(fp) :: r_ad, e_ad, esat_ad, p_ad
  REAL(fp) :: rdorv, ew, t1
  REAL(fp) :: ew_ad, t1_ad
  ...
  DO j= jms, jme
     DO i= ims, ime
        ...
        ELSE
           tk = t(i, j)
           CALL PUSHREAL4ARRAY(rhs, fp_kind/4)
           rhs = -(c1*(tf/tk-1.)) - c2*LOG10(tf/tk) + c3*(1.-tk/tf) + &
                LOG10(eis)
           ew = 10.**rhs
           CALL PUSHINTEGER4(2)
        END IF
        CALL PUSHREAL4ARRAY(esat, fp_kind/4)
  ...
  DO j= jme, jms, -1
     DO i= ime, ims, -1
        CALL POPINTEGER4(branch)
        IF (.NOT.branch .LT. 2) sh2rh_ad(i, j) = 0.0
        p = parr(i, j)
        r = hum(i, j)
        e = p*r/(rdorv+r)
        temp2_ad = 100*sh2rh_ad(i, j)/esat
        e_ad = temp2_ad
        esat_ad = -(e*temp2_ad/esat)
        sh2rh_ad(i, j) = 0.0
        CALL POPREAL4ARRAY(esat, fp_kind/4)
        ew_ad = 100.*esat_ad
        CALL POPINTEGER4(branch)
        IF (branch .LT. 2) THEN
           IF (branch .LT. 1) THEN
           ...
           ELSE
              t1 = t(i, j) - 273.16
              temp1 = t1/(t1+243.5)
              temp1_ad = es0*EXP(17.67*temp1)*17.67*ew_ad/(t1+243.5)
              t1_ad = (1.0-temp1)*temp1_ad
           END IF
        ELSE
           rhs_ad = 10.**rhs*LOG(10.)*ew_ad
           tk = t(i, j)
           CALL POPREAL4ARRAY(rhs, fp_kind/4)
           tk_ad = (c1*tf/tk**2+c2/(tk*LOG(10.0))-c3/tf)*rhs_ad
...
SUBROUTINE RH2SH_AD0(rh, rh_ad, t, t_ad, parr, parr_ad, rh2sh_ad)
  ...
  REAL(fp), DIMENSION(ims:ime, jms:jme) :: rh_ad, t_ad, parr_ad
  REAL(fp), DIMENSION(ims:ime, jms:jme) :: rh2sh
  REAL(fp), DIMENSION(ims:ime, jms:jme) :: rh2sh_ad
  INTEGER :: i, j
  REAL(fp) :: e, esat, p
  REAL(fp) :: e_ad, esat_ad, p_ad
  REAL(fp) :: rdorv, ew, t1
  REAL(fp) :: ew_ad, t1_ad
  ! From module_intialize_real.F:
  REAL(fp), PARAMETER :: t_ref=0.0
  ...
  DO j= jme, jms, -1
     DO i= ime, ims, -1
        ...
        p_ad = temp2_ad0
        rh2sh_ad(i, j) = 0.0
        esat_ad = rh(i, j)*e_ad/100.
        rh_ad(i, j) = rh_ad(i, j) + esat*e_ad/100.
        parr_ad(i, j) = parr_ad(i, j) + p_ad
        CALL POPREAL4ARRAY(esat, fp_kind/4)
        ew_ad = 100.*esat_ad
        CALL POPINTEGER4(branch)
