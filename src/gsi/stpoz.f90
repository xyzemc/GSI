...
!           .      .    .                                       .
! module:   stpozmod    module for stpoz_search and stpoz
!  prgmmr:
!
! abstract: module for stpoz_search and stpoz
!
! program history log:
...
!   2019-08-22  kbathmann - split into stpoz and stpoz_search
!
! subroutines included:
!   sub stpoz_search
!   sub stpozlay_search_
!   sub stpozlev_search_
!   sub stpoz
!   sub stpozlay_
!   sub stpozlev_
!
...
use constants, only: one,half,two,zero_quad,zero
...
PRIVATE
PUBLIC stpoz,stpoz_search
public:: stpozlay_search_    ! Non-generic interfaces are needed for now, to allow
public:: stpozlev_search_     ! passing-by-reference involkations, such as
                             !   call stpoz(..,pbcjo(1,i_oz_ob_type,ib),..)

public:: stpozlay_search     ! Generic interfaces are disirable, where full TKR
public:: stpozlev_search     ! matching are required if they are involked, such as
...
public:: stpozlev       ! matching are required if they are involked, such as
        ...
        interface stpozlay_search; module procedure stpozlay_search_; end interface
        interface stpozlev_search; module procedure stpozlev_search_; end interface
        interface stpozlay; module procedure stpozlay_; end interface
        interface stpozlev; module procedure stpozlev_; end interface

contains

subroutine stpoz_search(ozhead,o3lhead,rval,out,sges,nstep)
!$$$  subprogram documentation block
!                .      .    .                                       .
! subprogram:  stpoz_search  call components to calculate search direction,
!                            contribution to penalty and stepsize for ozone
!   prgmmr: sienkiewicz     org: GMAO                 date: 2009-01-22
!
! abstract: The routine calls individual components that calculate 
!           search direction, contribution to the penalty and step size
!           from layer and level ozone measurements
!
! program history log:
...
! Declare passed variables
  ...
  if(associated(ozhead))call stpozlay_search_(ozhead, rval,out,sges,nstep)
  if(associated(o3lhead))call stpozlev_search_(o3lhead,rval,out,sges,nstep)

  return

end subroutine stpoz_search
subroutine stpoz(ozhead,o3lhead,out,sges,nstep)
!$$$  subprogram documentation block
!                .      .    .                                       .
! subprogram:    stpoz   call components to calculate contribution
!                       to penalty and stepsize for ozone
!   prgmmr: sienkiewicz     org: GMAO                 date: 2009-01-22
!
! abstract: The routine calls individual components that calculate 
!           the contribution to the penalty and step size
!           from layer and level ozone measurements
!
! program history log:
!   2009-01-22  Sienkiewicz - incorporation of level ozone routine
...
! Declare passed variables
  ...
  if(associated(ozhead))call stpozlay_(ozhead,out,sges,nstep)
  if(associated(o3lhead))call stpozlev_(o3lhead,out,sges,nstep)

  return

end subroutine stpoz
subroutine stpozlay_search_(ozhead,rval,out,sges,nstep)
!$$$  subprogram documentation block
!                .      .    .                                       .
! subprogram:    stpoz       compute search direciton, contribution to penalty
!                            and stepsize for ozone, using nonlinear qc
!   prgmmr: derber          org: np23                 date: 1995-07-11
!
! abstract: The routine computes the search direction, contribution to the
!           penalty from ozone observations.  The routine also computes the
!           contribution of ozone observations to the step size.  This version
!           includes nonlinear qc.
!
! program history log:
!   2019-08-22 kbathmann split the computation of val into its own subroutine
!
!   input argument list:
...
! Can't do rank-2 pointer into rank-2, therefore, allocate work space
  allocate(roz(lat2*lon2,nsig)) !KAB can fix this somehow?
  do k=1,nsig
     ij=0
     do j=1,lon2
        do i=1,lat2
...
!       Accumulate contribution from layer observations
        ...
        if ( ozptr%nloz >= 1 ) then
           do k=1,ozptr%nloz
              ...
              do kk=1,nstep
                 oz=ozptr%val2(k)+sges(kk)*ozptr%val(k)
...
! End of routine.
  return
end subroutine stpozlay_search_
subroutine stpozlay_(ozhead,out,sges,nstep)
!$$$  subprogram documentation block
!                .      .    .                                       .
        ...
        if ( ozptr%nloz >= 1 ) then
           do k=1,ozptr%nloz
              if(nstep > 0)then
                 ...
                 pen(1)=ozptr%res(k)*ozptr%res(k)*ozptr%err2(k) !KAB is this right?
              end if
              out(1) = out(1)+pen(1)*ozptr%raterr2(k)
              do kk=2,nstep
                 out(kk) = out(kk)+(pen(kk)-pen(1))*ozptr%raterr2(k)
...
subroutine stpozlev_search_(o3lhead,rval,out,sges,nstep)
!$$$  subprogram documentation block
!                .      .    .                                       .
! subprogram:    stpozlev_search compute search direction, contribution to penalty and
...
! Loop over level ozone observations
  ...
  do while (associated(o3lptr))
     if(o3lptr%luse)then
        ...
        o3lptr%val= w1*roz1d(j1)+w2*roz1d(j2)+w3*roz1d(j3)+w4*roz1d(j4)+ &
             w5*roz1d(j5)+w6*roz1d(j6)+w7*roz1d(j7)+w8*roz1d(j8) 

        do kk=1,nstep
           oz=o3lptr%val2+sges(kk)*o3lptr%val
  ...
  end do !while associated(o3lptr)
! End of routine.
  return
end subroutine stpozlev_search_
subroutine stpozlev_(o3lhead,out,sges,nstep)
!$$$  subprogram documentation block
!                .      .    .                                       .
...
!   2007-01-02  sienkiewicz - separate subroutine
!   2007-01-05  sienkiewicz - update to 9/2006 GSI (new obs structure)
!   2009-01-21  sienkiewicz - update to 1/2009 GSI, changes based on stpq &
!   stpoz
!   2010-01-04  zhang,b - bug fix: accumulate penalty for multiple obs bins
!   2010-05-13  todling - udpate to use gsi_bundle
!
  ...
  do while (associated(o3lptr))
     if(o3lptr%luse)then
        if(nstep > 0)then
           ...
           pen(1) = o3lptr%res*o3lptr%res*o3lptr%err2 !KAB is this right?
        end if
     end if
     o3lptr => o3lNode_nextcast(o3lptr)
  end do !while associated(o3lptr)
