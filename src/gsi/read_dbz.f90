subroutine read_dbz(nread,ndata,nodata,infile,lunout,obstype,twind,sis,hgtl_full,nobs)
...
! === Grid dbz data declaration
  ...
  real(r_kind), allocatable, dimension(:)       :: dbzQC, data_r_1d, dbz_err, height,&
                                                   lon, lat, utime
...
!--Counters for diagnostics
 ...
 integer(i_kind)   num_dbz2mindbz,imissing2nopcp
  ...
  real(r_kind)    :: dbzhalo=15_r_kind           ! Reflectivity > than dbznoise and < dbzhalo that are NOT to be assimilated. Create Halo effect
...
!!end modified for thinning
 ...
 fileopen: if (if_input_exist) then         
  ...
  DO ivar = 1, var_num
    ...
    allocate( dbzQC(dims(6,1)), dbz_err(dims(2,1)), height(dims(3,1)), &
    ...
    do ivar = 2, var_num
       ...
       if( ivar == 6 )then
         ...
         dbz_err = data_r_1d  !input value actually variance for now
       else if( ivar == 3 )then
         height = data_r_1d
       else if( ivar == 4 )then
         lon    = data_r_1d
