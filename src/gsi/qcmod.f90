...
!     dsi          - scattering index quality control factor
!     fact1        - fact1 quality control parameter
!     lwp_ges      - LWP guess from model 
!     iwp_ges      - IWP guess from model
!     lwp_ret      - LWP guess from CSU1DVAR (tempestd bufr file) 
!     iwp_ret      - IWP guess from CSU1DVAR (tempestd bufr file) 
!
! attributes:
!     language: f90
!     machine:  JCSDA S4
...
! Declare passed variables
  ...
  real(r_kind),dimension(nsig,nchanl),intent(in   ) :: ptau5
  real(r_kind),dimension(nchanl),     intent(inout) :: errf,varinv
  real(r_kind), optional,             intent(in   ) :: lwp_ges, iwp_ges
  real(r_kind), optional,             intent(in   ) :: lwp_ret, iwp_ret

! Declare local parameters

  real(r_kind) :: demisf,dtempf,efact,vfact,dtbf,term,fact
  real(r_kind) :: iwp_threshold, lwp_threshold
  integer(i_kind) :: i
