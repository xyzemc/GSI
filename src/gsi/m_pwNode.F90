...
!                .      .    .                                       .
! subprogram:	 module m_pwNode
!   prgmmr:	 j guo <jguo@nasa.gov>
!      org:	 NASA/GSFC, Global Modeling and Assimilation Office, 610.3
!     date:	 2016-05-18
!
! abstract: class-module of obs-type pwNode (total column water)
!
! program history log:
...
!                         procedures to pwHeader_read_/pwHeader_write_.  This
...
! module interface:
  ...
  contains
    ...
    procedure, nopass:: headerRead  => pwHeader_read_
    procedure, nopass:: headerWrite => pwHeader_write_
    procedure:: init  => obsNode_init_
    procedure:: clean => obsNode_clean_
  end type pwNode

  public:: pwNode_typecast
  public:: pwNode_nextcast
        interface pwNode_typecast; module procedure typecast_ ; end interface
        interface pwNode_nextcast; module procedure nextcast_ ; end interface

  public:: pwNode_appendto
        interface pwNode_appendto; module procedure appendto_ ; end interface
...
!-- cast a class(obsNode) to a type(pwNode)
  use m_obsNode, only: obsNode
  implicit none
  type(pwNode  ),pointer:: ptr_
...
!-- cast an obsNode_next(obsNode) to a type(pwNode)
...
!-- append aNode to linked-list oLL
  ...
  implicit none
  type(pwNode),pointer,intent(in):: aNode
...
subroutine pwHeader_read_(iunit,mobs,jread,istat)
  ...
  character(len=*),parameter:: myname_=myname//".pwHeader_read_"
  integer(i_kind):: msig
_ENTRY_(myname_)
...
_EXIT_(myname_)
return
end subroutine pwHeader_read_

subroutine pwHeader_write_(junit,mobs,jwrite,jstat)
  use gridmod, only: nsig
  implicit none
  ...
  character(len=*),parameter:: myname_=myname//".pwHeader_write_"
_ENTRY_(myname_)
  write(junit,iostat=jstat) mobs,jwrite, nsig
_EXIT_(myname_)
return
end subroutine pwHeader_write_

subroutine obsNode_init_(aNode)
  use gridmod, only: nsig
  implicit none
  class(pwNode),intent(out):: aNode
