...
!           .      .    .                                       .
! module:   stpvwnd10mmod    module for stpvwnd10m_search and stpvwnd10m
!  prgmmr:
!
! abstract: module for stpvwnd10m_search and stpvwnd10m
!
! program history log:
!   2016-05-05  pondeca
!   2017-03-19  yang     - modify code to use polymorphic obsNode
!   2019-08-27  kbathmann - split into stpvwnd10m_search and stpvwnd10m
!
! subroutines included:
!   sub stpvwnd10m_search,stpvwnd10m
!
! attributes:
!   language: f90
!   machine:
...
PRIVATE
PUBLIC stpvwnd10m_search,stpvwnd10m

contains

subroutine stpvwnd10m_search(vwnd10mhead,rval,out,sges,nstep)
!$$$  subprogram documentation block
!                .      .    .                                       .
! subprogram: stpvwnd10m_search calculate search direction, penalty and contribution to stepsize
!
! abstract: calculate search direction, penalty and contribution to stepsize for 10m-vwind
...
! Simply return if any pointer not found
  ...
  do while (associated(vwnd10mptr))
     if(vwnd10mptr%luse)then
        ...
        vwnd10mptr%val =w1*rvwnd10m(j1)+w2*rvwnd10m(j2)+w3*rvwnd10m(j3)+w4*rvwnd10m(j4)

        do kk=1,nstep
           vwnd10m=vwnd10mptr%val2+sges(kk)*vwnd10mptr%val
...
!  Modify penalty term if nonlinear QC
  ...
  end do !while associated(vwnd10mptr)
  
  return
end subroutine stpvwnd10m_search
subroutine stpvwnd10m(vwnd10mhead,out,sges,nstep)
!$$$  subprogram documentation block
!                .      .    .                                       .
...
!                                         
!   output argument list:         
!     out(1:nstep)   - contribution to penalty for conventional vwnd10m -
!     sges(1:nstep)
!
! attributes:
!   language: f90
