...
!    also repoted using geopotential height.
     ...
     sfc_data = (itype >=280 .and. itype < 300) !x .and. (.not.twodvar_regional)
...
!       at observation location.
        call tintrp2a1(geop_hgtl,zges,dlat,dlon,dtime,hrdifsig,&
             nsig,mype,nfldsig)
        if (twodvar_regional) zges(nsig)= ten
           ...
           if (zob <= ten) then
              ...
              if(zob < ten)then
                 if (.not.twodvar_regional) then
                    ...
                    call rtma_comp_fact10(dlat,dlon,dtime,dpres,mype,factw)
                    print*,'xyz1----ugesin, vgestin= ', ugesin,vgesin 
                    print*,'xyz---- factw= ',factw
