...
!$$$
    ...
    real(r_kind),allocatable :: en_loc3(:,:,:,:)
...
!$$$
    ...
    use mpimod, only: mpi_comm_world,ierror,mpi_real8,mpi_integer4,mpi_max,mpi_rtype
    ...
    ! Declare passed variables
    integer(i_kind),     intent(in   ) :: ntindex
    real(r_kind),      intent(inout) :: en_loc3(lat2in,lon2in,nc2din+nc3din*nsigin,n_ensin)
    ...
    ! Declare internal variables
    character(len=*),parameter :: myname_='get_user_ens_arw_fastread_'
    ...
    real(r_kind),allocatable :: en_full(:,:,:,:)
    real(r_kind),allocatable :: en_loc(:,:,:,:)
    real(r_kind),allocatable :: tmp_subdo(:,:)
...
!!  read ensembles
    if(mype==io_pe) print*,iasm,iaemz,jasm,jaemz,kasm,kaemz,masm,maemz
...
!$$$
    ...
    ! Declare passed variables
    type(sub2grid_info), intent(in   ) :: grd
    real(r_kind),      intent(in   ) :: en_loc3(grd%lat2,grd%lon2,nc2d+nc3d*grd%nsig)
    type(gsi_bundle),    intent(inout) :: atm_bundle
    ...
    real(r_kind),allocatable,dimension(:,:)   :: scr2
    real(r_kind),allocatable,dimension(:,:,:) :: scr3
...
!   initialize atm_bundle to zero
    ...
    if ( i_radar_qr > 0 .and. i_radar_qg > 0 ) then
      ...
      ivar=getindex(cvars3d,'qnc')
      if( ivar > 0 ) call gsi_bundlegetpointer(atm_bundle,'qnc',qnc ,  ierr); iret = ierr + iret
...
!       automatically updated.
   ...
   !call update_scalar_poles_(grd3d,oz)
    if ( i_radar_qr > 0 .and. i_radar_qg > 0 ) then
      ...
      ivar=getindex(cvars3d,'qnc')
      if( ivar > 0 ) call update_scalar_poles_(grd3d,qnc)
    end if
    ...
    if ( i_radar_qr > 0 .and. i_radar_qg > 0 ) then
      ...
      ivar=getindex(cvars3d,'qnc')
      if( ivar > 0 ) call gsi_bundleputvar(atm_bundle,'qnc',qnc ,  ierr); iret = ierr + iret
    end if

    if ( iret /= 0 ) then
       if ( mype == 0 ) then
          write(6,'(A)') trim(myname) // ': ERROR!'
          write(6,'(A)') trim(myname) // ': For now, WRF needs to put all MetFields: ps,u,v,(sf,vp)tv,q,oz,cw'
          write(6,'(A)') trim(myname) // ': but some have not been found. Aborting ... '
       endif
       goto 100
    endif
...
subroutine parallel_read_wrfarw_state_(en_full,m_cvars2d,m_cvars3d,nlon,nlat,nsig, &
   ...
   use gridmod, only: eta1_ll,pt_ll,aeta1_ll,eta2_ll,aeta2_ll

   use hybrid_ensemble_parameters, only: q_hyb_ens
   use kinds, only: i_kind,r_kind,r_single
   use constants, only: r60,r3600,zero,one,half,pi,deg2rad,fv,rd_over_cp_mass,one_tenth,h300,rd,r1000
   use control_vectors, only: cvars2d,cvars3d,nc2d,nc3d
   use mpeu_util, only: getindex
   use control_vectors, only : w_exist, dbz_exist
   use general_sub2grid_mod, only: sub2grid_info
   ...
   real(r_kind),   intent(inout) :: en_full(iasm:iaemz,jasm:jaemz,kasm:kaemz,masm:maemz)
   character(len=*), intent(in   ) :: filename

   ! Declare local variables
   integer(i_kind) i,ii,j,jj,k,lonb,latb,levs,ivar
   ...
   real(r_kind),allocatable ::  temp3(:,:,:,:),temp2(:,:,:)
      ...
      integer(i_kind):: nx,ny,nz,d_max,file_id,var_id,ndim,mype
      ...
      integer(i_kind) iderivative, iens
   ...
   temp3 = zero
      ...
      allocate(gg_w(nlat,nlon,nsig))
      allocate(gg_dbz(nlat,nlon,nsig))
      allocate(gg_qr(nlat,nlon,nsig))
      allocate(gg_qs(nlat,nlon,nsig))
      allocate(gg_qi(nlat,nlon,nsig))
      allocate(gg_qg(nlat,nlon,nsig))
      allocate(gg_rho(nlat,nlon,nsig))
      allocate(gg_cwmr(nlat,nlon,nsig))
      allocate(gg_qnc(nlat,nlon,nsig))
      allocate(gg_qni(nlat,nlon,nsig))
      allocate(gg_qnr(nlat,nlon,nsig))
...
! u,v,t,q,w,dbz,qr,qs,qi,qg,qc,qnc,qni,qnr
   do k3=1,nc3d
     if(trim(cvars3d(k3))=='sf') temp3(:,:,:,k3) = gg_u
     if(trim(cvars3d(k3))=='vp') temp3(:,:,:,k3) = gg_v
     if(trim(cvars3d(k3))=='t') temp3(:,:,:,k3) = gg_tv
     if(trim(cvars3d(k3))=='q') temp3(:,:,:,k3) = gg_rh
     if(trim(cvars3d(k3))=='qr') temp3(:,:,:,k3) = gg_qr
     if(trim(cvars3d(k3))=='qs') temp3(:,:,:,k3) = gg_qs
     if(trim(cvars3d(k3))=='qi') temp3(:,:,:,k3) = gg_qi
     if(trim(cvars3d(k3))=='qg') temp3(:,:,:,k3) = gg_qg
     if(trim(cvars3d(k3))=='ql') temp3(:,:,:,k3) = gg_cwmr
     if(trim(cvars3d(k3))=='qnc') temp3(:,:,:,k3) = gg_qnc
     if(trim(cvars3d(k3))=='qni') temp3(:,:,:,k3) = gg_qni
     if(trim(cvars3d(k3))=='qnr') temp3(:,:,:,k3) = gg_qnr
     if(trim(cvars3d(k3))=='dbz') temp3(:,:,:,k3) = gg_dbz
     if(trim(cvars3d(k3))=='w') temp3(:,:,:,k3) = gg_w
   end do
   do k2=1,nc2d
     if(trim(cvars2d(k2))=='ps') temp2(:,:,k2) = gg_ps
   end do
...
!$$$
    ...
    integer(i_kind),intent(in   ) :: nlon,nlat
    real(r_kind), intent(in   ) :: work(nlon*(nlat))
    real(r_kind), intent(  out) :: temp(nlat,nlon)
