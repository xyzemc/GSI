...
!
  ...
  case('mls55','ompslpvis','ompslpuv','lims','uarsmls','mipas')
     select case(dfile_format(dfile))
     case('nc')
...
!  use mpeu_util, only: mprefix,stdout
  ...
  integer(kind=i_kind):: convVarID, qualVarID, errVarID, ozoneVarID
...
!!         if (press(ilev) .gt. 262.0 .or. press(ilev) .lt. 0.1 ) cycle ! undefined
           if (ozone(ilev, iprof) .lt. -900.0) cycle            ! undefined
           if (err(ilev, iprof) .lt. -900.0) cycle              ! undefined
