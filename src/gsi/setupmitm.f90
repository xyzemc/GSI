...
!                                     time in analysis
!   2020-01-23  guo     - removed mitmhead alias.
...
! Declare local parameters
  ...
  character(len=*),parameter:: myname='setupmitm'
...
! Interpolate guess mitm to observation location and time
     call tintrp2a11(ges_mitm,mitmges,dlat,dlon,dtime,hrdifsig,&
        mype,nfldsig)
...
!    in inner loop minimization (int* and stp* routines)
     if (.not. last .and. muse(i)) then
        ...
        allocate(my_head)
        call mitmNode_appendto(my_head,obsLL(ibin))
...
!    get mitm ...
     ...
     if (istatus==0) then
         if(allocated(ges_mitm))then
            write(6,*) trim(myname), ': ', trim(varname), ' already incorrectly alloc '
            call stop2(999)
         endif
         allocate(ges_mitm(size(rank2,1),size(rank2,2),nfldsig))
         ges_mitm(:,:,1)=rank2
         do ifld=2,nfldsig
...
900  format('conv_mitm_',i2.2,'.nc4')
...
!          call nc_diag_metadata("Nonlinear_QC_Var_Jb",     var_jb                 )
           ...
           call nc_diag_metadata("Observation",                   data(imitm,i)    )
           call nc_diag_metadata("Obs_Minus_Forecast_adjusted",   ddiff            )
           call nc_diag_metadata("Obs_Minus_Forecast_unadjusted", data(imitm,i)-mitmges )
  ...
  subroutine final_vars_
    ...
    if(allocated(ges_mitm)) deallocate(ges_mitm)
