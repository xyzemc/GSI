...
subroutine general_read_gfsatm_ncpar(grd,sp_a,filename,uvflag,vordivflag,zflag, &
           gfs_bundle,init_head,iret_read)
!$$$  subprogram documentation block
!                .      .    .                                       .
! subprogram:    general_read_gfsatm_ncpar  adaptation of read_gfsatm for general resolutions
!                                           uses netCDF parallel to read in one
!                                           level per PE
!   prgmmr: martin          org: NCEP/EMC                date: 2020-02-21
!
! abstract: copied from read_gfsatm_nc
!
! program history log:
!   2020-02-21  martin     - original; copied from general_read_gfsatm_nc
!
!   input argument list:
!     grd      - structure variable containing information about grid
!                    (initialized by general_sub2grid_create_info, located in general_sub2grid_mod.f90)
...
!$$$
   ...
   ! Declare local variables
   character(len=120) :: my_name = 'GENERAL_READ_GFSATM_NCPAR'
   character(len=1)   :: null = ' '
   integer(i_kind):: iret,nlatm2,nlevs,icm,nord_int
   ...
   !******************************************************************************
   if ( mype == 0) then
      print *, 'ERROR: GSI attempting to read netCDF file in parallel'
      print *, '       but parallell netCDF not included in build'
      print *, '       set parallel_ncio = .false. in &setup'
      stop(999)
   end if
   ...
   if ( iredundant==2 ) then
      if ( mype == 0 ) then
         write(6,*) 'general_read_gfsatm_ncpar: ERROR'
         write(6,*) 'cannot handle having both sf and div'
         write(6,*) 'Aborting ... '
      endif
      call stop2(999)
   ...
   if ( mype == 0 ) then
      write(6,700) lonb,latb,nlevs,grd%nlon,nlatm2,&
            fhour,odate,trim(filename)
700   format('GENERAL_READ_GFSATM_NCPAR: read lonb,latb,levs=',&
            3i6,', scatter nlon,nlat=',2i6,', hour=',f6.1,', idate=',4i5,1x,a)
   endif

   return
#endif
end subroutine general_read_gfsatm_ncpar
