...
subroutine compute_xy_orig(x_disp, y_disp, ix_orig, iy_orig, dx_n, dy_n, meta, order)
  ! purpose:
  ! apply displacements to a gridded 2d field
  ! input: original field in bg, displacements in disp
  ! output: displaced field in adj
...
subroutine exchange_needed(needed_ij_in, needed_wgts_in, num_needed_in, &
  ...
  ! This routine exchanges information about displacement vectors (starting and
...
subroutine exchange_interpolated(needed_adj_out, needed_adj_in, index_in)
  ...
  ! This routine exchanges interpolated values at the displacement vectors
