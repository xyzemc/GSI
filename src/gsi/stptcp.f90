...
!           .      .    .                                       .
! module:   stptcpmod    module for stptcp and stptcp_search
!  prgmmr:
!
! abstract: module for stptcp and stptcp_search
!
! program history log:
...
!   2019-08-26  kbathmann - split into stptcp and stptcp_search

! subroutines included:
!   sub stptcp_search,stptcp
!
! attributes:
!   language: f90
!   machine:
...
PRIVATE
PUBLIC stptcp_search,stptcp

contains
subroutine stptcp_search(tcphead,rval,out,sges,nstep)
!$$$  subprogram documentation block
!                .      .    .                                       .
! subprogram: stptcp_search calculate search direction, penalty and contribution to
!                           stepsize for synthetic tc-mslp obs
!
!   prgmmr: kleist            org: np23                date: 2009-02-02
!
! abstract: calculate search direction penalty and contribution to stepsize for
...
! Simply return if any pointer not found
  ...
  do while (associated(tcpptr))
     if(tcpptr%luse)then
        ...
        w3 = tcpptr%wij(3)
        w4 = tcpptr%wij(4)
        tcpptr%val =w1* rp(j1)+w2* rp(j2)+w3* rp(j3)+w4* rp(j4)
       
        do kk=1,nstep
           ps=tcpptr%val2+sges(kk)*tcpptr%val
...
!  Modify penalty term if nonlinear QC
  ...
  end do !while associated(tcpptr)
  
  return
end subroutine stptcp_search

subroutine stptcp(tcphead,out,sges,nstep)
!$$$  subprogram documentation block
!                .      .    .                                       .
...
!
!   output argument list:
!     out(1:nstep)   - contribution to penalty for surface pressure -
!     sges(1:nstep)
!
! attributes:
!   language: f90
