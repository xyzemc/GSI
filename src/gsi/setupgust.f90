...
! Declare local variables
  ...
  logical:: in_curbin, in_anybin, sfc_data
...
!    Process obs with reported height differently than obs those reported with pressure.
!    This includes most surface data assimilated in RTMA/URMA.

     sfc_data = ((itype >= 280 .and. itype <= 299).or.(itype>=180 .and. itype<=199))

!    Process observations with reported height
!    It should be noted that in 2dvar, most surface stations will report height
     drpx = zero
     dpres = data(ihgt,i)
     dstn = data(istnelv,i)
...
!    units.  Save the observation height in zob for later use.
     ...
     if (zob <= zero) then
        print*, "GUST WARNING: Negative zob reported for station,zob,type (correcting to 10 m):",station_id,zob,itype
        zob=ten
     endif
...
!          call nc_diag_metadata("Nonlinear_QC_Var_Jb",     var_jb                 )
           ...
           if (lobsdiagsave) then
              ...
              call nc_diag_data2d("ObsDiagSave_nldepart", obsdiags(i_gust_ob_type,ibin)%tail%nldepart )
              call nc_diag_data2d("ObsDiagSave_tldepart", obsdiags(i_gust_ob_type,ibin)%tail%tldepart )
              call nc_diag_data2d("ObsDiagSave_obssen",   obsdiags(i_gust_ob_type,ibin)%tail%obssen   )             
           endif
