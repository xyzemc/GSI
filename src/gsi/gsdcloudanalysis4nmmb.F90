...
!    2011-05-29  Todling - extra cloud-guess from metguess_bundle (see Remark 1)
...
!
!
  itsig=1 ! shouldn't this be ntguessig?
  itsfc=1 ! shouldn't this be ntguessig?
!
...
! write(6,*)'output bundle gsd::',gsi_metguess_bundle(itsig)%r3(8)%shortname
  end if
...
!    do ista=1,numsao
!       iob = min(max(int(oistation(ista)+0.5_r_single),1),im)
!       job = min(max(int(ojstation(ista)+0.5_r_single),1),jm)
...
!       soiltbk(i,j)=0.0_r_single
!       xlon(i,j)   =0.0_r_single
!       xlat(i,j)   =0.0_r_single
...
!          r_cleansnow_warmts_threshold = 4.0_r_kind    !to delete
              ...
              !* using obsref -99 value 
                 if((iclean_hydro_withref==1)) then
                    if( iclean_hydro_withref_allcol==1 .and. &
                       ...
                       (sat_ctp(i,j) >=1010.0_r_single .and. sat_ctp(i,j) <1050._r_single)) then     
...
!          t_inc=min(t_bk(i,j,k)/((100.0_r_kind/ges_prsl(j,i,k,1))**rd_over_cp)-ges_tsen(j,i,k,1),0.3_r_kind)
...
#else /* Start no nmmb cloud analysis library block */
subroutine  gsdcloudanalysis4nmmb(mype)
...
#endif /* End no nmmb cloud analysis library block */
