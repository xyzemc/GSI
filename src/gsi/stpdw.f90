...
!            .      .    .                                       .
! module:    stpdwmod    module for stpdw_search and stpdw
!  prgmmr:
!
! abstract: module for stpdw_search and stpdw
!
! program history log:
...
!   2019-08-21  kbathmann - split into stpdw and stpdw_search
!
! subroutines included:
!   sub stpdw,stpdw_search
!
! attributes:
!   language: f90
!   machine:
...
PRIVATE
PUBLIC stpdw,stpdw_search

contains

subroutine stpdw_search(dwhead,rval,out,sges,nstep)
!$$$  subprogram documentation block
!                .      .    .                                       .
! subprogram:    stpdw_search  compute the value of the current search direction
!                              in obs space, as well as the contribution to
!                              penalty, with nonlinear qc
!   prgmmr: derber           org: np23                date: 1991-02-26
!
! abstract: calculate search direction, contribution to penalty and stepsize from lidar winds
!
! program history log:
...
! Simply return if any pointer not found
  ...
  do while (associated(dwptr))
     if(dwptr%luse)then
        ...
        dwptr%val=(w1* ru(j1)+w2* ru(j2)+w3* ru(j3)+w4* ru(j4)+&
        ...
        do kk=1,nstep
           dw=dwptr%val2+sges(kk)*dwptr%val
...
!       Modify penalty term if nonlinear QC
  ...
  return
end subroutine stpdw_search

subroutine stpdw(dwhead,out,sges,nstep)
!$$$  subprogram documentation block
!                .      .    .                                       .
