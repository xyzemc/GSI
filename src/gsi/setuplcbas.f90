module lcbas_setup
  ...
  private
  public:: setup
        interface setup; module procedure setuplcbas; end interface

contains
subroutine setuplcbas(obsLL,odiagLL,lunin,mype,bwork,awork,nele,nobs,is,conv_diagsave)
...
!   2020-01-23  guo     - removed lcbashead alias.
...
!    in inner loop minimization (int* and stp* routines)
     if (.not. last .and. muse(i)) then
        ...
        allocate(my_head)
        call lcbasNode_appendto(my_head,obsLL(ibin))
...
!       Set (i,j) indices of guess gridpoint that bound obs location
        ...
        if (jiter==jiterstart .and. R_option) then
           ...
           wgt_lcbas(jlat+1,jlon)  =wgt_lcbas(jlat+1,jlon)+my_head%wij(2)
           wgt_lcbas(jlat,jlon+1)  =wgt_lcbas(jlat,jlon+1)+my_head%wij(3)
           wgt_lcbas(jlat+1,jlon+1)=wgt_lcbas(jlat+1,jlon+1)+my_head%wij(4)
        end if
...
900  format('conv_lcbas_',i2.2,'.nc4')
  ...
  type(obs_diag),pointer,intent(in):: odiag
! Observation class
  character(7),parameter     :: obsclass = '  lcbas'
...
!          call nc_diag_metadata("Nonlinear_QC_Var_Jb",     var_jb                 )
           ...
           call nc_diag_metadata("Observation",                   data(ilcbas,i)    )
           call nc_diag_metadata("Obs_Minus_Forecast_adjusted",   ddiff             )
           call nc_diag_metadata("Obs_Minus_Forecast_unadjusted", data(ilcbas,i)-lcbasges )
...
!_RT_NC4_TODO: is the following name ok?
           if (abs(data(iceil,i)-miss_obs)<100.0_r_kind)  then
              call nc_diag_metadata("Cloud_Ceiling",  data(iceil,i)                )
           else
              call nc_diag_metadata("Cloud_Ceiling",  missing                      )
           end if
           r_prvstg            = data(iprvd,i)
