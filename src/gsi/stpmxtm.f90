...
!           .      .    .                                       .
! module:   stpmxtmmod    module for stpmxtm_search and stpmxtm
!  prgmmr:
!
! abstract: module for stpmxtm_search and stpmxtm
!
! program history log:
!   2014-04-10  pondeca
!   2015-07-10  pondeca  - force return if no mxtm data available
!   2016-05-18  guo     - replaced ob_type with polymorphic obsNode through type casting
!   2019-08-26  kbathmann - split into stpmxtm and stpmxtm_search
!
! subroutines included:
!   sub stpmxtm_search,stpmxtm
!
! attributes:
!   language: f90
!   machine:
...
PRIVATE
PUBLIC stpmxtm_search,stpmxtm

contains

subroutine stpmxtm_search(mxtmhead,rval,out,sges,nstep)
!$$$  subprogram documentation block
!                .      .    .                                       .
! subprogram: stpmxtm_search  calculate search direction, penalty and contribution to stepsize
!
! abstract: calculate search direction, penalty and contribution to stepsize for daily maximum
...
! Simply return if any pointer not found
  ...
  do while (associated(mxtmptr))
     if(mxtmptr%luse)then
        ...
        mxtmptr%val =w1*rmxtm(j1)+w2*rmxtm(j2)+w3*rmxtm(j3)+w4*rmxtm(j4)

        do kk=1,nstep
           mxtm=mxtmptr%val2+sges(kk)*mxtmptr%val
...
!  Modify penalty term if nonlinear QC
  ...
  end do !while associated(mxtmptr)
  
  return
end subroutine stpmxtm_search

subroutine stpmxtm(mxtmhead,out,sges,nstep)
!$$$  subprogram documentation block
!                .      .    .                                       .
! subprogram:    stpmxtm      calculate penalty and contribution to stepsize
!
! abstract: calculate penalty and contribution to stepsize for daily maximum
! temperature
...
!
!   output argument list:
!     out(1:nstep)   - contribution to penalty for conventional mxtm -
!     sges(1:nstep)
!
! attributes:
!   language: f90
