...
!   17Dec2019 W. Gu/J. Guo - hide GMAO_ObsErrorCov format switch through a retry of header-read
!
!EOP
!-------------------------------------------------------------------------
!BOC
...
logical :: corr_obs
   ...
   if (corr_obs) then
      ...
      if (ioflag/=0) then
         if(VERBOSE_) call tell(myname_,'retry "read() nch_active ..." as a 2-parameter record, fname =',trim(fname))
         backspace(lu,iostat=ioflag)    ! any bad backspace() would be ignored
   ...
   do isurf=1,5
      ...
      if (nsat>0) then
         ...
         do jj0=1,nsat
            ...
            if(GSI_BundleErrorCov(itbl)%nctot<0) then   ! no need to check on %nctot against nchanl
               do jj=1,nch_active
...
! used in estimating the observation error covariance
   ...
   do jj=1,nchanl
     ...
     if (varinv(jj)>tiny_r_kind .and. iuse_rad(mm)>=1) then
       ifound=-1
       do ii=1,nch_active
          if(ErrorCov%nctot<0) then     ! no need to check on %nctot against nchanl
