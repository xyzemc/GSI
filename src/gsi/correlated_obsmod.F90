...
interface corr_oberr_qc; module procedure adjust_var_; end interface
...
!   28Oct2016 W. Gu, interface corr_oberr_qc was refered to new subroutine adjust_var_, so that the prescribed obs errors in satinfo
!                    can be replaced profile by profile with their corresponding correlated errors. subroutine upd_varqc_ was renamed
!                    to upd_varch_ and refered by update_varch.
...
subroutine adjust_var_ (iinstr,nindx,nchanl,ich,tnoise)
!$$$  subprogram documentation block
!                .      .    .
! subprogram:    adjust_var_
!
!   prgrmmr:     W. Gu  org: gmao                date: 2014-09-15
!
! abstract:  provide hook to update tnoise 
!
! program history log:
!   2016-09-15  W. Gu   - initial code
!
! attributes:
!   language: f90
...
!$$$ end documentation block
   ...
   use radinfo, only: iuse_rad,jpch_rad
   use mpeu_util, only: die
   use mpimod, only: mype
   implicit none
   integer(i_kind), intent(in) :: iinstr
   integer(i_kind), intent(in) :: nindx
   integer(i_kind), intent(in) :: nchanl
   integer(i_kind), intent(in) :: ich(nchanl)
   real(r_kind), intent(inout) :: tnoise(nchanl)

   character(len=*),parameter::myname_ = myname//'*adjust_var_'

   integer(i_kind) :: nch_active,ii,jj,mm,nn
   ...
   if( GSI_BundleErrorCov(iinstr)%method==2 ) then
!     if(nindx==1)write(6,'(1x,a,a20,1x,i4)')'>>>>> Obserrors in satinfo replaced by diag(R) ',idnames(iinstr),mype
     do jj=1,nch_active
       nn=GSI_BundleErrorCov(iinstr)%indxR(jj)
       mm=ich(nn)
       if( tnoise(nn)> 1.e4_r_kind .or. iuse_rad(mm)<1 ) then
         call die(myname_,' active channels used in R do not match those used in GSI, aborting')
       endif
       tnoise(nn) = sqrt(GSI_BundleErrorCov(iinstr)%R(jj,jj))
     enddo
   endif

end subroutine adjust_var_
!EOC
!-------------------------------------------------------------------------
!BOP
!
! !IROUTINE:  upd_varch_ ---  overwrite prescribed obs error (satinfo) for instrument treated as correlated
!
! !INTERFACE:
!
...
! !USES:
   ...
   use radinfo, only: jpch_rad,iuse_rad,nusis,varch
   use radinfo, only: varch_sea,varch_land,varch_ice,varch_snow,varch_mixed
implicit none
! !INPUT PARAMETERS:
! !DESCRIPTION: This routine replaces the prescribed obs errors in satinfo for instruments we account
...
!BOC
   ...
   character(len=*),parameter :: myname_=myname//'*upd_varch'
   character(len=80) covtype
   integer(i_kind) :: nch_active,ii,jj,mm,nn,jj0,itbl,nsatype,ntrow,iinstr
   ...
   do jj0=1,nsatype
     ...
     if( GSI_BundleErrorCov(itbl)%method==2 ) then
       do jj=1,nch_active
         ...
         varch(mm)=sqrt(GSI_BundleErrorCov(itbl)%R(jj,jj))
       enddo
     endif
...
! has reached.  
! 
! Moved here from radinfo
!
!!??     if (miter>0) call corr_obserr_qc(jpch_rad,iuse_rad,nusis,varch_sea, &

  call upd_varqc_(jpch_rad,iuse_rad,nusis,varch_sea, &
...
!BOC
   ...
   real(r_quad),   allocatable,dimension(:)   :: qcaj
   real(r_quad) rsqrtinv_quad,rinvdiag_quad
   real(r_quad) coeff
   logical subset
...
! Multiply departure with "right" eigenvectors
       ...
       do ii=1,ncp
       do jj=1,ncp
         rsqrtinv_quad=zero_quad
         do kk=1,ncp
           coeff = sqrt(qcaj(jj)/ErrorCov%Revals(IRsubset(kk)))
           rsqrtinv_quad=rsqrtinv_quad+ErrorCov%Revecs(IRsubset(ii),IRsubset(kk))*coeff*ErrorCov%Revecs(IRsubset(jj),IRsubset(kk))
         enddo
         rsqrtinv(ii,jj) = rsqrtinv_quad
       enddo
       enddo

       do ii=1,ncp
         rinvdiag_quad = zero_quad
         do kk=1,ncp
           coeff = qcaj(ii)/ErrorCov%Revals(IRsubset(kk))
           rinvdiag_quad=rinvdiag_quad+ErrorCov%Revecs(IRsubset(ii),IRsubset(kk))*coeff*ErrorCov%Revecs(IRsubset(ii),IRsubset(kk))
         enddo
         rinvdiag(ii)=rinvdiag_quad
       enddo

       do ii=1,ncp
         do jj=1,ncp
            nn=IJsubset(jj)
            col(ii)   = col(ii)   + rsqrtinv(ii,jj) * depart(nn)
            row(:,ii) = row(:,ii) + rsqrtinv(ii,jj) * jacobian(:,nn)
         enddo
       enddo
