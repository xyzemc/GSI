...
!   17Dec2019 W. Gu     - hide GMAO_ObsErrorCov format switch through a retry of header-read
!
!EOP
!-------------------------------------------------------------------------
!BOC
...
logical :: corr_obs
   ...
   if (corr_obs) then
      ...
      if (ioflag/=0) then
         backspace(lu)
         if(VERBOSE_) call tell(myname_,'retry as a 2-parameter record format, fname =',trim(fname))
