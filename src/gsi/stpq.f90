...
!           .      .    .                                       .
! module:   stpqmod    module for stpq_search and stpq
!  pgrmmr:
!
! abstract: module for stpq_search and stpq
!
! program history log:
...
!   2019-08-21  kbathmann - split into stpq and stpq_search
!
! subroutines included:
!   sub stpq_search,stqp
!
! attributes:
!   language: f90
!   machine:
...
PRIVATE
PUBLIC stpq,stpq_search

contains

subroutine stpq_search(qhead,rval,out,sges,nstep)
!$$$  subprogram documentation block
!                .      .    .                                       .
! subprogram:    stpq_search compute the value of the current search direction
!                            in obs space, as well as the contribution to
!                            penalty, with nonlinear qc
!   prgmmr: derber           org: np23                date: 1991-02-26
!
! abstract: calculate search direction, penalty and contribution to stepsize from q
!           using nonlinear qc.
!
! program history log:
...
!  If no q data return
  ...
  do while (associated(qptr))
     if(qptr%luse)then
        ...
        qptr%val= w1* rq(j1)+w2* rq(j2)+w3* rq(j3)+w4* rq(j4)+ &
             w5* rq(j5)+w6* rq(j6)+w7* rq(j7)+w8* rq(j8)
        do kk=1,nstep
           qq=qptr%val2+sges(kk)*qptr%val
...
!  Modify penalty term if nonlinear QC
  ...
  end do !while associated(qptr)

  return
end subroutine stpq_search
subroutine stpq(qhead,out,sges,nstep)
!$$$  subprogram documentation block
!                .      .    .                                       .
...
! Declare local variables
  integer(i_kind) kk
  real(r_kind) cg_q,val,val2,wgross,wnotgross,q_pg,qq
  real(r_kind),dimension(max(1,nstep))::pen
  type(qNode), pointer :: qptr
