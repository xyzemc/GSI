...
!           .      .    .                                       .
! module:   stprwmod    module for stprw_search and stprw
!  prgmmr:
!
! abstract: module for stprw_search and stprw
!
! program history log:
...
!   2019-08-21  kbathmann - split into stprw and stprw_search
!
! subroutines included:
!   sub stprw,stprw_search
!
! attributes:
!   language: f90
!   machine:
...
implicit none
PRIVATE
PUBLIC stprw,stprw_search

contains

subroutine stprw_search(rwhead,rval,out,sges,nstep)
!$$$  subprogram documentation block
!                .      .    .                                       .
! subprogram:    stprw_search   compute the value of the current search direction 
...
!       Gradient
        rwptr%val=(w1*ru(j1)+ w2*ru(j2)+ w3*ru(j3)+ &
        ...
        if(include_w) then
           rwptr%val=rwptr%val+(w1*rw(j1)+ w2*rw(j2)+ w3*rw(j3)+ &
                        w4*rw(j4)+ w5*rw(j5)+ w6*rw(j6)+ &
                        w7*rw(j7)+w8*rw(j8))*rwptr%sintilt
        end if

        do kk=1,nstep
           r=rwptr%val2+sges(kk)*rwptr%val
...
!  Modify penalty term if nonlinear QC
  ...
  return
end subroutine stprw_search
subroutine stprw(rwhead,out,sges,nstep)
!$$$  subprogram documentation block
!                .      .    .                                       .
