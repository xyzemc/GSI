...
!           .      .    .                                       .
! module:   stplightmod    module for stplight and stplight_search
!  prgmmr: k apodaca <karina.apodaca@colostate.edu>
!      org: CSU/CIRA, Data Assimilation Group
!     date: 2016-05-19
!
! abstract: module for calculating  stplight and stplight_search
!
! program history log:
!   2016-05-19  apodaca - original version
!   2019-08-23  kbathmann - split into stplight and stplight_search
!
! subroutines included:
!   sub stplight,stplight_search
!
! attributes:
!   language: f90
!   machine:
...
PRIVATE
PUBLIC stplight_search,stplight

contains

subroutine stplight_search(lighthead,rval,out,sges,nstep)
!$$$  subprogram documentation block
!                .      .    .                                       .
! subprogram: stplight_searh  compute search direction, contribution
!                       penalty and stepsize from lightning, using nonlinear qc    
!   prgmmr: apodaca     org: CSU/CIRA           date: 2016-05-19
!
! abstract:  This routine applies the (linear) operator of the 
!            lightning flash rate model and computes a search direction and
!            an estimate of step size as done in steepest descent or conjugate 
...
!  Forward Model
                 ...
                 lightptr%val = lightptr%val + rhtot_TL
...
!  Forward Model
            ...
            lightptr%val = lightptr%val + rflashrate_TL
...
!       penalty and gradient
         ...
         do kk=1,nstep
            nref=lightptr%val2+sges(kk)*lightptr%val
...
!       Cost function
 ...
 return
end subroutine stplight_search
subroutine stplight(lighthead,out,sges,nstep)
!$$$  subprogram documentation block
!                .      .    .                                       .
...
!   2018-03-07  k apodaca - replaced ob_type with polymorphic obsNode through
!   type casting
!   2018-08-27  k apodaca - add TL-related components of a second oservation
!   operator for
!                           lightning observations suitable for non-hydrostatic,
!                           cloud-resolving models  with additional ice-phase
!                           hydrometeor
!                           control variables
!
!   input argument list:
  ...
  do while (associated(lightptr))
     if(lightptr%luse)then
        if(nstep > 0)then
            ...
            pen(1)=lightptr%val2*lightptr%val2*lightptr%err2
