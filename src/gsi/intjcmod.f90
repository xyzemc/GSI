...
!
!$$$
  use gridmod, only: nsig,lat1,lon1,istart,wgtlats,rlats
...
!          Lower constraint limit
           if (q < zero) then
              rq(i,j,k) = rq(i,j,k) + (factqmin*cos(rlats(ii)))*q &
                          /(ges_qsat(i,j,k,itbin)*ges_qsat(i,j,k,itbin))

!          Upper constraint limit
           else if (q > ges_qsat(i,j,k,itbin)) then
              rq(i,j,k) = rq(i,j,k) + (factqmax*cos(rlats(ii)))*(q-ges_qsat(i,j,k,itbin))/ &
                          (ges_qsat(i,j,k,itbin)*ges_qsat(i,j,k,itbin))
