subroutine read_radarref_mosaic2(nread,ndata,nodata,infile,obstype,lunout,twind,sis,nobs)
!$$$  subprogram documentation block
!                .      .    .                                       .
! subprogram:  read_radarref_mosaic2    Reading in reflectivity mosaic in RR grid
!
...
!    2016-09-xx  g.zhao - modified Ming Hu's read_radarref_mosaic for 
!                         reading mosaic ref dbz for GSIv3.5 variational and
!                         enkf analysis, not for gsd-cloud-analysis
!    2016-11-xx  g.zhao - fix the bugs:
...
!
  ...
  use caps_radaruse_mod, only: oe_dbz, refl_lowbnd_dbz
...
! real(r_kind) :: rmins_an,rmins_ob                                                     
  ...
  data hgt_ref / 500.0,   750.0,  1000.0,  1250.0,  1500.0,  1750.0,  2000.0,   &
                2250.0,  2500.0,  2750.0,  3000.0,  3500.0,  4000.0,  4500.0,   &
                5000.0,  5500.0,  6000.0,  6500.0,  7000.0,  7500.0,  8000.0,   &
                8500.0,  9000.0, 10000.0, 11000.0, 12000.0, 13000.0, 14000.0,   &
...
!   INTEGER(i_kind),PARAMETER  ::  nmsgmax=100000   ! from GSIv3.3
!   INTEGER(i_kind),PARAMETER  ::  maxobs=2000000   ! from GSIv3.3
    ...
    character*128   :: myname='read_radarref_mosaic2'
...
!
   ...
   if(dbzob) then
      ...
      write(6,*) ' illegal obs type in read_RadarRef_mosaic2 : obstype=',obstype
      call stop2(94)
   end if
  if(perturb_obs .and. dbzob)nreal=nreal+1
  write(6,*)'read_RadarRef_mosaic2: nreal=',nreal
   ...
   do i=1,nconvtype
       if(trim(obstype) == trim(ioctype(i)) .and. abs(icuse(i))== 1) then
          ...
          write(6,*) 'read_RadarRef_mosaic2: Obs Type for dBZ is not in CONVINFO !'
          write(6,*) 'read_RadarRef_mosaic2: PLEASE modify the CONVINFO file !'
          write(6,*) 'read_RadarRef_mosaic2: abort read_RadarRef_mosaic2 !'
          return
       endif
   end do
 ...
 !! get message and subset counts
      ...
      write(6,*)'read_radarref_mosaic2: nmsgmax=',nmsgmax,'    maxobs=',maxobs
...
!     read in ref obs in bufr code format
      ...
      msg_report: do while (ireadmg(lunin,subset,idate) == 0)
         ...
         if (nmsg>nmsgmax) then
            write(6,*)'read_radarref_mosaic2: messages exceed maximum ',nmsgmax
            call stop2(50)
         endif
         loop_report: do while (ireadsb(lunin) == 0)
            ntb = ntb+1
            if (ntb>maxobs) then
                write(6,*)'read_radarref_mosaic2: reports exceed maximum ',maxobs
                call stop2(50)
            endif
...
! check time window in subset
            if (l4dvar.or.l4densvar) then
               ...
               if (t4dv<zero .OR. t4dv>winlen) then
                  write(6,*)'read_radarref_mosaic2:      time outside window ',&
...
! read in observations
            ...
            if (numlvl .ne. maxlvl) then
                write(6,*)' read_radarref_moaic2: numlvl is not equalt to maxlvl:',numlvl,maxlvl
            end if
...
!               write(6,*) 'read_radarref_mosaic2: single point/column obs run'
                do k=1,numlvl
!                   if (NINT(ref3d_column(1,ntb)) .eq. 213 .and. NINT(ref3d_column(2,ntb)) .eq. 170 .and.  &
                    if (NINT(ref3d_column(1,ntb)) .eq. 175 .and. NINT(ref3d_column(2,ntb)) .eq. 105 .and.  &
                        NINT(hgt_ref(k)) .ge. 100 ) then
!                       (NINT(hgt_ref(k)) .eq. 7500  .or.         &
!                        NINT(hgt_ref(k)) .eq. 8000)               ) then
                        write(6,*) 'read_radarref_mosaic2: single point/column obs run on grid: 175 105'
                        write(6,*) 'read_radarref_mosaic2: found the pseudo single(column) dbz obs:',ref3d_column(1:2,ntb),hgt_ref(k)
                    else
                        obs(1,k) = -999.0       ! reflectivity (column 33 levels)
                    end if
                end do
            end if
...
!               only use dbz obs > refl_lowbnd_dbz ! g.zhao (user-defined in ! namelist)
      ...
      write(6,*)'read_radarref_mosaic2: messages/reports = ',nmsg,'/',ntb
      ! numref is not obs number, it is obs report for each vertical profile
      numref=ntb
      write(6,*)'read_RadarRef_mosaic2: total No. of dBZ>0.0 = ',ndata
      nread=ndata
      nodata=ndata
...
!
      ...
      DO i=1,numref
        ...
        enddo
      enddo
      if(ndata .NE. ndata2) then
         write(6,*)'read_RadarRef_mosaic2: Error in counting dBZ>0.0dbz --> ndata  ndata2:',ndata,ndata2
         call stop2(111)
      end if
      ...
      nread=ndata
      nodata=ndata
!     nread=numref   ! used in Ming Hu's read_radarref_mosaic, but wrong here!
!     ndata=numref   ! used in Ming Hu's read_radarref_mosaic, but wrong here!
!     nreal=maxlvl+2 ! used in Ming Hu's read_radarref_mosaic, but wrong here!
!     if(numref > 0 ) then
      if(ndata > 0 ) then
         if(nlon==nlon_regional .and. nlat==nlat_regional) then
!!!         call count_obs(numref,maxlvl+2,ilat,ilon,ref3d_column,nobs)  ! out-of-date and only for GSIv3.3
...
!           the binary file through lunout is to be read in subroutine disobs
            ...
            write(6,*)'read_radarref_mosaic2: npe  nobs=',npe, nobs(1:npe), &
                ' total radarref_mosaic2=',sum(nobs(1:npe)),'   ndata=',ndata

         else
            write(6,*)' read_rdarRef_mosaic2: nlon /= nlon_regional and/or nlat/=nlat_regional:', &
                      nlon,nlat,'regional:',nlon_regional,nlat_regional
            write(6,*)' read_RadarRef_mosaic2:  abort and stop'
            call stop2(111)
...
!           write(lunout) ((ref3d_column(k,i),k=1,maxlvl+2),i=1,numobsa)
      ...
      write(6,'(1x,A,F12.5,1x,A,3(1x,F8.3),1x,I4)') &
          'read_RadarRef_mosaic2: max dbz =',dbz_max, '@ i j k =', &
...
200 continue
    write(6,*) 'read_radarref_mosaic2, Warning : cannot find or open bufr radar dbz data file: ', trim(infile)

end subroutine read_radarref_mosaic2
!
!
