...
!           .      .    .                                       .
! module:   stpcldchmod    module for stpcldch_search and stpcldch
!  prgmmr:
!
! abstract: module for stpcldch_search and stpcldch
!
! program history log:
!   2015-07-10  pondeca
!   2016-05-18  guo     - replaced ob_type with polymorphic obsNode through type casting
!   2019-08-27  kbathmann - split into stpcldch and stpcldch_search
!
! subroutines included:
!   sub stpcldch_search,stpcldch
!
! attributes:
!   language: f90
!   machine:
...
PRIVATE
PUBLIC stpcldch_search,stpcldch

contains

subroutine stpcldch_search(cldchhead,rval,out,sges,nstep)
!$$$  subprogram documentation block
!                .      .    .                                       .
! subprogram: stpcldch_search  calculate search direction, penalty and contribution to stepsize
!   prgmmr: derber           org: np23                date: 2004-07-20
!
! abstract: calculate search direction, penalty and contribution to stepsize for cloud ceiling 
!           height with addition of nonlinear qc
!
! program history log:
!   2019-08-27  pondeca  - split computation of val into its own subroutine
!
!   input argument list:
!     cldchhead
...
! Simply return if any pointer not found
  ...
  do while (associated(cldchptr))
     if(cldchptr%luse)then
        ...
        cldchptr%val =w1*rcldch(j1)+w2*rcldch(j2)+w3*rcldch(j3)+w4*rcldch(j4)

        do kk=1,nstep
           cldch=cldchptr%val2+sges(kk)*cldchptr%val
...
!  Modify penalty term if nonlinear QC
  ...
  end do !while associated(cldchptr)
  
  return
end subroutine stpcldch_search

subroutine stpcldch(cldchhead,out,sges,nstep)
!$$$  subprogram documentation block
!                .      .    .                                       .
! subprogram:    stpcldch      calculate penalty and contribution to stepsize
!   prgmmr: derber           org: np23                date: 2004-07-20
!
! abstract: calculate penalty and contribution to stepsize for cloud ceiling
! height
...
!
!   output argument list:
!     out(1:nstep)   - contribution to penalty for conventional cldch -
!     sges(1:nstep)
!
! attributes:
!   language: f90
