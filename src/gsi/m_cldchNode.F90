...
!                .      .    .                                       .
! subprogram:	 module m_cldchNode
!   prgmmr:	 j guo <jguo@nasa.gov>
!      org:	 NASA/GSFC, Global Modeling and Assimilation Office, 610.3
!     date:	 2016-05-18
!
! abstract: class-module of obs-type cldchNode (surface cldch(ibility))
!
! program history log:
...
! module interface:
  ...
  type,extends(obsNode):: cldchNode
  ...
  public:: cldchNode_typecast
  public:: cldchNode_nextcast
        interface cldchNode_typecast; module procedure typecast_ ; end interface
        interface cldchNode_nextcast; module procedure nextcast_ ; end interface
...
!-- cast a class(obsNode) to a type(cldchNode)
  use m_obsNode, only: obsNode
  implicit none
  type(cldchNode),pointer:: ptr_
...
!-- cast an obsNode_next(obsNode) to a type(cldchNode)
...
subroutine obsNode_xread_(aNode,iunit,istat,diagLookup,skip)
  ...
  implicit none
  class(cldchNode), intent(inout):: aNode
...
subroutine obsNode_xwrite_(aNode,junit,jstat)
  implicit none
  class(cldchNode),intent(in):: aNode
