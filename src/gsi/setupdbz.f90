...
!   2016-06-25 Thomas Jones	- Add Thompson reflectivity calculuation option and other changes
!
!
!   input argument list:
...
!$$$
  ...
  use obsmod, only: rmiss_single,i_dbz_ob_type,obsdiags, &
                    lobsdiagsave,nobskeep,lobsdiag_allocated,time_offset,ens_hx_dbz_cut
  ...
  use guess_grids, only: hrdifsig,geop_hgtl,nfldsig,&
       ges_lnprsl,ges_rho,ges_tsen, ges_prsl
  ...
  use setupdbz_lib, only:hx_wsm6,hx_thomp,jqr_dart,jqs_dart,jqg_dart
...
! Declare local variables
  ...
  real(r_kind) dz,denom,jqr_num,jqs_num,jqi_num,jqg_num,jqr,jqs,jqi,jqg
  ...
  real(r_kind) dbzgesin,qrgesin,qsgesin,qigesin,qggesin,qcgesin,rhogesin,presgesin,tempgesin,qligesin,nrgesin,qvgesin
  real(r_kind) qrgesin1,qsgesin1,qggesin1,qligesin1,qcgesin1,nrgesin1
  real(r_kind) Zer,Zeli,Ze,rdBZ,vt_dBZ,presw,dbznoise,dbznoise_runits
  ...
  !real(r_kind),allocatable,dimension(:,:,:,: ) :: ges_qni
  !real(r_kind),allocatable,dimension(:,:,:,: ) :: ges_qnc
...
!    Link obs to diagnostics structure
     if(luse_obsdiag)then
      ...
      if (.not.lobsdiag_allocated) then      
        ...
        if (obsdiags(i_dbz_ob_type,ibin)%tail%indxglb/=i) then
...
!     if(dpres > 12000) cycle !don't need obs above 12 km
...
!    Compute observation pressure (only used for diagnostics)
     ...
     presw  = ten*exp(pobl)
     if( (k1 == k2) .and. (k1 == 1) ) presw=ten*exp(prsltmp(k1)) !solution to Nan in some members only
                                                                     ! for EnKF which causes problem?
...
!    Observation error currently assumed from convinfo file and is *not* adjusted
     ratio_errors = error/(abs(data(ier,i) + 1.0e6_r_kind*rhgh + r8*rlow))
     ...
     !######### SET REFLECTIVITY ASSIMILATION OPTION #######
     refltype = 5    !5=USE DBZ FROM STATE,   6=USE DBZ FROM WSM6 MP,    8=USE DBZ FORM THOMPSON MP
                     !    External		Internal			Internal


     ! IF dbz_exist = true THEN INTERPOLATE FROM MODEL REFLECTIVITY (external method)
     if( dbz_exist .and. refltype == 5 ) then
       ! Interpolate guess reflectivity to observation location and time.
       call tintrp31(ges_dbz,dbzgesin,dlat,dlon,dpres,dtime,&
          hrdifsig,mype,nfldsig)
       rdBZ = dbzgesin
       !print*, "USING STATE REFL ", rdBZ, dlat,dlon,dpres
     endif

     if (refltype == 6 ) then
     ! INTERPOLTE HYDROMEREOR VARIABLES TO OBSERVATION LOCATION THEN CALC REFLECTIVITY (internal method)
!    Interpolate guess qr, qs, qg, and rho to observation location and time.
     ...
     if (refltype == 8 ) then
       ...
       call tintrp31(ges_qr,qrgesin,dlat,dlon,dpres,dtime,& !modified
            hrdifsig,mype,nfldsig)
       call tintrp31(ges_qc,qcgesin,dlat,dlon,dpres,dtime,& !modified
            hrdifsig,mype,nfldsig)
       call tintrp31(ges_qs,qsgesin,dlat,dlon,dpres,dtime,&
            hrdifsig,mype,nfldsig)
       call tintrp31(ges_qg,qggesin,dlat,dlon,dpres,dtime,&
       ...
       !print*, "USING THOMPSON FO ", nrgesin
   ...
   ! APPLY MIN-MAX THRESHOLDS TO DBZ
   if(miter == 0.or.l_hyb_ens) then !ie an enkf run
     if(rDBZ < 0.0) rDBZ=0.0 ! should be the same as in the read_dbz when nopcp=.true.
   endif
   if(miter == 0.and.ens_hx_dbz_cut) then !ie an enkf run
     if(rDBZ > 70.0) rDBZ=70.0
   endif

   ! ==== These adjoint will not be used in EnKF.
   ! ===
!   call jqr_dart(qrgesin,qsgesin,qggesin,rhogesin,tempgesin,jqr)
!   call jqs_dart(qrgesin,qsgesin,qggesin,rhogesin,tempgesin,jqs)
!   call jqg_dart(qrgesin,qsgesin,qggesin,rhogesin,tempgesin,jqg)
...
!     endif !oneob
     ...
     if(rdBZ <= 0) irefsmlobs=irefsmlobs+1
...
!       statistics as a function of observation type.
     ...
     if(luse_obsdiag)then
         obsdiags(i_dbz_ob_type,ibin)%tail%luse=luse(i)
...
!       Set (i,j,k) indices of guess gridpoint that bound obs location
        ...
        !my_head%jqr     = jqr                 ! for TL and ADJ !modified:
...
!    Save select output for diagnostic file
    if(conv_diagsave .and. luse(i) )then
        ...
        if (errinv_adjst .gt. 3.0*err_input) errinv_adjst = 3.0*err_input
        if (errinv_final .gt. 3.0*err_input) errinv_final = 3.0*err_input
...
!    get qc ...
     varname='ql'
     call gsi_bundlegetpointer(gsi_metguess_bundle(1),trim(varname),rank3,istatus)
     !WRITE(*,*) '*** MAX - MIN QC ***', MAXVAL(rank3),MINVAL(rank3)
     if (istatus==0) then
...
!    get qnr ...
     varname='qnr'
     call gsi_bundlegetpointer(gsi_metguess_bundle(1),trim(varname),rank3,istatus)
     !WRITE(*,*) '*** MAX - MIN QNR ***', MAXVAL(rank3),MINVAL(rank3)
     if (istatus==0) then
...
!    get dbz ...
     varname='dbz'
     call gsi_bundlegetpointer(gsi_metguess_bundle(1),trim(varname),rank3,istatus)
     !WRITE(*,*) '*** MAX - MIN DBZ ***', MAXVAL(rank3),MINVAL(rank3)
     if (istatus==0) then
...
900  format('conv_dbz_',i2.2,'.nc4')
  ...
  subroutine contents_binary_diag_
        ...
        rdiagbuf(16,ii) = errinv_input         ! final inverse observation error (dBZ)**-1
