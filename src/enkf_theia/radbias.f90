...
USE enkf_obsmod, ONLY:  ensmean_ob, ensmean_obnobc, biaspreds, nobs_conv, nobs_oz,nobs_aod,&
  ...
  do i=i1,i2
   ...
   do n=1,npred
      ...
      do m=1,nobs_sat
          ...
          if (indxsat(m) == i) then
             nn = nn + 1
             biaspredtmp(n,nn) = biaspreds(n,m)/SQRT(oberrvar(nobs_conv+nobs_oz+nobs_aod+m))
          end if
      enddo
   ...
   do n=1,npred 
      ...
      do m=1,nobs_sat
          if (indxsat(m) == i) then
             nn = nn + 1
             biaspredtmp(n,nn) = biaspreds(n,m)/oberrvar(nobs_conv+nobs_oz+nobs_aod+m)
          end if
      enddo
   enddo
   nn = 0
   do m=1,nobs_sat
      if (indxsat(m) == i) then
         nn = nn + 1
         obinc(nn) = obfit_post(nobs_conv+nobs_oz+nobs_aod+m)
      end if
   enddo
