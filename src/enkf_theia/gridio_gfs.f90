...
!$$$
 ...
 subroutine readgriddata(nanal,vars3d,vars2d,n3d,n2d,levels,ndim,ntimes,fileprefixes,reducedgrid,grdin,qsat)
  ...
  DO i=1,ntracers_gocart
     ...
     IF (nproc == 0) PRINT *,vars3d_supported_aero(i),aero_ind(i)
  ENDDO
...
!  endif
  ...
  if (use_gfs_nemsio) then
     ...
     do k=1,nlevs
        ...
        DO i=1,ntracers_gocart
           IF (aero_ind(i) > 0) THEN
              CALL nemsio_readrecv(gfile,vars3d_supported_aero(i),'mid layer',k,nems_wrk2,iret=iret)
              IF (iret/=0) THEN
                 WRITE(6,*)'gridio/readgriddata: gfs model: problem with nemsio_readrecv, iret=',vars3d_supported_aero(i),iret
                 CALL stop2(23)
              ENDIF
              IF (cliptracers)  WHERE (nems_wrk2 < clip) nems_wrk2 = clip
              ug = nems_wrk2
              CALL copytogrdin(ug,grdin(:,levels(aero_ind(i)-1)+k,nb))
           ENDIF
        ENDDO
...
!no need to aero_ind since spectral not implemented anyway
     do k=1,nlevs
...
!$omp end parallel do
     ...
     do k=1,nlevs
        ...
        DO i=1,ntracers_gocart
           CALL nemsio_readrecv(gfilein,vars3d_supported_aero(i),'mid layer',k,nems_wrk,iret=iret)
           IF (iret/=0) THEN
              WRITE(6,*)'gridio/writegriddata: gfs model: problem with nemsio_readrecv, iret=',vars3d_supported_aero(i),iret
              CALL stop2(23)
           ENDIF

           ug = 0.
           IF (aero_ind(i) > 0) THEN
              CALL copyfromgrdin(grdin(:,levels(aero_ind(i)-1)+k,nb),ug)
           ENDIF
           nems_wrk = nems_wrk + ug
           IF (cliptracers)  WHERE (nems_wrk < clip) nems_wrk = clip
           CALL nemsio_writerecv(gfileout,vars3d_supported_aero(i),'mid layer',k,nems_wrk,iret=iret)
           IF (iret/=0) THEN
              WRITE(6,*)'gridio/writegriddata: gfs model: problem with nemsio_writerecv, iret=',vars3d_supported_aero(i),iret
              CALL stop2(23)
           ENDIF
