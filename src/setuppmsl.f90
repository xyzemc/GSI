...
use abstract_setup_mod
  ...
  contains
    procedure, pass(this) :: setupDerived => setuppmsl
  end type setuppmsl_class
  interface setuppmsl_class
     module procedure setup_ctor
  end interface
contains
  type(setuppmsl_class) function setup_ctor(obsname,varname1,varname2,varname3)
  ...
  subroutine setuppmsl(this,lunin,mype,bwork,awork,nele,nobs,is,conv_diagsave,luse,data)
...
!   2016-10-07  pondeca - if(.not.proceed) advance through input file first
  ...
  !    Link obs to diagnostics structure
       if(luse_obsdiag)then
          if (.not.lobsdiag_allocated) then
           ...
           if (.not.associated(obsdiags(i_pmsl_ob_type,ibin)%tail)) then
              call die(this%myname,'.not.associated(obsdiags(i_pmsl_ob_type,ibin)%tail)')
           end if
  ...
  ! Interpolate guess pmsl to observation location and time
       call tintrp2a11(this%ges_pmsl,pmslges,dlat,dlon,dtime,hrdifsig,&
          mype,nfldsig)
  ...
  ! Write information to diagnostic file
    if(conv_diagsave .and. ii>0)then
       call dtime_show(this%myname,'diagsave:pmsl',i_pmsl_ob_type)
