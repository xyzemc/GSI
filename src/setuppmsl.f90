...
!
! abstract: For pmsl observations
...
!$$$
  ...
  use constants, only: zero,huge_r_kind,tiny_r_kind,one,half,one_tenth,r10,r1000,wgtlim, &
...
! Declare local parameters
  ...
  character(len=*),parameter:: myname='setuppmsl'
...
! Declare local variables
  ...
  real(r_kind) cg_pmsl,wgross,wnotgross,wgt,arg,exp_arg,rat_err2,qcgross
...
!  index information for data array (see reading routine)
  ...
  ipmsl=5     ! index of pmsl observation
  ihgt=6      ! index of surface height
  itemp=7     ! index of surface temperature observation
     ...
     if(luse_obsdiag)then
        if (.not.lobsdiag_allocated) then
           if (.not.associated(obsdiags(i_pmsl_ob_type,ibin)%head)) then
              ...
              allocate(obsdiags(i_pmsl_ob_type,ibin)%tail%next,stat=istat)
              if (istat/=0) then
                 write(6,*)'setuppmsl: failure to allocate obsdiags',istat
                 call stop2(295)
              end if
           ...
           obsdiags(i_pmsl_ob_type,ibin)%tail%wgtjo=-huge(zero)
           obsdiags(i_pmsl_ob_type,ibin)%tail%obssen(:)=zero
...
! Interpolate guess pmsl to observation location and time
     call tintrp2a11(ges_pmsl,pmslges,dlat,dlon,dtime,hrdifsig,&
        mype,nfldsig)

     ddiff=data(ipmsl,i)-pmslges  ! in cb
...
! If requested, setup for single obs test.
     if (oneobtest) then
        ddiff=one_tenth*maginnov
        error=one/(one_tenth*magoberr)
        ratio_errors=one
        muse(i) = .true.
     endif

     if (nobskeep>0 .and. luse_obsdiag) muse(i)=obsdiags(i_pmsl_ob_type,ibin)%tail%muse(nobskeep)
...
!    Compute penalty terms (linear & nonlinear qc).
     ...
     if(luse(i))then
        ...
        if (cvar_pg(ikx) > tiny_r_kind .and. error > tiny_r_kind) then
           ...
           cg_pmsl=cvar_b(ikx)
           wgross = cg_term*cvar_pg(ikx)/(cg_pmsl*wnotgross)
...
! Write information to diagnostic file
  if(conv_diagsave)then
     ...
     if(binary_diag .and. ii>0)then
        call dtime_show(myname,'diagsave:pmsl',i_pmsl_ob_type)
        write(7)'psl',nchar,nreal,ii,mype,ioff0
...
!    get pmsl ...
     ...
     if (istatus==0) then
         if(allocated(ges_pmsl))then
            write(6,*) trim(myname), ': ', trim(varname), ' already incorrectly alloc '
            call stop2(999)
         endif
         allocate(ges_pmsl(size(rank2,1),size(rank2,2),nfldsig))
         ges_pmsl(:,:,1)=rank2
         do ifld=2,nfldsig
...
900  format('conv_pmsl_',i2.2,'.nc4')
  ...
  subroutine contents_binary_diag_
        ...
        rdiagbuf(17,ii) = data(ipmsl,i)      ! PMSL observation (K)
        ...
        if (lobsdiagsave) then
           do jj=1,miter 
              ioff=ioff+1 
              if (obsdiags(i_pmsl_ob_type,ibin)%tail%muse(jj)) then
           ...
           do jj=1,miter+1
              ioff=ioff+1
              rdiagbuf(ioff,ii) = obsdiags(i_pmsl_ob_type,ibin)%tail%nldepart(jj)
           enddo
           do jj=1,miter
              ioff=ioff+1
              rdiagbuf(ioff,ii) = obsdiags(i_pmsl_ob_type,ibin)%tail%tldepart(jj)
           enddo
           do jj=1,miter
              ioff=ioff+1
              rdiagbuf(ioff,ii) = obsdiags(i_pmsl_ob_type,ibin)%tail%obssen(jj)
           enddo
        endif
...
! Observation class
  character(7),parameter     :: obsclass = '   pmsl'
...
!          call nc_diag_metadata("Nonlinear_QC_Var_Jb",     var_jb                 )
           ...
           call nc_diag_metadata("Observation",                   data(ipmsl,i)    )
           call nc_diag_metadata("Obs_Minus_Forecast_adjusted",   ddiff            )
           call nc_diag_metadata("Obs_Minus_Forecast_unadjusted", data(ipmsl,i)-pmslges )

           call nc_diag_metadata("Horizontal_local",  data(25,i)                   )
           call nc_diag_metadata("Vertical_local",    data(26,i)                   )
           ...
           if (lobsdiagsave) then
              ...
              call nc_diag_data2d("ObsDiagSave_nldepart", obsdiags(i_pmsl_ob_type,ibin)%tail%nldepart )
              call nc_diag_data2d("ObsDiagSave_tldepart", obsdiags(i_pmsl_ob_type,ibin)%tail%tldepart )
              call nc_diag_data2d("ObsDiagSave_obssen",   obsdiags(i_pmsl_ob_type,ibin)%tail%obssen   )             
           endif
  ...
  subroutine final_vars_
    ...
    if(allocated(ges_pmsl)) deallocate(ges_pmsl)
