...
!   2016-05-03  pondeca  - add uwnd10m, and vwnd10m
!   2019-06-25  twu      - add qr, qs, qg, and qh to state variables for hwrf
!
!   input argument list:
!     rval - State variable
!     bval
...
real(r_kind),pointer,dimension(:,:)   :: rv_pblh=>NULL(),rv_wspd10m=>NULL()
real(r_kind),pointer,dimension(:,:)   :: rv_tcamt=>NULL(),rv_lcbas=>NULL()
real(r_kind),pointer,dimension(:,:)   :: rv_td2m=>NULL(),rv_mxtm=>NULL()
real(r_kind),pointer,dimension(:,:)   :: rv_mitm=>NULL(),rv_pmsl=>NULL()
real(r_kind),pointer,dimension(:,:)   :: rv_howv=>NULL(),rv_cldch=>NULL()
real(r_kind),pointer,dimension(:,:)   :: rv_uwnd10m=>NULL(),rv_vwnd10m=>NULL()
real(r_kind),pointer,dimension(:,:,:) :: rv_u=>NULL(),rv_v=>NULL()
real(r_kind),pointer,dimension(:,:,:) :: rv_w=>NULL(),rv_prse=>NULL()
real(r_kind),pointer,dimension(:,:,:) :: rv_q=>NULL(),rv_tsen=>NULL()
real(r_kind),pointer,dimension(:,:,:) :: rv_tv=>NULL(),rv_oz=>NULL()
