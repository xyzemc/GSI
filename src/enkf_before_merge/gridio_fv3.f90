    ...
    IF (tsen_ind > 0 ) THEN
       DO k=1,nlevs
          ...
          DO j=1,ny_res
             DO i=1,nx_res
                nn=nn+1
                vargrid(nn,levels(tsen_ind-1)+k,nb)=workvar3d(i,j,k) 
             ENDDO
          ENDDO
       ENDDO
    ...
    IF (tv_ind >  0) THEN
       ...
       DO k=1,nlevs
          ...
          DO j=1,ny_res
             DO i=1,nx_res
                nn=nn+1
                vargrid(nn,levels(tv_ind-1)+k,nb)=tvworkvar3d(i,j,k) 
             ENDDO
          ENDDO
       ENDDO
       ...
       IF (aero_ind(itracer) > 0) THEN
          ...
          DO k=1,nlevs
             ...
             DO j=1,ny_res
                DO i=1,nx_res
                   nn=nn+1
                   vargrid(nn,levels(aero_ind(itracer)-1)+k,nb)=workvar3d(i,j,k) 
                ENDDO
             ENDDO
          ENDDO
...
!print *,'min/max delp',ntile,minval(delp),maxval(delp)
  ...
  subroutine writegriddata(nanal,vars3d,vars2d,n3d,n2d,levels,ndim,vargrid,no_inflate_flag)
    ...
    !----------------------------------------------------------------------
    ! Define counting variables
    INTEGER :: i,j,k,nn,ntile,nn_tile0, nb, itracer
       ...
       IF (aero_ind(itracer) > 0) THEN
          ...
          DO k=1,nlevs
             ...
             DO j=1,ny_res
                DO i=1,nx_res
                   nn=nn+1
                   workinc3d(i,j,k)=vargrid(nn,levels(aero_ind(itracer)-1)+k,nb) 
                ENDDO
             ENDDO
          ENDDO
          workvar3d=workvar3d+workinc3d
    ...
    if (ps_ind > 0) then
      ...
      do j=1,ny_res
         do i=1,nx_res
            nn=nn+1
!ps increment
            psworkinc(i,j)=vargrid(nn,levels(n3d)+ps_ind,nb )
         enddo
      enddo
