...
!                .      .    .                                       .
! subprogram:    module m_tdNode
!   prgmmr:      JJH <junjun.hu@noaa.gov>
!      org:      CIMMS/NSSL 
!     date:      2018-02-14
!
! abstract: class-module of obs-type tdNode (dew point temperature)
!
! program history log:
!   2018-02-14  JJH   - added this document block for the initial polymorphic
!                         implementation.
!
...
! module interface:
  ...
  type,extends(obsNode):: tdNode
     !type(td_ob_type),pointer :: llpoint => NULL()
     type(obs_diag), pointer :: diags => NULL() 
     real(r_kind)    :: res           !  dewpoint temperature residual
     real(r_kind)    :: err2          !  dew point temperature error squared
     ...
     real(r_kind)    :: tdpertb        !  random number adding to the obs
  ...
  public:: tdNode_typecast
  public:: tdNode_nextcast
        interface tdNode_typecast; module procedure typecast_ ; end interface
        interface tdNode_nextcast; module procedure nextcast_ ; end interface

  character(len=*),parameter:: MYNAME="m_tdNode"
...
!-- cast an obsNode_next(obsNode) to a type(tdNode)
...
subroutine obsNode_xread_(aNode,iunit,istat,diagLookup,skip)
  ...
  implicit none
  class(tdNode),intent(inout):: aNode
