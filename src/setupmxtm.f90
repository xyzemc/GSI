...
!$$$
  ...
  use obsmod, only: mxtmhead,mxtmtail,rmiss_single,mxtm_ob_type,i_mxtm_ob_type, & 
...
! Declare local variables
  ...
  type(mxtm_ob_type),pointer:: my_head
...
!    Link obs to diagnostics structure
     if(luse_obsdiag)then
        if (.not.lobsdiag_allocated) then
           ...
           obsdiags(i_mxtm_ob_type,ibin)%tail%indxglb=i
           obsdiags(i_mxtm_ob_type,ibin)%tail%nchnperobs=-99999
           obsdiags(i_mxtm_ob_type,ibin)%tail%luse=.false.
           ...
           if (obsdiags(i_mxtm_ob_type,ibin)%tail%indxglb/=i) then
...
!    in inner loop minimization (int* and stp* routines)
     if (.not. last .and. muse(i)) then
        ...
        if(.not. associated(mxtmhead(ibin)%head))then
           allocate(mxtmhead(ibin)%head,stat=istat)
           if(istat /= 0)write(6,*)' failure to write mxtmhead '
           mxtmtail(ibin)%head => mxtmhead(ibin)%head
        else
           allocate(mxtmtail(ibin)%head%llpoint,stat=istat)
           if(istat /= 0)write(6,*)' failure to write mxtmtail%llpoint '
           mxtmtail(ibin)%head => mxtmtail(ibin)%head%llpoint
        end if

	m_alloc(ibin) = m_alloc(ibin) + 1
	my_head => mxtmtail(ibin)%head
	my_head%idv = is
	my_head%iob = i

!       Set (i,j) indices of guess gridpoint that bound obs location
        call get_ij(mm1,dlat,dlon,mxtmtail(ibin)%head%ij(1),mxtmtail(ibin)%head%wij(1))

        mxtmtail(ibin)%head%res     = ddiff
        mxtmtail(ibin)%head%err2    = error**2
        mxtmtail(ibin)%head%raterr2 = ratio_errors**2    
        mxtmtail(ibin)%head%time    = dtime
        mxtmtail(ibin)%head%b       = cvar_b(ikx)
        mxtmtail(ibin)%head%pg      = cvar_pg(ikx)
        mxtmtail(ibin)%head%luse    = luse(i)
        if(luse_obsdiag)then
           mxtmtail(ibin)%head%diags => obsdiags(i_mxtm_ob_type,ibin)%tail
 
           my_head => mxtmtail(ibin)%head
           my_diag => mxtmtail(ibin)%head%diags
...
!    get mxtm ...
     ...
     if (istatus==0) then
         if(allocated(this%ges_mxtm))then
            write(6,*) trim(this%myname), ': ', trim(varname), ' already incorrectly alloc '
            call stop2(999)
         endif
         allocate(this%ges_mxtm(size(rank2,1),size(rank2,2),nfldsig))
         this%ges_mxtm(:,:,1)=rank2
         do ifld=2,nfldsig
            call gsi_bundlegetpointer(gsi_metguess_bundle(ifld),trim(varname),rank2,istatus)
            this%ges_mxtm(:,:,ifld)=rank2
         enddo
     else
