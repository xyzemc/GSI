...
! if true use model-space localization in serial filter
logical,public :: ensrf_modloc = .false.
...
namelist /nam_enkf/datestring,datapath,iassim_order,nvars,&
                   ...
                   fso_cycling,fso_calculate,imp_physics,lupp, ensrf_modloc
...
if (ensrf_modloc) then
  if (modelspace_vloc) then
    if (nproc .eq. 0) print *, 'error: can not use both ensrf_modloc and modulated ensemble'
    call stop2(20)
  endif
  if (letkf_flag) then
    if (nproc .eq. 0) print *, 'error: can not use ensrf_modloc with LETKF'
  endif
endif
