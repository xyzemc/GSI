...
!
!$$$
 use constants, only: zero,one,cp,fv,rd,max_varname_length,t0c,r0_05,grav,&
                      tiny_r_kind
 use params, only: nlons,nlats,nlevs,use_gfs_nemsio,pseudo_rh, &
                   nbackgrounds, anlfileprefixes,fgfileprefixes,reducedgrid, &
                   cliptracers,datapath,imp_physics,lupp
 ...
 subroutine readgriddata(nanal,vars3d,vars2d,n3d,n2d,levels,ndim,ntimes,fileprefixes,reducedgrid,grdin,qsat)
  ...
  integer(i_kind) :: k,iunitsig,iret,nb,nt,i,idvc,nlonsin,nlatsin,nlevsin
  logical ice
...
!$omp end parallel do
 ...
 subroutine writegriddata(nanal,vars3d,vars2d,n3d,n2d,levels,ndim,grdin,no_inflate_flag)
  ...
  real(r_kind), dimension(nlons*nlats) :: ug,vg,uginc,vginc,psfg,psg,work
...
!$omp end parallel do
     ...
     do k=1,nlevs
        ...
        nems_wrk = nems_wrk * ( 1.0 + fv*nems_wrk2 ) ! convert background T to Tv
        ug = ug + nems_wrk
        vg = vg + nems_wrk2 
        ...
        if (imp_physics == 11) then
           ! only icmr has to be in the file, the others are optional
