...
use loadbal, only: numobsperproc, numptsperproc, indxproc_obs, iprocob, &
                   lnp_chunk,  kdtree_grid, indxob_chunk, grdloc_chunk,  &
                   npts_max, ensmean_chunk, anal_chunk, &
                   global_proc_id, global_proc_index, &
                   dhx_dx_chunk, interp_chunk
use controlvec, only: cvars3d,  ncdim, index_pres
use enkf_obsmod, only: oberrvar, ob, obloc, ensmean_ob_linerr,&
                  nobstot, nobs_sat, obsprd_prior, &
                  obtype, corrlengthsq,lnsigl,obloclat,obloclon
use constants, only: pi, one, zero
use params, only: paoverpb_thresh, nanals,&
                  iassim_order,numiter,nlevs,&
                  univaroz,nbackgrounds,nhr_anal,fhr_assim,&
...
!      if (nproc == 0) print *, nobx, ' ob assimilated; ', hpfht
  ...
  tend = mpi_wtime()
  if (nproc .eq. 0) then
      write(6,8003) niter,'timing on proc',nproc,' = ',tend-tbegin,t2,t3,t3a, t3b, t4,t5,t6,t7
...
end subroutine enkf_modloc_update

end module enkf_modloc 
