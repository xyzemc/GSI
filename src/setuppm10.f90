...
use constants, only: max_varname_length
  ...
  interface setuppm10_class
     module procedure setup_ctor
  end interface
contains
  type(setuppm10_class) function setup_ctor(obsname,varname1,varname2,varname3,varname4)
  ...
  subroutine setuppm10(this,lunin,mype,nreal,nobs,isis,is,conv_diagsave)
...
!   call this%init_ges
  ...
  !check if aerosol species in control
       ...
       do i=1,naero_gocart_wrf
          ...
          call gsi_chemguess_get ('var::'//trim(this%aeroname), ipm10, ier )
       ...
       if (size(gsi_chemguess_bundle)==nfldsig) then
          ...
          call gsi_bundlegetpointer(gsi_chemguess_bundle(1),trim(this%aeroname),&
               rank3,ier)
          if (ier==0) then
             allocate(this%ges_pm10(size(rank3,1),size(rank3,2),size(rank3,3),&
                  nfldsig))
             this%ges_pm10(:,:,:,1)=rank3
             do ifld=2,nfldsig
                call gsi_bundlegetpointer(gsi_chemguess_bundle(ifld),trim(this%aeroname),rank3,ier)
                this%ges_pm10(:,:,:,ifld)=rank3
             enddo
          else
             write(6,*) 'setuppm10: ',trim(this%aeroname),' not found in chem bundle, ier= ',ier
             call stop2(453)
          endif
          ...
          call gsi_bundlegetpointer(gsi_chemguess_bundle(1),trim(this%aeroname),&
               rank3,ier)
          if (ier==0) then
             this%ges_pm10(:,:,:,1)=this%ges_pm10(:,:,:,1)+rank3
             do ifld=2,nfldsig
                call gsi_bundlegetpointer(gsi_chemguess_bundle(ifld),trim(this%aeroname),rank3,ier)
                this%ges_pm10(:,:,:,ifld)=this%ges_pm10(:,:,:,ifld)+rank3
             enddo
          else
             write(6,*) 'setuppm10: ',trim(this%aeroname),' not found in chem bundle, ier= ',ier
             call stop2(453)
          ...
          call gsi_bundlegetpointer(gsi_chemguess_bundle(1),trim(this%aeroname),&
               rank3,ier)
          if (ier==0) then
             this%ges_pm10(:,:,:,1)=this%ges_pm10(:,:,:,1)+rank3*nh4_mfac
             do ifld=2,nfldsig
                call gsi_bundlegetpointer(gsi_chemguess_bundle(ifld),trim(this%aeroname),rank3,ier)
                this%ges_pm10(:,:,:,ifld)=this%ges_pm10(:,:,:,ifld)+rank3*nh4_mfac
             enddo
          else
             write(6,*) 'setuppm10: ',trim(this%aeroname),' not found in chem bundle, ier= ',ier
             call stop2(453)
          ...
          call gsi_bundlegetpointer(gsi_chemguess_bundle(1),trim(this%aeroname),&
               rank3,ier)
          if (ier==0) then
             this%ges_pm10(:,:,:,1)=this%ges_pm10(:,:,:,1)+rank3*oc_mfac
             do ifld=2,nfldsig
                call gsi_bundlegetpointer(gsi_chemguess_bundle(ifld),trim(this%aeroname),rank3,ier)
                this%ges_pm10(:,:,:,ifld)=this%ges_pm10(:,:,:,ifld)+rank3*oc_mfac
             enddo
          else
             write(6,*) 'setuppm10: ',trim(this%aeroname),' not found in chem bundle, ier= ',ier
             call stop2(453)
          ...
          call gsi_bundlegetpointer(gsi_chemguess_bundle(1),TRIM(this%aeroname),&
               rank3,ier)
          if (ier==0) then
             this%ges_pm10(:,:,:,1)=this%ges_pm10(:,:,:,1)+rank3*d_10
             do ifld=2,nfldsig
                call gsi_bundlegetpointer(gsi_chemguess_bundle(ifld),TRIM(this%aeroname),rank3,ier)
                this%ges_pm10(:,:,:,ifld)=this%ges_pm10(:,:,:,ifld)+rank3*d_10
             enddo
          else
             write(6,*) 'setuppm10: ',TRIM(this%aeroname),' not found in chem bundle, ier= ',ier
             call stop2(453)
  ...
  !    link obs to diagnostics structure
          if(luse_obsdiag)then
             if (.not.lobsdiag_allocated) then
              ...
              if (.not.associated(obsdiags(i_pm10_ob_type,ibin)%tail)) then
                  call die(this%myname,'.not.associated(obsdiags(i_pm10_ob_type,ibin)%tail)')
              end if
  ...
  !if elevation unknown include observation nevertheless
          ...
          call tintrp2a11(this%ges_pm10,pm10ges,dlat,dlon,dtime,hrdifsig,&
               mype,nfldsig)
          innov = conc - pm10ges
