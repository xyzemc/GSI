...
!$$$
  ...
  use obsmod, only: howvhead,howvtail,rmiss_single,howv_ob_type,i_howv_ob_type, & 
...
! Declare local variables
  ...
  type(howv_ob_type),pointer:: my_head
...
!    Link obs to diagnostics structure
     if(luse_obsdiag)then
        if (.not.lobsdiag_allocated) then
           ...
           obsdiags(i_howv_ob_type,ibin)%tail%indxglb=i
           obsdiags(i_howv_ob_type,ibin)%tail%nchnperobs=-99999
           obsdiags(i_howv_ob_type,ibin)%tail%luse=.false.
           ...
           if (obsdiags(i_howv_ob_type,ibin)%tail%indxglb/=i) then
...
!    in inner loop minimization (int* and stp* routines)
     if (.not. last .and. muse(i)) then
        ...
        if(.not. associated(howvhead(ibin)%head))then
           allocate(howvhead(ibin)%head,stat=istat)
           if(istat /= 0)write(6,*)' failure to write howvhead '
           howvtail(ibin)%head => howvhead(ibin)%head
        else
           allocate(howvtail(ibin)%head%llpoint,stat=istat)
           if(istat /= 0)write(6,*)' failure to write howvtail%llpoint '
           howvtail(ibin)%head => howvtail(ibin)%head%llpoint
        end if

	m_alloc(ibin) = m_alloc(ibin) + 1
	my_head => howvtail(ibin)%head
	my_head%idv = is
	my_head%iob = i

!       Set (i,j) indices of guess gridpoint that bound obs location
        call get_ij(mm1,dlat,dlon,howvtail(ibin)%head%ij(1),howvtail(ibin)%head%wij(1))

        howvtail(ibin)%head%res     = ddiff
        howvtail(ibin)%head%err2    = error**2
        howvtail(ibin)%head%raterr2 = ratio_errors**2    
        howvtail(ibin)%head%time    = dtime
        howvtail(ibin)%head%b       = cvar_b(ikx)
        howvtail(ibin)%head%pg      = cvar_pg(ikx)
        howvtail(ibin)%head%luse    = luse(i)
        if(luse_obsdiag)then
           howvtail(ibin)%head%diags => obsdiags(i_howv_ob_type,ibin)%tail
