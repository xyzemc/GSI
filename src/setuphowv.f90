...
use abstract_setup_mod
  ...
  contains
    procedure, pass(this) :: setupDerived => setuphowv
  end type 
  interface setuphowv_class
     module procedure setup_ctor
  end interface
contains
  type(setuphowv_class) function setup_ctor(obsname,varname1,varname2,varname3)
  ...
  subroutine setuphowv(this,lunin,mype,bwork,awork,nele,nobs,is,conv_diagsave,luse,data)
  ...
  !    Link obs to diagnostics structure
       if(luse_obsdiag)then
          if (.not.lobsdiag_allocated) then
             ...
             if (.not.associated(obsdiags(i_howv_ob_type,ibin)%tail)) then
                call die(this%myname,'.not.associated(obsdiags(i_howv_ob_type,ibin)%tail)')
             endif
