...
!   call this%init_ges
  ...
  !    Link obs to diagnostics structure
     if ( luse_obsdiag ) then
          if (.not.lobsdiag_allocated) then
           ...
           if (.not.associated(obsdiags(i_tcp_ob_type,ibin)%tail)) then
              call die(this%myname,'.not.associated(obsdiags(i_tcp_ob_type,ibin)%tail)')
           end if
  ...
  ! Get guess sfc hght at obs location
       call intrp2a11(this%ges_z(1,1,ntguessig),zsges,dlat,dlon,mype)
  ...
  ! Get guess temperature at observation location and surface
       call tintrp31(this%ges_tv,tges,dlat,dlon,psges,dtime, &
  ...
  !  No observed temperature
       ...
       call tintrp31(this%ges_tv,tges2,dlat,dlon,psges2,dtime, &
            hrdifsig,mype,nfldsig)
...
!   call this%final_vars_
  ...
  ! Write information to diagnostic file
    if(conv_diagsave .and. ii>0)then
       call dtime_show(this%myname,'diagsave:tcp',i_tcp_ob_type)
