...
use abstract_setup_mod
  ...
  interface setupoz_class
     module procedure setup_ctor
     module procedure setup_ctor2
  end interface
contains
  type(setupoz_class) function setup_ctor(obsname,varname1)
  ...
  type(setupoz_class) function setup_ctor2(obsname,varname1,varname2)
      character(*),                        intent(in) :: obsname
      character(*),                        intent(in) :: varname1
      character(*),                        intent(in) :: varname2
      call setup_ctor2%initialize(obsname,varname1=varname1,varname2=varname2)
             ...
             do k=1,nlevs
                if (luse_obsdiag) then
                   if (.not.lobsdiag_allocated) then
                      ...
                      if (.not.associated(obsdiags(i_oz_ob_type,ibin)%tail)) then
                         call die(this%myname,'.not.associated(obsdiags(i_oz_ob_type,ibin)%tail)')
                      endif
             ...
             if(in_curbin) then
                ...
                if (.not. last .and. ikeep==1) then
                   ...
                   if(.not.associated(my_node)) &
                      call die(this%myname,'unexpected, associated(my_node) =',associated(my_node))
                   my_head => ozNode_typecast(my_node)
                   if(.not.associated(my_head)) &
                      call die(this%myname,'unexpected, associated(my_head) =',associated(my_head))
                   my_node => my_head
  ...
  !    Link obs to diagnostics structure
       if (luse_obsdiag) then
          if (.not.lobsdiag_allocated) then
             ...
             if (.not.associated(obsdiags(i_o3l_ob_type,ibin)%tail)) then
                call die(this%myname,'.not.associated(obsdiags(i_o3l_ob_type,ibin)%tail)')
             endif
