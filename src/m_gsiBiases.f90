...
!-------------------------------------------------------------------------
  ...
  real(r_kind),allocatable,dimension(:,:,:)   :: bias_uwnd10m
  real(r_kind),allocatable,dimension(:,:,:)   :: bias_vwnd10m
...
!$$$ end documentation block
  ...
  allocate(bias_ps(lat2,lon2,nbc),bias_tskin(lat2,lon2,nbc),&
           ...
           bias_uwnd10m(lat2,lon2,nbc),bias_vwnd10m(lat2,lon2,nbc),&
  ...
  do n=1,nbc
     do j=1,lon2
        do i=1,lat2
           ...
           bias_uwnd10m (i,j,n)=zero
           bias_vwnd10m (i,j,n)=zero
        end do
     end do
  end do
  do n=1,nbc
...
!$$$ end documentation block
   ...
   deallocate(bias_ps,bias_tskin,bias_gust,bias_vis,bias_pblh,bias_wspd10m, &
              bias_td2m,bias_mxtm,bias_mitm,bias_pmsl,bias_howv,bias_cldch,bias_uwnd10m,bias_vwnd10m,bias_vor,bias_div,&
...
!  call gsi_bundlegetpointer (xhat,'tsen',sv_tsen,istatus); ier=istatus+ier
   ...
   if (twodvar_regional) then
      ...
      call gsi_bundlegetpointer (xhat,'uwnd10m' ,i_uwnd10m, istatus)
      if (i_uwnd10m>0) then
         call gsi_bundlegetpointer (xhat,'uwnd10m' ,sv_uwnd10m, istatus); ier=istatus+ier
      end if

      call gsi_bundlegetpointer (xhat,'vwnd10m' ,i_vwnd10m, istatus)
      if (i_vwnd10m>0) then
         call gsi_bundlegetpointer (xhat,'vwnd10m' ,sv_vwnd10m, istatus); ier=istatus+ier
      end if
  ...
  if (twodvar_regional) then
     ...
     if (i_uwnd10m>0) call update2d_  (bias_uwnd10m,lat2,lon2,sv_uwnd10m,hour)
     if (i_vwnd10m>0) call update2d_  (bias_vwnd10m,lat2,lon2,sv_vwnd10m,hour)
  end if
...
! local variables
  ...
  real(r_kind),allocatable,dimension(:,:)  :: b_uwnd10m
  real(r_kind),allocatable,dimension(:,:)  :: b_vwnd10m
...
! Get memory for bias-related arrays
  ...
  allocate(b_ps(lat2,lon2),b_tskin(lat2,lon2),b_gust(lat2,lon2),&
           ...
           b_cldch(lat2,lon2),b_uwnd10m(lat2,lon2),b_vwnd10m(lat2,lon2))
...
!    Get pointer to could water mixing ratio
     ...
     if (twodvar_regional) then
        ...
        if (getindex(svars2d,'uwnd10m')>0) &
        call comp2d_(b_uwnd10m,bias_uwnd10m,hours(it))
        if (getindex(svars2d,'vwnd10m')>0) &
        call comp2d_(b_vwnd10m,bias_vwnd10m,hours(it))
     end if
     ...
     if (twodvar_regional) then
        ...
        call gsi_bundlegetpointer (gsi_metguess_bundle(it),'uwnd10m',ptr2dges,ier)
        if (ier==0.and.getindex(svars2d,'uwnd10m')>0) then
           ptr2dges = ptr2dges + b_uwnd10m
        end if
        call gsi_bundlegetpointer (gsi_metguess_bundle(it),'vwnd10m',ptr2dges,ier)
        if (ier==0.and.getindex(svars2d,'vwnd10m')>0) then
           ptr2dges = ptr2dges + b_vwnd10m
        end if
     end if
  end do
...
! Clean up bias-related arrays
  ...
  deallocate(b_ps,b_tskin,b_gust,b_vis,b_pblh,b_wspd10m,b_td2m,b_mxtm,b_mitm,b_pmsl,b_howv, &
             b_vor,b_div,b_cwmr,b_oz,b_q,b_tv,b_u,b_v,b_tcamt,b_lcbas,b_cldch,b_uwnd10m,b_vwnd10m)
