...
use abstract_setup_mod
  ...
  contains
    procedure, pass(this) :: setupDerived => setuppblh
  end type setuppblh_class
  interface setuppblh_class
     module procedure setup_ctor
  end interface
contains
  type(setuppblh_class) function setup_ctor(obsname,varname1,varname2)
  ...
  subroutine setuppblh(this,lunin,mype,bwork,awork,nele,nobs,is,conv_diagsave,luse,data)
...
!   2016-10-07  pondeca - if(.not.proceed) advance through input file first
  ...
  use obsmod, only: rmiss_single,i_pblh_ob_type,obsdiags,&
     ...
     if (luse_obsdiag) then
          if (.not.lobsdiag_allocated) then
             if (.not.associated(obsdiags(i_pblh_ob_type,ibin)%head)) then
              obsdiags(i_pblh_ob_type,ibin)%n_alloc = 0
           ...
           obsdiags(i_pblh_ob_type,ibin)%n_alloc = obsdiags(i_pblh_ob_type,ibin)%n_alloc +1
           ...
           obsdiags(i_pblh_ob_type,ibin)%tail%indxglb=ioid(i)
           ...
           if (.not.associated(obsdiags(i_pblh_ob_type,ibin)%tail)) then
              call die(this%myname,'.not.associated(obsdiags(i_pblh_ob_type,ibin)%tail)')
           endif
           if (obsdiags(i_pblh_ob_type,ibin)%tail%indxglb/=ioid(i)) then
  ...
  ! Interpolate to get pblh at obs location/time
       call tintrp2a11(this%ges_pblh,pblhges,dlat,dlon,dtime,hrdifsig,&
          mype,nfldsig)
  ...
  !    in inner loop minimization (int* and stp* routines)
	...
	m_alloc(ibin) = m_alloc(ibin) + 1
        ...
        call obsLList_appendNode(pblhhead(ibin),my_node)
        my_node => null()
  ...
  !       Set (i,j) indices of guess gridpoint that bound obs location
        ...
        if (luse_obsdiag) then
           my_head%diags => obsdiags(i_pblh_ob_type,ibin)%tail
  ...
  ! Write information to diagnostic file
    if(conv_diagsave .and. ii>0)then
       call dtime_show(this%myname,'diagsave:pblh',i_pblh_ob_type)
