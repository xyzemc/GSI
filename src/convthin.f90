...
!     thin      - number of obs to retain per thinning grid box
...
!     -->  update score, count, and best obs counters
    elseif (icount(itx,ip) > 0 .and. crit < score_crit(itx,ip)) then
       ...
       rusage(iiout)=101.0_r_kind
       rusage(iobs)=usage
!   print *,'map3_grids_m:iobs,iobsout,score_crit,ibest_obs,icount,ibest_save,rusage=',&
!           itx,ip,iobs,iobsout,iiout,score_crit(itx,ip),ibest_obs(itx,ip),icount(itx,ip),ibest_save(itx,ip),rusage(iobs),rusage(iiout)
