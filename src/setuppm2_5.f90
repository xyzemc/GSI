...
use abstract_setup_mod
  ...
  interface setuppm2_5_class
     module procedure setup_ctor
  end interface
contains
  type(setuppm2_5_class) function setup_ctor(obsname,varname1,varname2,varname3,varname4)
    ...
    !    link obs to diagnostics structure
          if (luse_obsdiag) then
               if (.not.lobsdiag_allocated) then
                ...
                if (.not.associated(obsdiags(i_pm2_5_ob_type,ibin)%tail)) then
                   call die(this%myname,'.not.associated(obsdiags(i_pm2_5_ob_type,ibin)%tail)')
                end if
