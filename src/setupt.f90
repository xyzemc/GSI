...
contains
  ...
  !   2006-07-31  kleist - use this%ges_ps instead of lnps
...
!                           muse(k),muse(l)
  ...
  !    Link obs to diagnostics structure
       ...
       if (.not.lobsdiag_allocated) then
          ...
          if (.not.associated(obsdiags(i_t_ob_type,ibin)%tail)) then
             call die(this%myname,'.not.associated(obsdiags(i_t_ob_type,ibin)%tail)')
          end if
  ...
  ! Implementation of forward model ----------
       ...
       if(sfctype.and.sfcmodel) then
        ...
        if(itype == 180 .or. itype == 182 .or. itype == 183 .or. itype == 199) then    !sea
          ...
          call tintrp2a1(this%ges_tv,tvtmp,dlat,dlon,dtime,hrdifsig,&
               nsig,mype,nfldsig)
          call tintrp2a1(this%ges_q,qtmp,dlat,dlon,dtime,hrdifsig,&
               nsig,mype,nfldsig)
          call tintrp2a1(this%ges_u,utmp,dlat,dlon,dtime,hrdifsig,&
               nsig,mype,nfldsig)
          call tintrp2a1(this%ges_v,vtmp,dlat,dlon,dtime,hrdifsig,&
               nsig,mype,nfldsig)
  ...
  !          Interpolate guess tv to observation location and time
             call tintrp31(this%ges_tv,tges,dlat,dlon,dpres,dtime, &
...
!          Interpolate guess th 2m to observation location and time
              call tintrp2a11_csln(this%ges_th2,tges2m,tges2m_water,dlat,dlon,dtime,hrdifsig,&
                mype,nfldsig)
              ...
              if(iqtflg)then
                 call tintrp2a11_csln(this%ges_q2,qges2m,qges2m_water,dlat,dlon,dtime,hrdifsig,&
                     mype,nfldsig)
  ...
  !          Interpolate guess th 2m to observation location and time
              call tintrp2a11(this%ges_th2,tges2m,dlat,dlon,dtime,hrdifsig,&
                mype,nfldsig)
              tges2m=tges2m*(r10*psges/r1000)**rd_over_cp_mass  ! convert to sensible T         
              if(iqtflg)then
