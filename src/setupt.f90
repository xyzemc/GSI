...
contains
 ...
 !    get th2m ...
         ...
         if (istatus==0) then
             ...
             allocate(this%ges_th2(size(rank2,1),size(rank2,2),nfldsig))
             this%ges_th2(:,:,1)=rank2
             do ifld=2,nfldsig
                call gsi_bundlegetpointer(gsi_metguess_bundle(ifld),trim(varname),rank2,istatus)
                this%ges_th2(:,:,ifld)=rank2
             enddo
         else
             write(6,*) trim(this%myname),': ', trim(varname), ' not found in met bundle, ier= ',istatus
             call stop2(999)
 ...
 !    get q2m ...
         ...
         if (istatus==0) then
             ...
             allocate(this%ges_q2(size(rank2,1),size(rank2,2),nfldsig))
             this%ges_q2(:,:,1)=rank2
             do ifld=2,nfldsig
                call gsi_bundlegetpointer(gsi_metguess_bundle(ifld),trim(varname),rank2,istatus)
                this%ges_q2(:,:,ifld)=rank2
             enddo
         else
             write(6,*) trim(this%myname),': ', trim(varname), ' not found in met bundle, ier= ',istatus
             call stop2(999)
