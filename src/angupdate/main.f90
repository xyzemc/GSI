...
! Declare local variables
  logical outask,lexist,done,update,restart
  ...
  real:: wgtang,wgtlap,scan,dtmax,bias,tlap,x,y,dth,ratio,rsize
...
!       Open file and read header
        ...
        call read_diag_header(lndiag,header_fix,header_chan,istatus)
        if (istatus/=0) then
           write(6,*)' Task ',mype,' problem reading file ',trim(diag_rad),' header, iostat=',istatus
           close(lndiag)
           cycle loopt
        endif
...
!          Read a record.  If read flag, istatus does not equal zero, exit loopd
           call read_diag_data( lndiag, header_fix, data_fix, data_chan, data_extra, istatus )
...
!             done to increase the sample size for the statistics.
              ...
              if (obstype=='goesimg') then
                 if (ispot<=10) then
                    do i=1,10
                       c_ang1(i,jj) = c_ang1(i,jj) + bias*data_chan(j)%errinv
                       csum(i,jj)   = csum(i,jj)   + data_chan(j)%errinv
                       count(i,jj)  = count(i,jj)  + one
                    end do
                    tlap1(jj)       = tlap1(jj)    + tlap*data_chan(j)%errinv
                    tsum(jj)        = tsum(jj)     + data_chan(j)%errinv
                    tcnt(jj)        = tcnt(jj)     + one
                 ...
                 c_ang1(ispot,jj)= c_ang1(ispot,jj)+ bias*data_chan(j)%errinv
                 csum(ispot,jj)  = csum(ispot,jj)  + data_chan(j)%errinv
