!
! CRTM_VIS_Ice_SfcOptics
!
! Module to compute the surface optical properties for ICE surfaces at
! visible frequencies required for determining the ICE surface
...
MODULE CRTM_VIS_Ice_SfcOptics
  ...
  USE CRTM_VISiceCoeff         , ONLY: VISiceC
  ...
  PUBLIC :: iVar_type
  ! Science routines
  PUBLIC :: Compute_VIS_Ice_SfcOptics
  PUBLIC :: Compute_VIS_Ice_SfcOptics_TL
  PUBLIC :: Compute_VIS_Ice_SfcOptics_AD
  ...
  '$Id: CRTM_VIS_Ice_SfcOptics.f90 55163 2015-04-21 13:59:28Z paul.vandelst@noaa.gov $'
...
!----------------------------------------------------------------------------------
  ...
  FUNCTION Compute_VIS_Ice_SfcOptics( &
    Surface     , &  ! Input
    SensorIndex , &  ! Input
    ChannelIndex, &  ! Input
    SfcOptics   , &  ! Output
  ...
  RESULT( err_stat )
    ...
    INTEGER :: err_stat
    ! Local parameters
    CHARACTER(*), PARAMETER :: ROUTINE_NAME = 'Compute_VIS_Ice_SfcOptics'
    ! Local variables
    CHARACTER(ML) :: msg
    INTEGER  :: j
  ...
  END FUNCTION Compute_VIS_Ice_SfcOptics
...
!       reflectivity at UV/visible frequencies over an ice surface.
!
!       This function is a wrapper for third party code.
!
!       NB: CURRENTLY THIS IS A STUB FUNCTION AS THERE ARE NO TL
!           COMPONENTS IN THE VIS ICE SFCOPTICS COMPUTATIONS.
!
! CALLING SEQUENCE:
...
!----------------------------------------------------------------------------------
  ...
  FUNCTION Compute_VIS_Ice_SfcOptics_TL( &
    SfcOptics_TL ) &  ! Output
  RESULT( err_stat )
    ! Arguments
...
!       reflectivity at UW/visible frequencies over an ice surface.
!
!       This function is a wrapper for third party code.
!
...
!----------------------------------------------------------------------------------
  ...
  FUNCTION Compute_VIS_Ice_SfcOptics_AD( &
    SfcOptics_AD ) &  ! Input
  RESULT( err_stat )
    ! Arguments
...
END MODULE CRTM_VIS_Ice_SfcOptics
