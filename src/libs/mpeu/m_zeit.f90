...
!_______________________________________________________________________
  ...
  if(present(usrtime))then
    if(.not.started) call reset_(usrtime)
  else
...
! !INTERFACE:
    ...
    subroutine co_(name,tms,usrtime)
      use m_stdio ,only : stderr
      use m_die   ,only : die
      use m_mpif90,only : MP_wtime
      implicit none
...
! !INTERFACE:
    ...
    subroutine flush_(lu,umask,usrtime)
      use m_stdio ,only : stderr
...
!  character(len=max(NSZ,lnmax)) :: name
  ...
  write(lu,'(a,t14,a,t24,a,t34,a,t52,a)')	&
...
!_______________________________________________________________________
  ...
  do l=1,mname		! For all local accounts other than ZEIT,
	...
	! Update the account for this timing module.
...
!   \begin{eqnarray*}
!	x &=& \frac{\max(t) - \tb}{N\tb}	\times 100\%	\!	i &=& \frac{\max(t) - \tb}{\max(t)}	\times 100\%	\!	r &=& \frac{1}{N\tb} \sum^{t>\tb}{(t-\tb)}
!		\times 100\%
!   \end{eqnarray*}
!   where
!   \begin{center}
!     \begin{tabular}{rl}
!       $t$: & time by any process element			\!     $\tb$: & mean time by all process elements		\!	$x$: & the ma{\bf x}imum percentage load deviation	\!	$i$: & percentage {\bf i}dle process-time or
!					load {\bf i}mbalance	\!	$r$: & percentage {\bf r}elocatable loads		\!	$N$: & {\bf n}umber of process elements
!     \end{tabular}
!   \end{center}
!
! !INTERFACE:
...
!EOP ___________________________________________________________________
  ...
  do l=0,nl
    do k=0,nk
      do i=0,ni
	zabv(i,k,l)=max(0.,real(ztmp(i,k,l)-zavg(i,k,l)))
      end do
    end do
  end do
