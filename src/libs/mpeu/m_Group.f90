...
!EOP ___________________________________________________________________
  ...
  integer :: n,i,isave,jsave,ksave
	...
	If (.not. (size(key)==size(gid)) ) call assert_("size(key)==size(gid)","m_Group.F90",135)
...
!EOP ___________________________________________________________________
  ...
  if(present(displs)) then
    If (.not. (size(displs)==size(counts)) ) call assert_("size(displs)==size(counts)","m_Group.F90",188)
  endif
