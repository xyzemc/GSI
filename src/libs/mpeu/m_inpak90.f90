...
!     ------------------------------    
!      print *,'buffer= ',i90_now%buffer(1:i90_now%nbuf)
!      print *,'label= ',EOL//label 
      i = index_ ( i90_now%buffer(1:i90_now%nbuf), EOL//label ) + 1
      if ( i .eq. 1 ) then
           i90_now%this_line = BLK // EOL
           iret = -2
           return
      elseif(i.le.0) then
	   call die(myname_,'invalid index_() return',i)
      end if
