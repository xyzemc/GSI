...
!
  type nemsio_dbta_sfc2d

    real(dblekind),pointer:: sfcdata2d(:,:)

  end type nemsio_dbta_sfc2d
!
...
!   
!-----------------------------------------------------------------------   
  subroutine nemsio_gfs_rdsfc8(gfile,nemsiohead,nemsiodbta,iret)
!-----------------------------------------------------------------------   
!
...
!   
!-----------------------------------------------------------------------   
  subroutine nemsio_gfs_rdsfc4(gfile,nemsiohead,nemsiodata,iret)
!-----------------------------------------------------------------------   
!
    use nemsio_module, only : nemsio_gfile,nemsio_getfilehead,nemsio_readrecv
    implicit none
...
!   
!-----------------------------------------------------------------------   
  subroutine nemsio_gfs_wrtsfc8(gfile,nemsiohead,nemsiodbta,iret)
!-----------------------------------------------------------------------   
!
...
!
!-----------------------------------------------------------------------   
  subroutine nemsio_gfs_wrtsfc4(gfile,nemsiohead,nemsiodata,iret)
!-----------------------------------------------------------------------   
!
    use nemsio_module, only : nemsio_gfile,nemsio_getfilehead,nemsio_writerecv
    implicit none
...
!-----------------------------------------------------------------------
  ...
  subroutine nemsio_gfs_rdgrd8(gfile,nemsiohead,nemsiodbta,iret)
!-----------------------------------------------------------------------
!
    use nemsio_module, only : nemsio_gfile,nemsio_getfilehead,nemsio_readrecv
    implicit none
!
    ...
    integer, optional,intent(out):: iret
!local
    integer im,jm,n,l,fieldsize,ierr
    real(dblekind),allocatable ::tmp(:)
!
!---read out data from nemsio file
!
    ...
    do n=1,nemsiohead%ntrac
      do l=1,nemsiohead%dimz
        ...
        if(n==2) then  
          ...
          if(ierr==0) then
            if(nemsiohead%idvt==0.or.mod(nemsiohead%idvt,10)==1.or.  &
             nemsiohead%idvt>=100) then
              nemsiodbta%q(:,:,l,n)=reshape(tmp,(/im,jm/) )
            elseif(mod(nemsiohead%idvt,10)==2.and.nemsiohead%idvt>10) then
              nemsiodbta%q(:,:,l,3)=reshape(tmp,(/im,jm/) )
            endif
          else
            if(present(iret)) iret=ierr
            print *, 'ERROR in rdgrd (ps), iret=', ierr
        ...
        if(n==3) then
          ...
          if(ierr==0) then
            if(nemsiohead%idvt==0.or. nemsiohead%idvt/10==2.or.nemsiohead%idvt>=100) then
              nemsiodbta%q(:,:,l,n)=reshape(tmp,(/im,jm/) )
            elseif(mod(nemsiohead%idvt,10)==2) then
              nemsiodbta%q(:,:,l,2)=reshape(tmp,(/im,jm/) )
            endif
          else
            if(present(iret)) iret=ierr
            print *, 'ERROR in rdgrd (ps), iret=', ierr
...
!-----------------------------------------------------------------------
  subroutine nemsio_gfs_rdgrd4(gfile,nemsiohead,nemsiodata,iret)
!-----------------------------------------------------------------------
!
    use nemsio_module, only : nemsio_gfile,nemsio_getfilehead,nemsio_readrecv
    implicit none
          ...
          if(ierr==0) then
            if(nemsiohead%idvt==0.or.mod(nemsiohead%idvt,10)==1.or.  &
             nemsiohead%idvt>=100) then
              nemsiodata%q(:,:,l,n)=reshape(tmp,(/im,jm/) )
            elseif(mod(nemsiohead%idvt,10)==2.and.nemsiohead%idvt>10) then
              nemsiodata%q(:,:,l,3)=reshape(tmp,(/im,jm/) )
            endif
          else
            if(present(iret)) iret=ierr
            print *, 'ERROR in rdgrd (ps), iret=', ierr
        ...
        if(n==3) then
          ...
          if(ierr==0) then
            if(nemsiohead%idvt==0.or. nemsiohead%idvt/10==2.or.nemsiohead%idvt>=100) then
              nemsiodata%q(:,:,l,n)=reshape(tmp,(/im,jm/) )
            elseif(mod(nemsiohead%idvt,10)==2) then
              nemsiodata%q(:,:,l,2)=reshape(tmp,(/im,jm/) )
            endif
          else
            if(present(iret)) iret=ierr
            print *, 'ERROR in rdgrd (ps), iret=', ierr
...
!-----------------------------------------------------------------------
  subroutine nemsio_gfs_wrtgrd8(gfile,nemsiohead,nemsiodbta,iret)
!-----------------------------------------------------------------------
!
    use nemsio_module, only : nemsio_gfile,nemsio_getfilehead,nemsio_writerecv
    implicit none
!
    ...
    integer, optional,intent(out):: iret
!local
    integer im,jm,nframe,n,l,fieldsize,ierr
    real(dblekind),allocatable ::tmp(:)
!
!---read out data from nemsio file
!

    call nemsio_getfilehead(gfile,dimx=im,dimy=jm,nframe=nframe,iret=ierr)
    if(ierr/=0) then
       if(present(iret)) iret=ierr
       print *,'ERROR: cannot get dimension from gfile'
       return
      ...
      do l=1,nemsiohead%dimz
        ...
        if(n==2) then  
          if(nemsiohead%idvt==0.or.mod(nemsiohead%idvt,10)==1.or.  &
            ...
            tmp=reshape(nemsiodbta%q(:,:,l,n),(/fieldsize/) )
          elseif(mod(nemsiohead%idvt,10)==2.and.nemsiohead%idvt>10) then
            tmp=reshape(nemsiodbta%q(:,:,l,3),(/fieldsize/) )
          endif
        ...
        if(n==3) then
          if(nemsiohead%idvt==0.or. nemsiohead%idvt/10==2.or.nemsiohead%idvt>=100) then
            tmp=reshape(nemsiodbta%q(:,:,l,n),(/fieldsize/) )
          elseif(mod(nemsiohead%idvt,10)==2) then
            tmp=reshape(nemsiodbta%q(:,:,l,2),(/fieldsize/) )
          endif
          call nemsio_writerecv(gfile,'clwmr','mid layer',l,tmp,iret=ierr)
        endif
        if(ierr/=0) then
          if(present(iret)) iret=ierr
          print *,'write trac=',n,'l=',l,'tracer,ierr=',ierr
        endif
      enddo
    enddo
!w
...
!-----------------------------------------------------------------------
  subroutine nemsio_gfs_wrtgrd4(gfile,nemsiohead,nemsiodata,iret)
!-----------------------------------------------------------------------
!
    use nemsio_module, only : nemsio_gfile,nemsio_getfilehead,nemsio_writerecv
    implicit none
      ...
      do l=1,nemsiohead%dimz
        ...
        if(n==2) then  
          if(nemsiohead%idvt==0.or.mod(nemsiohead%idvt,10)==1.or.  &
            ...
            tmp=reshape(nemsiodata%q(:,:,l,n),(/fieldsize/) )
          elseif(mod(nemsiohead%idvt,10)==2.and.nemsiohead%idvt>10) then
            tmp=reshape(nemsiodata%q(:,:,l,3),(/fieldsize/) )
          endif
          call nemsio_writerecv(gfile,'o3mr','mid layer',l,tmp,iret=ierr)
        endif
        if(n==3) then
          if(nemsiohead%idvt==0.or. nemsiohead%idvt/10==2.or.nemsiohead%idvt>=100) then
            tmp=reshape(nemsiodata%q(:,:,l,n),(/fieldsize/) )
          elseif(mod(nemsiohead%idvt,10)==2) then
            tmp=reshape(nemsiodata%q(:,:,l,2),(/fieldsize/) )
          endif
          call nemsio_writerecv(gfile,'clwmr','mid layer',l,tmp,iret=ierr)
        endif
        if(ierr/=0) then
          if(present(iret)) iret=ierr
          print *,'write  ntrac=',n,' l=',l,'tracers,ierr=',ierr
        endif
      enddo
    enddo
!w
