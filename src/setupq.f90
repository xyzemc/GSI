...
! Declare local variables  
  ...
  real(r_kind),dimension(nsig):: prsltmp, qgtmp, qtmp
...
!    Link obs to diagnostics structure
     if (luse_obsdiag) then
        if (.not.lobsdiag_allocated) then
           ...
           if (.not.associated(obsdiags(i_q_ob_type,ibin)%tail)) then
              call die(myname,'.not.associated(obsdiags(i_q_ob_type,ibin)%tail)')
           end if
...
! CSD - new calls for ncdiag output

     call tintrp2a1(qg,qgtmp,dlat,dlon,dtime,hrdifsig,&
          nsig,mype,nfldsig)
...
! Observation class
  ...
  real(r_kind),dimension(miter) :: obsdiag_iuse
           ...
           call nc_diag_metadata("Pressure",                presq                  )
           ...
           call nc_diag_metadata("Observation",                   data(iqob,i)     )
           call nc_diag_metadata("Obs_Minus_Forecast_adjusted",   ddiff            )
           call nc_diag_metadata("Obs_Minus_Forecast_unadjusted", qob-qges         )
           call nc_diag_metadata("Forecast_Saturation_Spec_Hum",  qsges            )
           if (lobsdiagsave) then
              do jj=1,miter
                 if (obsdiags(i_q_ob_type,ibin)%tail%muse(jj)) then
                       obsdiag_iuse(jj) =  one
              ...
              call nc_diag_data2d("ObsDiagSave_nldepart", obsdiags(i_q_ob_type,ibin)%tail%nldepart )
              call nc_diag_data2d("ObsDiagSave_tldepart", obsdiags(i_q_ob_type,ibin)%tail%tldepart )
              call nc_diag_data2d("ObsDiagSave_obssen",   obsdiags(i_q_ob_type,ibin)%tail%obssen   )             
           endif
      ...
      call nc_diag_data2d("qgtmp", qgtmp)
