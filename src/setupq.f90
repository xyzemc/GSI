...
! Observation class
  ...
  real(r_kind),dimension(miter) :: obsdiag_iuse
           ...
           call nc_diag_metadata("Pressure",                real(presq,r_single)                  )
           ...
           call nc_diag_metadata("Observation",                   real(data(iqob,i),r_single)     )
           call nc_diag_metadata("Obs_Minus_Forecast_adjusted",   real(ddiff,r_single)            )
           call nc_diag_metadata("Obs_Minus_Forecast_unadjusted", real(qob-qges,r_single)         )
           call nc_diag_metadata("Forecast_Saturation_Spec_Hum",  real(qsges,r_single)            )
           if (lobsdiagsave) then
              do jj=1,miter
                 if (obsdiags(i_q_ob_type,ibin)%tail%muse(jj)) then
                       obsdiag_iuse(jj) =  one
