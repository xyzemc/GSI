...
!                              "processor"
!     sbuv2."id"."processor" - sbuv ozone diagnostic file for satellite
...
!==================================================================================================
  ...
  use jfunc, only: iout_iter,iguess,miter,factqmin,factqmax,niter,niter_no_qc,biascor,&
     init_jfunc,qoption,switch_on_derivatives,tendsflag,l_foto
  use berror, only: norh,ndeg,vs,as,bw,init_berror,hzscl,hswgt,pert_berr,pert_berr_fct,&
     bkgv_flowdep,bkgv_rewgtfct,bkgv_write,tsfc_sdv
...
!            add use of guess file later for regional mode.
  ...
  namelist/setup/gencode,factqmin,factqmax,deltim,dtphys,biascor, &
       ...
       oneobtest,switch_on_derivatives,tendsflag,&
       sfcmodel,dtbduv_on,ifact10,l_foto,offtime_data,&
       npred_conv_max,&
       id_bias_ps,id_bias_t,id_bias_spd, &
       conv_bias_ps,conv_bias_t,conv_bias_spd, &
...
! Force use of perturb_obs for oberror_tune
  if (oberror_tune ) then
     perturb_obs=.true.
     if (mype==0) write(6,*)'GSIMAIN:  ***WARNING*** reset perturb_obs=',perturb_obs
  endif
