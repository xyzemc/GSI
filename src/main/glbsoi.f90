...
!   2008-06-30  derber - replace call to destroy_sfc_grids with destroy_sfct
!
!   input argument list:
!     mype - mpi task id
!
...
!$$$
  ...
  use guess_grids, only: sfct,create_ges_bias_grids,create_sfc_grids,&
       destroy_sfct,destroy_ges_bias_grids,nfldsig
  ...
  use radinfo, only: n_sensors,sensorlist
...
! Declare local variables
  logical tlm_check,adj_check,slow_pole_in

  integer(i_kind) error_status,i,nlev_mp
...
! Allocate arrays used in minimization
  ...
  if ( (jcstrong .or. jcdivt) .and. nvmodes_keep.gt.0) then
...
! Read observations
  call read_obs(ndata,mype,channelinfo)
