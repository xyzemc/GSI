...
!   2008-06-30  derber - separate out sfct from destroy sfc to allow rest of 
!                       fields go to removed earlier
!
! !AUTHOR: 
...
!-------------------------------------------------------------------------
   integer(i_kind):: i,j,k,istatus
    ...
    allocate( isli(lat2,lon2,nfldsfc),fact10(lat2,lon2,nfldsfc),&
         ...
         soil_moi(lat2,lon2,nfldsfc),isli_g(nlat,nlon,nfldsfc),&
         dsfct(lat2,lon2),isli2(lat2,lon2),sno2(lat2,lon2,nfldsfc),&
         stat=istatus)
...
!-------------------------------------------------------------------------
    ...
    deallocate(ges_z,ges_ps,ges_prsi,ges_prsl,ges_lnprsl,ges_lnprsi,&
         ...
         ges_oz,ges_tv,ges_tsen,ges_teta,geop_hgtl,geop_hgti,ges_prslavg,&
         tropprs,dsfct,isli2,sno2,stat=istatus)
    if (istatus/=0) &
...
!BOP
!
! !IROUTINE: destroy_sfct --- Deallocate surface temperature field
!
! !INTERFACE:
!
...
! !DESCRIPTION: deallocate surface temperature grids
!
! !REVISION HISTORY:
!   2008-06-30  derber
!
...
!   2008-06-30  derber - remove sfct_t deallocate to allow earlier call
!
! !REMARKS:
!   language: f90
!   machine:  ibm rs/6000 sp; SGI Origin 2000; Compaq/HP
...
!   CRTM toa_pressure unit is hPa.   prsitmp unit is cb
    if (r10*prsitmp(nsig) < toa_pressure)then
...
!   Declare local variables
    ...
    real(r_kind):: dpres,dpres2,pgesin,ugesin,vgesin,qgesin,tgesin,prsigesin1
...
!   Get time interpolation factors for sigma files
    ...
    ts =(dsfct(ix,iy )*w00 + dsfct(ixp,iy )*w10 +          &
         dsfct(ix,iyp)*w01 + dsfct(ixp,iyp)*w11) + skint
    ...
    if (sfcmod_gfs) then
       ...
       call SFC_WTQ_FWD (pgesin,ts,lnpgesin1,tgesin,qgesin,ugesin, &
                vgesin,lnpgesin2,tgesin2,qgesin2,geopgesin,sfcrough,islimsk, &
