...
contains
  ...
  ! Interpolate guess td2m to observation location and time
       call tintrp2a11(this%ges_td2m,td2mges,dlat,dlon,dtime,hrdifsig,&
          mype,nfldsig)
  ...
  ! Write information to diagnostic file
    if(conv_diagsave .and. ii>0)then
       call dtime_show(this%myname,'diagsave:td2m',i_td2m_ob_type)
 ...
 !    get td2m ...
      ...
      if (istatus==0) then
          if(allocated(this%ges_td2m))then
             write(6,*) trim(this%myname), ': ', trim(varname), ' already incorrectly alloc '
             call stop2(999)
          endif
          allocate(this%ges_td2m(size(rank2,1),size(rank2,2),nfldsig))
          this%ges_td2m(:,:,1)=rank2
          do ifld=2,nfldsig
             call gsi_bundlegetpointer(gsi_metguess_bundle(ifld),trim(varname),rank2,istatus)
             this%ges_td2m(:,:,ifld)=rank2
          enddo
      else
