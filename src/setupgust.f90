...
!   2016-10-07  pondeca - if(.not.proceed) advance through input file first
  ...
  !    Link obs to diagnostics structure
     if (luse_obsdiag) then
          if (.not.lobsdiag_allocated) then
           ...
           if (.not.associated(obsdiags(i_gust_ob_type,ibin)%tail)) then
              call die(this%myname,'.not.associated(obsdiags(i_gust_ob_type,ibin)%tail)')
           end if
  ...
  ! Interpolate to get gust at obs location/time
       call tintrp2a11(this%ges_gust,gustges,dlat,dlon,dtime,hrdifsig,&
            mype,nfldsig)
  ...
  ! Write information to diagnostic file
    if(conv_diagsave .and. ii>0)then
       call dtime_show(this%myname,'diagsave:gust',i_gust_ob_type)
